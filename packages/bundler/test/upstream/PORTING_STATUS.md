...
does not exist yet (the Zig port at `packages/bundler/src/bun/`
is still in the "blocked on `bun` stdlib aggregator" state per
`packages/bundler/src/bun/PORTING_STATUS.md`).

The intended runner shape, once the bundler can produce output:

1. **`packages/bundler/test/run_bun_corpus.zig`** — a Zig test
   that walks `packages/bundler/test/bun/`, parses each
