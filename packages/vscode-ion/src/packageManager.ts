...
export class IonPackageManager {
    private readonly REGISTRY_URL = 'https://registry.ion-lang.org';
    ...
    public async publishPackage(): Promise<void> {
        ...
        // Check if package.ion exists
        const packageFilePath = path.join(workspaceFolder.uri.fsPath, 'package.ion');
        if (!fs.existsSync(packageFilePath)) {
            vscode.window.showErrorMessage('No package.ion file found in workspace');
            return;
        }
