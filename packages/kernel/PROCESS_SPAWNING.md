...
pub fn runCommand(parent: _kernel.process.Process, allocator: std.mem.Allocator) !void {
    // Fork the process
    const child = try kernel.fork.fork(parent, allocator);
...
```zig
fn kernelThreadEntry(arg: _anyopaque) void {
    const data = @as(_MyData, @ptrCast(@alignCast(arg)));
    // Do work...
}
...
```zig
fn createContainer(allocator: std.mem.Allocator) !_kernel.process.Process {
