...
var next_native_callback_id: usize = 1;
var next_native_external_id: usize = 1;
var next_native_module_id: usize = 1;
...
fn loadNativeNodeModule(
    ...
    const module_id = next_native_module_id;
    next_native_module_id += 1;
    setNumberProperty(actual_ctx, module_object, "__home_native_module_id", module_id);
    native_module_meta.put(allocator, module_id, .{
        .lib_index = lib_index,
...
fn callNativeOnBeforeParse(
    ...
    const module_id = readNativeObjectId(actual_ctx, module_object, "__home_native_module_id") orelse 0;
    const meta = native_module_meta.get(module_id) orelse {
    ...
    if (arguments[2]) |external_value| {
        if (!extern_fns.JSValueIsUndefined(actual_ctx, external_value) and !extern_fns.JSValueIsNull(actual_ctx, external_value)) {
            ...
            const external_id = readNativeObjectId(actual_ctx, external_object, "__home_napi_external_id") orelse 0;
            if (native_externals.get(external_id)) |external| {
                native_external = external.data;
                std.debug.print("onBeforeParse external 0x{x}\n", .{@intFromPtr(external.data orelse null)});
            } else {
...
fn nativeNapiFunctionCallback(
    ...
    const actual_ctx = ctx.?;
    const actual_function = function orelse return null;
    const callback_id = readNativeObjectId(actual_ctx, actual_function, "__home_napi_callback_id") orelse 0;
    const callback = native_callbacks.get(callback_id) orelse return null;
...
fn nativeNapiFunctionCallbackById(
    callback_id: usize,
    ctx: ?*JSContextRef,
    this: ?*JSObject,
    argument_count: usize,
...
fn invokeNapiCallback(
    ...
    _ = this;
    const actual_ctx = ctx.?;
    if (argument_count < 3 or arguments[0] == null or arguments[2] == null) {
        setException(actual_ctx, exception, "N-API callback dispatcher requires id, this, and arguments");
        return null;
    }

    const callback_id_float = extern_fns.JSValueToNumber(actual_ctx, arguments[0], exception);
    if (!std.math.isFinite(callback_id_float) or callback_id_float <= 0) {
        setException(actual_ctx, exception, "N-API callback dispatcher received an invalid id");
        return null;
    }
    const callback_id: usize = @intFromFloat(callback_id_float);
    std.debug.print("invoke napi callback {d}\n", .{callback_id});
    const callback = native_callbacks.get(callback_id) orelse {
        setException(actual_ctx, exception, "N-API callback dispatcher received an unknown id");
        return null;
    };
    const cb = callback.callback orelse return null;
    const args_object = extern_fns.JSValueToObject(actual_ctx, arguments[2], exception) orelse return null;
    const length_value = getProperty(actual_ctx, args_object, "length", exception) orelse return null;
    const length_float = extern_fns.JSValueToNumber(actual_ctx, length_value, exception);
    const length: usize = if (std.math.isFinite(length_float) and length_float > 0) @intFromFloat(length_float) else 0;

    const allocator = std.heap.smp_allocator;
    const callback_args = allocator.alloc(?*JSValue, length) catch {
        setException(actual_ctx, exception, "N-API callback dispatcher failed to allocate argv");
        return null;
    };
    defer allocator.free(callback_args);
    for (0..length) |index| {
        callback_args[index] = extern_fns.JSObjectGetPropertyAtIndex(actual_ctx, args_object, @intCast(index), exception);
    }
    ...
    var frame = NativeCallbackFrame{
        .ctx = actual_ctx,
        .this_value = if (arguments[1]) |this_value| extern_fns.JSValueToObject(actual_ctx, this_value, exception) else null,
        .args = callback_args.ptr,
        .arg_count = callback_args.len,
        .data = callback.data,
    };
    return cb(env, &frame);
}

fn nativeNapiFunctionCallback1(ctx: ?*JSContextRef, function: ?*JSObject, this: ?*JSObject, argument_count: usize, arguments: [*c]const ?*JSValue, exception: extern_fns.ExceptionRef) callconv(.c) ?*JSValue {
    _ = function;
    return nativeNapiFunctionCallbackById(1, ctx, this, argument_count, arguments, exception);
}
fn nativeNapiFunctionCallback2(ctx: ?*JSContextRef, function: ?*JSObject, this: ?*JSObject, argument_count: usize, arguments: [*c]const ?*JSValue, exception: extern_fns.ExceptionRef) callconv(.c) ?*JSValue {
    _ = function;
    return nativeNapiFunctionCallbackById(2, ctx, this, argument_count, arguments, exception);
}
fn nativeNapiFunctionCallback3(ctx: ?*JSContextRef, function: ?*JSObject, this: ?*JSObject, argument_count: usize, arguments: [*c]const ?*JSValue, exception: extern_fns.ExceptionRef) callconv(.c) ?*JSValue {
    _ = function;
    return nativeNapiFunctionCallbackById(3, ctx, this, argument_count, arguments, exception);
}
fn nativeNapiFunctionCallback4(ctx: ?*JSContextRef, function: ?*JSObject, this: ?*JSObject, argument_count: usize, arguments: [*c]const ?*JSValue, exception: extern_fns.ExceptionRef) callconv(.c) ?*JSValue {
    _ = function;
    return nativeNapiFunctionCallbackById(4, ctx, this, argument_count, arguments, exception);
}
fn nativeNapiFunctionCallback5(ctx: ?*JSContextRef, function: ?*JSObject, this: ?*JSObject, argument_count: usize, arguments: [*c]const ?*JSValue, exception: extern_fns.ExceptionRef) callconv(.c) ?*JSValue {
    _ = function;
    return nativeNapiFunctionCallbackById(5, ctx, this, argument_count, arguments, exception);
}
fn nativeNapiFunctionCallback6(ctx: ?*JSContextRef, function: ?*JSObject, this: ?*JSObject, argument_count: usize, arguments: [*c]const ?*JSValue, exception: extern_fns.ExceptionRef) callconv(.c) ?*JSValue {
    _ = function;
    return nativeNapiFunctionCallbackById(6, ctx, this, argument_count, arguments, exception);
}
fn nativeNapiFunctionCallback7(ctx: ?*JSContextRef, function: ?*JSObject, this: ?*JSObject, argument_count: usize, arguments: [*c]const ?*JSValue, exception: extern_fns.ExceptionRef) callconv(.c) ?*JSValue {
    _ = function;
    return nativeNapiFunctionCallbackById(7, ctx, this, argument_count, arguments, exception);
}
fn nativeNapiFunctionCallback8(ctx: ?*JSContextRef, function: ?*JSObject, this: ?*JSObject, argument_count: usize, arguments: [*c]const ?*JSValue, exception: extern_fns.ExceptionRef) callconv(.c) ?*JSValue {
    _ = function;
    return nativeNapiFunctionCallbackById(8, ctx, this, argument_count, arguments, exception);
}

fn nativeCallbackFnForId(callback_id: usize) extern_fns.JSObjectCallAsFunctionCallback {
    return switch (callback_id) {
        1 => nativeNapiFunctionCallback1,
        2 => nativeNapiFunctionCallback2,
        3 => nativeNapiFunctionCallback3,
        4 => nativeNapiFunctionCallback4,
        5 => nativeNapiFunctionCallback5,
        6 => nativeNapiFunctionCallback6,
        7 => nativeNapiFunctionCallback7,
        8 => nativeNapiFunctionCallback8,
        else => nativeNapiFunctionCallback,
    };
}
...
pub export fn napi_create_function(
    ...
    _ = utf8name;
    _ = length;
    const callback_id = next_native_callback_id;
    next_native_callback_id += 1;
    native_callbacks.put(std.heap.smp_allocator, callback_id, .{
        .env = env,
        .callback = callback,
        .data = data,
    ...
    const source = std.fmt.allocPrint(
        ...
        "(function(){{ const id = {d}; return function __home_napi_fn(){{ return globalThis.__home_invokeNapiCallback(id, this, Array.prototype.slice.call(arguments)); }}; }})()",
        .{callback_id},
    ) catch return setNapiLastError(env, .generic_failure);
    defer std.heap.smp_allocator.free(source);
    const script = makeJSString(source) catch return setNapiLastError(env, .generic_failure);
    defer extern_fns.JSStringRelease(script);
    const url = makeJSString("home:napi-create-function") catch return setNapiLastError(env, .generic_failure);
    defer extern_fns.JSStringRelease(url);
    out.* = extern_fns.JSEvaluateScript(env.ctx, script, null, url, 1, env.exception) orelse return setNapiLastError(env, .pending_exception);
...
pub export fn napi_create_external(
    ...
    const external_id = next_native_external_id;
    next_native_external_id += 1;
    setNumberProperty(env.ctx, object, "__home_napi_external_id", external_id);
    native_externals.put(std.heap.smp_allocator, external_id, .{
        .env = env,
        .data = data,
...
pub export fn napi_get_value_external(env_: napi_env, value: napi_value, result: ?*?*anyopaque) napi_status {
    ...
    const external_id = readNativeObjectId(env.ctx, object, "__home_napi_external_id") orelse return setNapiLastError(env, .invalid_arg);
    const external = native_externals.get(external_id) orelse return setNapiLastError(env, .invalid_arg);
    std.debug.print("get external id {d} ptr 0x{x}\n", .{ external_id, @intFromPtr(external.data orelse null) });
    out.* = external.data;
    return setNapiLastError(env, .ok);
}
...
fn readNativeObjectId(ctx: *JSContextRef, object: *JSObject, name: []const u8) ?usize {
    const value = getProperty(ctx, object, name, null) orelse return null;
    if (extern_fns.JSValueIsUndefined(ctx, value) or extern_fns.JSValueIsNull(ctx, value)) return null;
