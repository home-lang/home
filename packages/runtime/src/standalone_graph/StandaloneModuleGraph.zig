...
pub const StandaloneModuleGraph = struct {
    ...
    pub fn inject(bytes: []const u8, self_exe: [:0]const u8, inject_options: InjectOptions, target: *const CompileTarget) bun.FD {
        ...
        var zname: [:0]const u8 = bun.fs.FileSystem.tmpname("bun-build", &buf, @as(u64, @bitCast(bun.milliTimestamp()))) catch |err| {
    ...
    pub fn serializeJsonSourceMapForStandalone(
        ...
        try appendU32Little(header_list, @intCast(sources_paths.items.len));
        try appendU32Little(header_list, @intCast(map_blob.len));
        ...
        for (sources_paths.items.slice()) |item| {
            ...
            try appendU32Little(header_list, slice.offset);
            try appendU32Little(header_list, slice.length);
        }
    ...
    fn appendU32Little(list: *std.array_list.Managed(u8), value: u32) !void {
