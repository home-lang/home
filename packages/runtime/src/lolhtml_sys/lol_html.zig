// Copied verbatim from bun/src/lolhtml_sys/lol_html.zig at upstream
// SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../cli/LICENSE.bun.md.
//
// Zig wrapper around the vendored cloudflare/lol-html C library. The HTML
// rewriter C ABI (`lol_html_*`) is the link-time contract with liblolhtml.a
// and must not be renamed.
//
// PORT NOTES:
// - `bun.FeatureFlags.disable_lolhtml` gates `auto_disable()`; we stub it as
//   a no-op until `FeatureFlags` lands on home_rt. Behaviour is identical to
//   upstream when the flag is `false` (the only shipped value).
// - `HTMLString.toString` and `HTMLString.toJS` reach into `bun.String` /
//   `bun.strings.isAllASCII` / `../runtime/api/lolhtml_jsc.zig`. Those land
//   alongside the JSC port; we keep the FFI surface compiling by stashing the
//   bridge under an `if (false)` (`unreachable`) until then. Callers that
//   need a JS string lift `HTMLString.slice` into `bun.String` directly.
...
/// PORT NOTE: upstream gates this on `bun.FeatureFlags.disable_lolhtml`.
/// `FeatureFlags` hasn't landed on `home_rt` yet — only the always-false
/// shipped value matters for compilation, so we hard-code `false` here. When
/// `FeatureFlags` lands, restore the original `comptime bun.FeatureFlags.disable_lolhtml`.
...
pub const HTMLString = extern struct {
    ...
    // ^ JSC-tied; re-attaches when `runtime/api/lolhtml_jsc.zig` is ported.
};
...
test "lol_html extern symbol signatures compile" {
    _ = @typeName(@TypeOf(HTMLRewriter));
    _ = @typeName(@TypeOf(HTMLRewriter.Builder));
    _ = @typeName(@TypeOf(HTMLSelector));
    _ = @typeName(@TypeOf(Element));
    _ = @typeName(@TypeOf(Comment));
    _ = @typeName(@TypeOf(TextChunk));
    _ = @typeName(@TypeOf(EndTag));
    _ = @typeName(@TypeOf(DocEnd));
    _ = @typeName(@TypeOf(HTMLString));
    _ = @typeName(@TypeOf(MemorySettings));
    _ = @typeName(@TypeOf(SourceLocationBytes));
    _ = @typeName(@TypeOf(Encoding));
    _ = @typeName(@TypeOf(HTMLString.toString));
    _ = ptrWithoutPanic;
    _ = auto_disable;
    try std.testing.expectEqualStrings("UTF-8", Encoding.label.get(.UTF8).?);
    try std.testing.expectEqualStrings("UTF-16", Encoding.label.get(.UTF16).?);
}
