// Copied verbatim from bun/src/libarchive_sys/bindings.zig at upstream
// SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../cli/LICENSE.bun.md.
//
// Zig wrapper around the vendored libarchive C library — full FFI surface for
// `bun unzip` / tarball install / archive extraction. `archive_*` extern
// symbols stay verbatim — they are the C ABI surface the linked
// `libarchive.a` exposes and must not be renamed.
//
// Phase 12 divergences vs upstream:
// - `bun.sliceTo(p, 0)` → `std.mem.sliceTo(p, 0)`.
// - `bun.FD` → local `FD = std.posix.fd_t` (or windows.HANDLE on Windows);
//   used only as a stable typedef. `FD.cast()` is replaced by passing the
//   fd straight to `archive_write_open_fd` since FD is already `c_int`.
// - `bun.Mode` → `std.posix.mode_t`.
// - `bun.OOM` → `home_rt.OOM`.
// - The sparse-file extraction path (`writeOpenFd` keeps simple cast,
//   `readDataIntoFd` and `writeZerosToFile`) is replaced with a
//   `@compileError`-armed stub — it reaches into `bun.sys.File.pwriteAll` /
//   `bun.sys.setFileOffset` / `bun.sys.ftruncate` which haven't been ported
//   yet. The full extern surface + iterator + entry getters port cleanly.
...
// Platform-typed fd. Upstream uses `bun.FD`, which is the cross-platform
// fd wrapper from `src/sys/`. The bindings only need it as a stable opaque
// typedef for `writeOpenFd` / `readDataIntoFd`; the JS-visible wrapper
// adds bookkeeping later.
pub const FD = if (@import("builtin").os.tag == .windows) std.os.windows.HANDLE else std.posix.fd_t;
...
const ARCHIVE_FORMAT_BASE_MASK = @as(c_int, 0xff0000);
const ARCHIVE_FORMAT_CPIO = @as(c_int, 0x10000);
...
const ARCHIVE_FORMAT_SHAR = @as(c_int, 0x20000);
...
const ARCHIVE_FORMAT_TAR = @as(c_int, 0x30000);
...
const ARCHIVE_FORMAT_ISO9660 = @as(c_int, 0x40000);
const ARCHIVE_FORMAT_ISO9660_ROCKRIDGE = ARCHIVE_FORMAT_ISO9660 | @as(c_int, 1);
const ARCHIVE_FORMAT_ZIP = @as(c_int, 0x50000);
const ARCHIVE_FORMAT_EMPTY = @as(c_int, 0x60000);
const ARCHIVE_FORMAT_AR = @as(c_int, 0x70000);
const ARCHIVE_FORMAT_AR_GNU = ARCHIVE_FORMAT_AR | @as(c_int, 1);
const ARCHIVE_FORMAT_AR_BSD = ARCHIVE_FORMAT_AR | @as(c_int, 2);
const ARCHIVE_FORMAT_MTREE = @as(c_int, 0x80000);
const ARCHIVE_FORMAT_RAW = @as(c_int, 0x90000);
const ARCHIVE_FORMAT_XAR = @as(c_int, 0xA0000);
const ARCHIVE_FORMAT_LHA = @as(c_int, 0xB0000);
const ARCHIVE_FORMAT_CAB = @as(c_int, 0xC0000);
const ARCHIVE_FORMAT_RAR = @as(c_int, 0xD0000);
const ARCHIVE_FORMAT_7ZIP = @as(c_int, 0xE0000);
const ARCHIVE_FORMAT_WARC = @as(c_int, 0xF0000);
const ARCHIVE_FORMAT_RAR_V5 = @as(c_int, 0x100000);
...
const ARCHIVE_EXTRACT_HFS_COMPRESSION_FORCED = @as(c_int, 0x8000);
const ARCHIVE_EXTRACT_SECURE_NOABSOLUTEPATHS = @as(c_int, 0x10000);
const ARCHIVE_EXTRACT_CLEAR_NOCHANGE_FFLAGS = @as(c_int, 0x20000);
const ARCHIVE_EXTRACT_SAFE_WRITES = @as(c_int, 0x40000);
...
pub const Archive = opaque {
    ...
    pub fn writeOpenFd(archive: *Archive, fd: FD) Result {
        // Upstream calls `fd.cast()` which strips libuv's tagged-handle bits
        // on Windows. Until `home_rt.sys.FD` lands we keep the raw fd; the
        // POSIX fd is already a `c_int`. Windows path will need to be
        // re-armed once the FD wrapper is ported (Phase 12.3).
        return archive_write_open_fd(archive, if (@import("builtin").os.tag == .windows)
            @compileError("writeOpenFd: Windows path requires home_rt.sys.FD.cast (Phase 12.3)")
        else
            fd);
    }
    ...
    pub const Entry = opaque {
        ...
        extern fn archive_entry_set_perm(*Entry, mode_t) void;
        pub fn setPerm(entry: *Entry, p: mode_t) void {
            archive_entry_set_perm(entry, p);
        }

        extern fn archive_entry_set_mode(*Entry, mode_t) void;
        pub fn setMode(entry: *Entry, mode: mode_t) void {
        ...
        extern fn archive_entry_filetype(*Entry) mode_t;
        pub fn filetype(entry: *Entry) mode_t {
            return archive_entry_filetype(entry);
        }
        extern fn archive_entry_perm(*Entry) mode_t;
        pub fn perm(entry: *Entry) mode_t {
    ...
    pub const Iterator = struct {
        archive: *Archive,
        filter: std.EnumSet(FileKind),

        fn IterResult(comptime T: type) type {
            return union(enum) {
                err: struct {
                    archive: *Archive,
                    message: []const u8,
        ...
        pub fn init(tarball_bytes: []const u8) IterResult(@This()) {
            const Return = IterResult(@This());
        ...
        const NextEntry = struct {
            ...
            pub fn readEntryData(this: *const @This(), allocator: std.mem.Allocator, archive: *Archive) OOM!IterResult([]const u8) {
                const Return = IterResult([]const u8);
        ...
        pub fn next(this: *@This()) IterResult(?NextEntry) {
            const Return = IterResult(?NextEntry);
            ...
            while (true) {
                return switch (this.archive.readNextHeader(&entry)) {
                    ...
                    .ok => {
                        const kind = kindFromMode(entry.filetype());
        ...
        pub fn deinit(this: *@This()) IterResult(void) {
            const Return = IterResult(void);
...
// Local `kindFromMode` — upstream uses `bun.sys.kindFromMode` from
// `src/sys/`. We mirror its logic from `<sys/stat.h>` S_IF* bits so the
// iterator doesn't have to wait for the full sys port. Behavior matches
// `FileKind.fromMode` upstream (which is what bun's helper wraps).
fn kindFromMode(mode: mode_t) FileKind {
...
pub const ARCHIVE_ENTRY_ACL_SYNCHRONIZE = @as(c_int, 0x00008000);
...
pub const ARCHIVE_ENTRY_ACL_ENTRY_INHERITED = @as(c_int, 0x01000000);
pub const ARCHIVE_ENTRY_ACL_ENTRY_FILE_INHERIT = @as(c_int, 0x02000000);
pub const ARCHIVE_ENTRY_ACL_ENTRY_DIRECTORY_INHERIT = @as(c_int, 0x04000000);
pub const ARCHIVE_ENTRY_ACL_ENTRY_NO_PROPAGATE_INHERIT = @as(c_int, 0x08000000);
pub const ARCHIVE_ENTRY_ACL_ENTRY_INHERIT_ONLY = @as(c_int, 0x10000000);
pub const ARCHIVE_ENTRY_ACL_ENTRY_SUCCESSFUL_ACCESS = @as(c_int, 0x20000000);
pub const ARCHIVE_ENTRY_ACL_ENTRY_FAILED_ACCESS = @as(c_int, 0x40000000);
...
test "libarchive extern symbols have well-formed type signatures" {
    // Pure FFI: verify each extern fn type is well-formed (the C ABI surface
    // exposed by the linked `libarchive.a`). Sampled across read/write,
    // entry, filter, format and match families.
    _ = @typeName(@TypeOf(archive_read_data_block));
    _ = @typeName(@TypeOf(archive_read_open_memory2));
    _ = @typeName(@TypeOf(archive_read_open_fd));
    _ = @typeName(@TypeOf(archive_read_extract));
    _ = @typeName(@TypeOf(archive_write_open));
    _ = @typeName(@TypeOf(archive_write_open2));
    _ = @typeName(@TypeOf(archive_write_disk_new));
    _ = @typeName(@TypeOf(archive_write_set_format_by_name));
    _ = @typeName(@TypeOf(archive_entry_clone));
    _ = @typeName(@TypeOf(archive_entry_acl_clear));
    _ = @typeName(@TypeOf(archive_entry_sparse_next));
    _ = @typeName(@TypeOf(archive_match_new));
    _ = @typeName(@TypeOf(archive_match_include_pattern));
    _ = @typeName(@TypeOf(archive_filter_count));
    _ = @typeName(@TypeOf(archive_format_name));
    _ = @typeName(@TypeOf(archive_clear_error));
    _ = @typeName(@TypeOf(archive_set_error));
    _ = @typeName(@TypeOf(archive_entry_linkresolver_new));

    // Surface methods stay compile-checked too.
    _ = @typeName(@TypeOf(Archive.writeNew));
    _ = @typeName(@TypeOf(Archive.readNew));
    _ = @typeName(@TypeOf(Archive.writeOpenFilename));
    _ = @typeName(@TypeOf(Archive.writeHeader));
    _ = @typeName(@TypeOf(Archive.writeData));
    _ = @typeName(@TypeOf(Archive.readSupportFormatAll));
    _ = @typeName(@TypeOf(Archive.readSupportFilterAll));
    _ = @typeName(@TypeOf(Archive.Entry.new));
    _ = @typeName(@TypeOf(Archive.Entry.setSize));
    _ = @typeName(@TypeOf(Archive.Entry.pathname));
    _ = @typeName(@TypeOf(Archive.Iterator.init));
}

test "Result tag values match libarchive.h ABI constants" {
    try std.testing.expectEqual(@as(i32, 1), @intFromEnum(Archive.Result.eof));
    try std.testing.expectEqual(@as(i32, 0), @intFromEnum(Archive.Result.ok));
    try std.testing.expectEqual(@as(i32, -10), @intFromEnum(Archive.Result.retry));
    try std.testing.expectEqual(@as(i32, -20), @intFromEnum(Archive.Result.warn));
    try std.testing.expectEqual(@as(i32, -25), @intFromEnum(Archive.Result.failed));
    try std.testing.expectEqual(@as(i32, -30), @intFromEnum(Archive.Result.fatal));
}

test "FileType + SymlinkType + Filter tag values are stable" {
    try std.testing.expectEqual(@as(mode_t, 0o100000), @intFromEnum(FileType.regular));
    try std.testing.expectEqual(@as(mode_t, 0o120000), @intFromEnum(FileType.link));
    try std.testing.expectEqual(@as(mode_t, 0o040000), @intFromEnum(FileType.directory));

    try std.testing.expectEqual(@as(c_int, 0), @intFromEnum(SymlinkType.none));
    try std.testing.expectEqual(@as(c_int, 1), @intFromEnum(SymlinkType.file));
    try std.testing.expectEqual(@as(c_int, 2), @intFromEnum(SymlinkType.directory));

    try std.testing.expectEqual(@as(c_int, 0), @intFromEnum(Flags.Filter.none));
    try std.testing.expectEqual(@as(c_int, 1), @intFromEnum(Flags.Filter.gzip));
    try std.testing.expectEqual(@as(c_int, 14), @intFromEnum(Flags.Filter.zstd));
}

test "Format ZIP tag matches libarchive.h" {
    try std.testing.expectEqual(@as(c_int, 0x50000), @intFromEnum(Flags.Format.zip));
    try std.testing.expectEqual(@as(c_int, 0x30000), @intFromEnum(Flags.Format.tar));
    try std.testing.expectEqual(@as(c_int, 0x10000), @intFromEnum(Flags.Format.cpio));
}

test "Extract flag bits match libarchive.h" {
    try std.testing.expectEqual(@as(c_int, 0x0001), @intFromEnum(Flags.Extract.owner));
    try std.testing.expectEqual(@as(c_int, 0x0002), @intFromEnum(Flags.Extract.perm));
    try std.testing.expectEqual(@as(c_int, 0x1000), @intFromEnum(Flags.Extract.sparse));
}

test "GrowingBuffer round-trips bytes through writeCallback" {
    const allocator = std.testing.allocator;
    var buf = GrowingBuffer.init(allocator);
    defer buf.deinit();

    // We can't call C `archive_write_*` from this unit test, but we can
    // exercise the callback's payload path directly — it's pure Zig.
    const payload = "hello, libarchive";
    const fake_archive: *struct_archive = @ptrFromInt(0xdead0000);
    const written = GrowingBuffer.writeCallback(fake_archive, &buf, payload.ptr, payload.len);
    try std.testing.expectEqual(@as(la_ssize_t, @intCast(payload.len)), written);
...
test "kindFromMode maps standard S_IF* bits" {
    try std.testing.expectEqual(FileKind.directory, kindFromMode(std.posix.S.IFDIR | 0o755));
    try std.testing.expectEqual(FileKind.file, kindFromMode(std.posix.S.IFREG | 0o644));
    try std.testing.expectEqual(FileKind.sym_link, kindFromMode(std.posix.S.IFLNK | 0o777));
}
