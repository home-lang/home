// Copied verbatim from bun/src/safety/CriticalSection.zig at upstream
// SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../cli/LICENSE.bun.md.
//
// Debug-mode race-condition detector. The runtime body (when enabled) is the
// upstream copy; only the comptime gates are rewired to the symbols Home
// currently exports: `enabled` follows `Environment.allow_assert` (Bun spells
// this `ci_assert`), and `traces_enabled` is forced false so we don't depend
// on `crash_handler.StoredTrace` / `dumpStackTrace`, neither of which has been
// ported yet.
...
const State = struct {
    ...
    fn showTrace(self: *State) void {
        _ = self;
        // stubbed: re-attaches when home_rt.crash_handler.dumpStackTrace + StoredTrace land.
...
/// this `false` keeps every reference to `StoredTrace.capture` / `StoredTrace.trace`
...
test "CriticalSection: nested exclusive begin/end is safe on a single thread" {
    var cs: Self = .{};
    cs.begin();
    cs.begin();
    cs.end();
...
test "CriticalSection: nested shared beginReadOnly/end is safe on a single thread" {
    var cs: Self = .{};
    cs.beginReadOnly();
    cs.beginReadOnly();
    cs.end();
