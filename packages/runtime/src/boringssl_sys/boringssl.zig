// Copied verbatim from bun/src/boringssl_sys/boringssl.zig at upstream
...
pub const SSL = opaque {
    ...
    // PORT NOTE: upstream pulled this struct from `bun.uws.us_bun_verify_error_t`
    // (`src/uws/uws.zig`). uws.zig hasn't been ported in full yet (the JSC-tied
    // `verifyErrorToJS` decl blocks it). Inline the wire-format struct here so
    // the boringssl FFI surface compiles standalone; `uws_sys` re-exports the
    ...
    extern fn us_ssl_socket_verify_error_from_ssl(ssl: *SSL) us_bun_verify_error_t;
    pub fn getVerifyError(this: *SSL) us_bun_verify_error_t {
...
test "boringssl extern symbol signatures compile" {
    // Pure FFI surface — only assert that the canonical symbols resolve.
    _ = @typeName(@TypeOf(SSL_new));
    _ = @typeName(@TypeOf(SSL_free));
    _ = @typeName(@TypeOf(SSL_CTX_new));
    _ = @typeName(@TypeOf(SSL_CTX_free));
    _ = @typeName(@TypeOf(SSL_read));
    _ = @typeName(@TypeOf(SSL_write));
    _ = @typeName(@TypeOf(SSL_connect));
    _ = @typeName(@TypeOf(SSL_get_error));
    _ = @typeName(@TypeOf(BIO_new));
    _ = @typeName(@TypeOf(BIO_free));
    _ = @typeName(@TypeOf(X509_new));
    _ = @typeName(@TypeOf(X509_free));
    _ = @typeName(@TypeOf(EVP_MD_CTX_new));
    _ = @typeName(@TypeOf(EVP_DigestInit_ex));
    _ = @typeName(@TypeOf(EVP_DigestUpdate));
    _ = @typeName(@TypeOf(EVP_DigestFinal_ex));
    _ = @typeName(@TypeOf(RSA_new));
    _ = @typeName(@TypeOf(EC_KEY_new));
    _ = @typeName(@TypeOf(ERR_get_error));
    _ = @typeName(@TypeOf(SSL.us_bun_verify_error_t));
}
