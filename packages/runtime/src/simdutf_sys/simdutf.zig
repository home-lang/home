// Copied verbatim from bun/src/simdutf_sys/simdutf.zig at upstream
// SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../cli/LICENSE.bun.md.
//
// Zig wrapper around the vendored simdutf (SIMD UTF validation / conversion)
// C++ library. `simdutf__*` extern symbols stay verbatim — they are the C ABI
// surface the linked `libsimdutf.a` exposes and must not be renamed.
...
// Local stand-in for upstream `bun.jsc.markBinding`. In Bun this hooks into
// the JSC binding-trace machinery; we have no JSC here, so it's a no-op.
const jsc = struct {
...
test "simdutf extern symbol signatures compile" {
    _ = @typeName(@TypeOf(simdutf__validate_utf8));
    _ = @typeName(@TypeOf(simdutf__convert_utf8_to_utf16le));
    _ = @typeName(@TypeOf(simdutf__count_utf8));
    _ = @typeName(@TypeOf(base64.simdutf__base64_encode));

    // SIMDUTFResult layout/status matches the C ABI struct.
    try std.testing.expectEqual(@as(i32, 0), @intFromEnum(SIMDUTFResult.Status.success));
    try std.testing.expectEqual(@as(i32, 9), @intFromEnum(SIMDUTFResult.Status.output_buffer_too_small));

    // trim.utf8_len handles short inputs deterministically (no FFI required).
    try std.testing.expectEqual(@as(usize, 0), trim.utf8_len(""));
    // ASCII "abc": no leading multi-byte sequence at boundary.
    try std.testing.expectEqual(@as(usize, 3), trim.utf8_len("abc"));
    // trim.utf16_len returns 0 for empty input.
    const empty16: []const u16 = &.{};
    try std.testing.expectEqual(@as(usize, 0), trim.utf16_len(empty16));
}
