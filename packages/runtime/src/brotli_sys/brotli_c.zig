// Copied verbatim from bun/src/brotli_sys/brotli_c.zig at upstream
// SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../cli/LICENSE.bun.md.
//
// Zig wrapper around the vendored google/brotli C library.
// `Brotli*` extern symbols stay verbatim — they are the C ABI surface the
// linked `libbrotli{enc,dec,common}.a` exposes and must not be renamed.
...
pub const BrotliDecoder = opaque {
    ...
    pub fn errorString(c: BrotliDecoderErrorCode) callconv(.c) [:0]const u8 {
        // upstream uses `bun.sliceTo` which is just `std.mem.sliceTo`.
...
test "brotli extern symbol signatures compile" {
    _ = @typeName(@TypeOf(BrotliDecoderCreateInstance));
    _ = @typeName(@TypeOf(BrotliDecoderDecompress));
    _ = @typeName(@TypeOf(BrotliEncoderCreateInstance));
    _ = @typeName(@TypeOf(BrotliEncoderCompress));
    try std.testing.expectEqual(@as(c_uint, 1), @intFromEnum(BrotliDecoderResult.success));
    try std.testing.expectEqual(@as(c_uint, 3), @intFromEnum(BrotliEncoder.Operation.emit_metadata));
    try std.testing.expectEqual(@as(c_int, -31), @intFromEnum(BrotliDecoderErrorCode.UNREACHABLE));
    try std.testing.expectEqual(@as(c_int, 0), BROTLI_MODE_GENERIC);
    try std.testing.expectEqual(@as(c_int, 11), BROTLI_DEFAULT_QUALITY);
}
