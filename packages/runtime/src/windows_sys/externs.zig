// Copied verbatim from bun/src/windows_sys/externs.zig at upstream
...
test "windows_sys.externs type aliases resolve at comptime" {
    // extern fn bodies aren't callable here on non-Windows; just confirm the
    // type aliases tie back to std.os.windows so the file compiles on every
    // host and the symbol table stays available to wave-13 callers.
    try std.testing.expectEqual(*DWORD, LPDWORD);
    try std.testing.expectEqual(*anyopaque, HPCON);
}
