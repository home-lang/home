...
pub const DisplayPair = struct {
    ...
    pub fn parse(input: *css.Parser) css.Result(DisplayPair) {
        ...
        const display_ident_map = bun.ComptimeStringMap(DisplayPair, .{
        ...
        if (display_ident_map.getASCIIICaseInsensitive(ident)) |pair| {
            return .{ .result = pair };
        }
    ...
    pub fn toCss(this: *const @This(), dest: *Printer) PrintErr!void {
        if (this.outside == .@"inline" and this.inside == .flow_root and !this.is_list_item) {
            ...
            if (!this.inside.eql(&DisplayInside{ .flow = {} })) {
                if (needs_space) try dest.writeChar(' ');
...
pub const DisplayInside = union(enum) {
    ...
    pub fn parse(input: *css.Parser) css.Result(DisplayInside) {
        const display_inside_map = bun.ComptimeStringMap(DisplayInside, .{
        ...
        if (display_inside_map.getASCIIICaseInsensitive(ident)) |value| {
            return .{ .result = value };
        }
