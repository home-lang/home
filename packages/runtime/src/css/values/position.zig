...
test "Position holds two PositionComponent variants" {
    const p = Position{
        .x = HorizontalPosition.center,
        .y = VerticalPosition.center,
    };
    try std.testing.expect(p.x == .center);
    try std.testing.expect(p.y == .center);
}

test "HorizontalPositionKeyword.eql compares enum tags" {
    const left = HorizontalPositionKeyword.left;
    const right = HorizontalPositionKeyword.right;
    try std.testing.expect(left.eql(&left));
    try std.testing.expect(!left.eql(&right));
}

test "Position.center() builds a center/center pair" {
