    ...
    pub fn getReference(this: *CssModule, allocator: Allocator, name: []const u8, source_index: u32) void {
        ...
        if (gop.found_existing) {
            ...
            gop.value_ptr.* = CssModuleExport{
                .name = this.config.pattern.writeToString(allocator, ArrayList(u8).empty, this.hashes.items[source_index], this.sources.items[source_index], name),
                .composes = .empty,
                .is_referenced = true,
            };
        }
...
pub fn hash(allocator: Allocator, comptime fmt: []const u8, args: anytype, at_start: bool) []const u8 {
    ...
    var stack_fallback = bun.stackFallback(128, allocator);
