// Copied verbatim from bun/src/cares_sys/c_ares.zig at upstream
// SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../cli/LICENSE.bun.md.
//
// Zig wrapper around the vendored c-ares C library — pure FFI surface plus a
// thin `Channel` helper used by node:dns / Bun.dns. `ares_*` extern symbols
// stay verbatim — they are the C ABI surface the linked `libcares.a` exposes
// and must not be renamed.
//
// Phase 12 divergences vs upstream:
// - `../runtime/dns_jsc/cares_jsc.zig` is JSC-tied and not ported yet, so the
//   `toJSResponse` / `toJS` / `toJSArray` / `toJSWithSyscall` / `Deferred` /
//   `toDeferred` / `errorToDeferred` / `errorToJSWithSyscallAndHostname` hooks
//   are replaced by `cares_jsc_stub` opaque/undefined sentinels. Re-attaches
//   when the JSC bridge lands (Phase 12.2).
// - `bun.cast(T, x)` → `@as(T, @ptrCast(@alignCast(x)))` per use site.
// - `bun.handleOom` → `home_rt.handleOom`.
// - `bun.default_allocator` → `home_rt.default_allocator`.
// - `bun.span` → `std.mem.span`.
// - `bun.mimalloc.mi_malloc/free/realloc` → `home_rt.mimalloc_sys.mimalloc.*`.
// - `bun.strings.eqlComptime` → `home_rt.strings.eqlComptime`.
// - `bun.enumMap`, `bun.zero`, `bun.todo` → local inline helpers.
// - `jsc.Node.Maybe(T, Error)` → local `MaybeT(T, Error)` union.
// - `bun.windows.libuv` is not ported, so the `initEAI` Windows branch is
...
pub const ares_socket_t = if (home_rt.Environment.isWindows) std.os.windows.ws2_32.SOCKET else c_int;
...
fn MaybeT(comptime T: type, comptime E: type) type {
    return union(enum) {
        result: T,
        err: E,
    };
...
// Local replacements for the helpers that live on `bun.zig` in upstream.
inline fn castPtr(comptime To: type, value: anytype) To {
...
pub const struct_hostent = extern struct {
    ...
    const hostent_int = if (home_rt.Environment.isWindows) c_short else c_int;

    pub const toJSResponse = cares_jsc.hostentToJSResponse;
...
pub const hostent_with_ttls = struct {
    ...
    pub const toJSResponse = cares_jsc.hostentWithTtlsToJSResponse;
    ...
    pub fn parse(comptime lookup_name: []const u8, buffer: [*c]u8, buffer_length: c_int) MaybeT(*hostent_with_ttls, Error) {
        ...
        if (comptime strings.eqlComptime(lookup_name, "a")) {
            ...
            var with_ttls = home_rt.handleOom(home_rt.default_allocator.create(hostent_with_ttls));
...
pub const struct_nameinfo = extern struct {
    ...
    pub const toJSResponse = cares_jsc.nameinfoToJSResponse;
...
fn libraryInit() void {
    ...
    const rc = ares_library_init_mem(
        ARES_LIB_INIT_ALL,
        home_rt.mimalloc_sys.mimalloc.mi_malloc,
        home_rt.mimalloc_sys.mimalloc.mi_free,
        home_rt.mimalloc_sys.mimalloc.mi_realloc,
    );
    if (rc != ARES_SUCCESS) {
...
pub const struct_ares_caa_reply = extern struct {
    ...
    pub const toJSResponse = cares_jsc.caaReplyToJSResponse;

    pub const toJS = cares_jsc.caaReplyToJS;
...
pub const struct_ares_srv_reply = extern struct {
    ...
    pub const toJSResponse = cares_jsc.srvReplyToJSResponse;
    pub const toJS = cares_jsc.srvReplyToJS;
...
pub const struct_ares_mx_reply = extern struct {
    ...
    pub const toJSResponse = cares_jsc.mxReplyToJSResponse;
    pub const toJS = cares_jsc.mxReplyToJS;
...
pub const struct_ares_txt_reply = extern struct {
    ...
    pub const toJSResponse = cares_jsc.txtReplyToJSResponse;
    pub const toJS = cares_jsc.txtReplyToJS;
    pub const toJSForAny = cares_jsc.txtReplyToJSForAny;
...
pub const struct_ares_naptr_reply = extern struct {
    ...
    pub const toJSResponse = cares_jsc.naptrReplyToJSResponse;
    pub const toJS = cares_jsc.naptrReplyToJS;
...
pub const struct_ares_soa_reply = extern struct {
    ...
    pub const toJSResponse = cares_jsc.soaReplyToJSResponse;
    pub const toJS = cares_jsc.soaReplyToJS;
...
pub const struct_any_reply = struct {
    ...
    pub const toJSResponse = cares_jsc.anyReplyToJSResponse;
    pub const toJS = cares_jsc.anyReplyToJS;
    ...
    pub fn callbackWrapper(
        ...
        return &struct {
            pub fn handleAny(ctx: ?*anyopaque, status: c_int, timeouts: c_int, buffer: [*c]u8, buffer_length: c_int) callconv(.c) void {
                ...
                var reply = home_rt.handleOom(home_rt.default_allocator.create(struct_any_reply));
                reply.* = .{};
...
pub const Error = enum(i32) {
    ...
    pub const Deferred = cares_jsc.ErrorDeferred;
    pub const toDeferred = cares_jsc.errorToDeferred;
    pub const toJSWithSyscall = cares_jsc.errorToJSWithSyscall;
    pub const toJSWithSyscallAndHostname = cares_jsc.errorToJSWithSyscallAndHostname;

    pub fn initEAI(rc: i32) ?Error {
        if (comptime home_rt.Environment.isWindows) {
            // libuv_sys is not ported in Phase 12 yet — re-attaches once
            // `home_rt.libuv_sys` lands. Until then, treat any non-zero
            // Windows EAI as a generic not-found so the runtime can still
            // build for cross-platform smoke checks.
            return if (rc == 0) null else Error.ENOTFOUND;
        }
        ...
        if (comptime home_rt.Environment.isLinux) {
            if (eai == .SOCKTYPE) return Error.ECONNREFUSED;
        }
        if (comptime is_glibc) {
            // glibc-only async getaddrinfo_a / IDN extensions; absent on
            // musl and bionic. Lifted from upstream's `bun.Environment.isGlibc`
            // gate; on non-glibc targets these tags don't exist so the block
            // is skipped at comptime.
            switch (eai) {
        ...
        return switch (eai) {
            ...
            else => Error.ENOTIMP, // upstream uses bun.todo here; mapped to ENOTIMP.
        };
    }

    pub const code = EnumMap(Error, .{
        .{ .ENODATA, "DNS_ENODATA" },
...
pub const ARES_SOCKET_BAD = if (home_rt.Environment.isWindows) std.os.windows.ws2_32.INVALID_SOCKET else -@as(c_int, 1);
...
// Bun__canonicalizeIP_ host fn: see src/runtime/dns_jsc/cares_jsc.zig (re-attaches in Phase 12.2).
...
// `bun.Environment.isGlibc` is gated on actually linking glibc — we inline
// the same condition so the `IDN_ENCODE` / `ALLDONE` / etc. EAI tags only
// surface on glibc targets where they exist.
const is_glibc = builtin.target.abi.isGnu() and builtin.os.tag == .linux;
...
test "c-ares extern symbols have well-formed type signatures" {
    // Pure FFI: verify each extern fn type is well-formed (the C ABI surface
    // exposed by the linked `libcares.a`).
    _ = @typeName(@TypeOf(ares_library_init));
    _ = @typeName(@TypeOf(ares_library_init_mem));
    _ = @typeName(@TypeOf(ares_library_cleanup));
    _ = @typeName(@TypeOf(ares_init));
    _ = @typeName(@TypeOf(ares_init_options));
    _ = @typeName(@TypeOf(ares_destroy));
    _ = @typeName(@TypeOf(ares_getaddrinfo));
    _ = @typeName(@TypeOf(ares_freeaddrinfo));
    _ = @typeName(@TypeOf(ares_query));
    _ = @typeName(@TypeOf(ares_gethostbyname));
    _ = @typeName(@TypeOf(ares_gethostbyaddr));
    _ = @typeName(@TypeOf(ares_getnameinfo));
    _ = @typeName(@TypeOf(ares_process_fd));
    _ = @typeName(@TypeOf(ares_inet_pton));
    _ = @typeName(@TypeOf(ares_inet_ntop));
    _ = @typeName(@TypeOf(ares_set_servers_csv));
    _ = @typeName(@TypeOf(ares_parse_a_reply));
    _ = @typeName(@TypeOf(ares_parse_aaaa_reply));
    _ = @typeName(@TypeOf(ares_parse_caa_reply));
    _ = @typeName(@TypeOf(ares_parse_ptr_reply));
    _ = @typeName(@TypeOf(ares_parse_ns_reply));
    _ = @typeName(@TypeOf(ares_parse_srv_reply));
    _ = @typeName(@TypeOf(ares_parse_mx_reply));
    _ = @typeName(@TypeOf(ares_parse_txt_reply));
    _ = @typeName(@TypeOf(ares_parse_naptr_reply));
    _ = @typeName(@TypeOf(ares_parse_soa_reply));
    _ = @typeName(@TypeOf(ares_free_data));
    _ = @typeName(@TypeOf(ares_free_hostent));
}

test "Error enum maps to upstream ares.h tag values" {
    try std.testing.expectEqual(@as(i32, ARES_ENODATA), @intFromEnum(Error.ENODATA));
    try std.testing.expectEqual(@as(i32, ARES_ENOSERVER), @intFromEnum(Error.ENOSERVER));
    try std.testing.expectEqual(@as(i32, ARES_ENOTFOUND), @intFromEnum(Error.ENOTFOUND));
    // get(0) -> null; nonzero -> matching enum
    try std.testing.expect(Error.get(0) == null);
    try std.testing.expectEqual(Error.ENOTFOUND, Error.get(ARES_ENOTFOUND).?);
    // ENODATA/ENONAME canonicalize to ENOTFOUND (matches Node.js)
    try std.testing.expectEqual(Error.ENOTFOUND, Error.get(ARES_ENODATA).?);
    try std.testing.expectEqual(Error.ENOTFOUND, Error.get(ARES_ENONAME).?);
}

test "Error.code labels are stable" {
    try std.testing.expectEqualStrings("DNS_ENOTFOUND", Error.code(.ENOTFOUND));
    try std.testing.expectEqualStrings("DNS_ENOMEM", Error.code(.ENOMEM));
    try std.testing.expectEqualStrings("DNS_ECANCELLED", Error.code(.ECANCELLED));
}

test "AddrInfo_hints isEmpty" {
    const empty: AddrInfo_hints = .{};
    try std.testing.expect(empty.isEmpty());
    const non_empty: AddrInfo_hints = .{ .ai_family = 2 };
    try std.testing.expect(!non_empty.isEmpty());
}

test "zeroInit yields all-zero option struct" {
    const opts = zeroInit(Options);
    try std.testing.expectEqual(@as(c_int, 0), opts.flags);
    try std.testing.expectEqual(@as(c_int, 0), opts.timeout);
    try std.testing.expectEqual(@as(c_int, 0), opts.tries);
}

test "getSockaddr signature compiles" {
    // We can't actually call `getSockaddr` here without linking libcares — it
    // reaches `ares_inet_pton`. Just verify the type-level surface.
    _ = @typeName(@TypeOf(getSockaddr));
}
