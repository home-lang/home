//! Home Runtime — ported from Bun `src/collections/multi_array_list.zig`
...
//!     no-op methods) — matching upstream's release-build behavior.
//!     When Home grows `MimallocArena`/`NullableAllocator`/etc., flip
//!     `safety/alloc.zig`'s `enabled` flag to re-engage the check.
//!
...
pub fn MultiArrayList(comptime T: type) type {
    return struct {
        ...
        const Entry = entry: {
            // Zig 0.17: `@Type(.{...})` was replaced by per-kind `@Struct`
...
// can't reach across module boundaries to `safety/alloc.zig` until
// `home_rt.safety.CheckedAllocator` lands as a top-level export (gated
// behind hard rule #4 — the aggregator stays untouched in this pass).
// Disabled mode is layout/semantically equivalent to the dedicated
// file in `safety/alloc.zig`, so vending the same zero-sized struct
// inline lets this leaf stand alone until the aggregator wires the
// real type through.
// ----------------------------------------------------------------------
const checked_allocator_enabled = false;
const CheckedAllocator = struct {
    stored: if (checked_allocator_enabled) ?Allocator else void = if (checked_allocator_enabled) null else {},

    pub inline fn init(alloc: Allocator) @This() {
        var self: @This() = .{};
        self.set(alloc);
        return self;
    }

    pub fn set(self: *@This(), alloc: Allocator) void {
        _ = self;
        _ = alloc;
        if (comptime !checked_allocator_enabled) return;
    }

    pub fn assertEq(self: @This(), alloc: Allocator) void {
        _ = self;
        _ = alloc;
        if (comptime !checked_allocator_enabled) return;
    }

    pub inline fn transferOwnership(self: *@This(), new_allocator: anytype) void {
        _ = self;
        _ = new_allocator;
        if (comptime !checked_allocator_enabled) return;
    }
...
test "home_rt.multi_array_list: append + items round-trip" {
    const Point = struct { x: u32, y: u16 };
    var list: MultiArrayList(Point) = .empty;
    defer list.deinit(std.testing.allocator);
    try list.append(std.testing.allocator, .{ .x = 7, .y = 3 });
    try list.append(std.testing.allocator, .{ .x = 100, .y = 200 });
    try std.testing.expectEqual(@as(usize, 2), list.len);
    const xs = list.items(.x);
    try std.testing.expectEqual(@as(u32, 7), xs[0]);
    try std.testing.expectEqual(@as(u32, 100), xs[1]);
    const ys = list.items(.y);
    try std.testing.expectEqual(@as(u16, 3), ys[0]);
    try std.testing.expectEqual(@as(u16, 200), ys[1]);
}

test "home_rt.multi_array_list: zero leaves valid empty state" {
    const Pair = struct { a: u8, b: u8 };
    var list: MultiArrayList(Pair) = .empty;
    defer list.deinit(std.testing.allocator);
    try list.setCapacity(std.testing.allocator, 4);
    list.zero();
    try std.testing.expectEqual(@as(usize, 0), list.len);
    try std.testing.expect(list.memoryCost() >= 4 * (@sizeOf(u8) + @sizeOf(u8)));
}
