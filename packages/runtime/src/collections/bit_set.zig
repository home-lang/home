//! Home Runtime — ported from Bun `src/collections/bit_set.zig`
...
//!     exports.
//!   * `callconv(bun.callconv_inline)` rewritten to the literal
//!     `callconv(.@"inline")` (valid Zig 0.17 spelling; matches the
//!     release-build branch of Bun's `callconv_inline` constant).
//!
...
pub fn IntegerBitSet(comptime size: u16) type {
    return packed struct {
        ...
        pub fn isSet(self: Self, index: usize) bool {
            if (comptime Environment.allow_assert) home_rt.assert(index < bit_length);
        ...
        pub fn setRangeValue(self: *Self, range: Range, value: bool) void {
            if (comptime Environment.allow_assert) {
                home_rt.assert(range.end <= bit_length);
                home_rt.assert(range.start <= range.end);
            }
...
pub fn ArrayBitSet(comptime MaskIntType: type, comptime size: usize) type {
    ...
    return extern struct {
        ...
        /// Creates a bit set with no elements present.
        pub fn initEmpty() Self {
            // Zig 0.17 deprecates the `T{x} ** N` repeat form on arrays of
            // generic element type; `@splat` is the modern replacement.
            const masks_init: [num_masks]MaskInt = @splat(0);
            return .{ .masks = masks_init };
        }
        ...
        pub fn initFull() Self {
            if (num_masks == 0) {
                ...
                // Zig 0.17: `T{x} ** N` lifted to `@splat` + array concat.
                const prefix: [num_masks - 1]MaskInt = @splat(~@as(MaskInt, 0));
                return .{ .masks = prefix ++ [_]MaskInt{last_item_mask} };
            }
        }
        ...
        fn maskBit(index: usize) callconv(.@"inline") MaskInt {
            return @as(MaskInt, 1) << @as(ShiftInt, @truncate(index));
        }
        fn maskIndex(index: usize) callconv(.@"inline") usize {
            return index >> @bitSizeOf(ShiftInt);
        }
        fn boolMaskBit(index: usize, value: bool) callconv(.@"inline") MaskInt {
            return @as(MaskInt, @intFromBool(value)) << @as(ShiftInt, @intCast(index));
        }
    };
}
...
pub const DynamicBitSetUnmanaged = struct {
    ...
    pub fn resize(self: *@This(), allocator: Allocator, new_len: usize, fill: bool) !void {
        ...
        if (new_masks == 0) {
            if (comptime Environment.allow_assert) home_rt.assert(new_len == 0);
    ...
    pub fn clone(self: *const Self, new_allocator: Allocator) !Self {
        ...
        bunCompatCopy(MaskInt, copy.masks[0..num_masks], self.masks[0..num_masks]);
        return copy;
    }
    ...
    /// is present in the set, false otherwise.
    pub fn isSet(self: Self, index: usize) bool {
        if (comptime Environment.allow_assert) home_rt.assert(index < self.bit_length);
        return (self.masks[maskIndex(index)] & maskBit(index)) != 0;
    }
    ...
    /// match the passed boolean.
    pub fn setRangeValue(self: *Self, range: Range, value: bool) void {
        if (comptime Environment.allow_assert) home_rt.assert(range.end <= self.bit_length);
        if (comptime Environment.allow_assert) home_rt.assert(range.start <= range.end);
        if (range.start == range.end) return;
    ...
    /// same bit_length.
    pub fn toggleSet(self: *Self, toggles: Self) void {
        if (comptime Environment.allow_assert) home_rt.assert(toggles.bit_length == self.bit_length);
    ...
    pub fn setUnion(self: *Self, other: Self) void {
        if (comptime Environment.allow_assert) home_rt.assert(other.bit_length == self.bit_length);
        const num_masks = numMasks(self.bit_length);
        for (self.masks[0..num_masks], other.masks) |*mask, other_mask| {
            mask.* |= other_mask;
        }
...
pub const AutoBitSet = union(enum) {
    // Zig 0.17: `@bitSizeOf` is unavailable for auto-layout structs; use the
    // byte size (this struct is exactly two `usize`s, so `@sizeOf * 8` matches).
    pub const Static = ArrayBitSet(usize, (@sizeOf(DynamicBitSetUnmanaged) * 8 - 1));

    static: Static,
    dynamic: DynamicBitSetUnmanaged,

    pub fn needsDynamic(bit_length: usize) callconv(.@"inline") bool {
    ...
    pub fn eql(this: *const AutoBitSet, b: *const AutoBitSet) bool {
        return bunCompatEqlLong(this.rawBytes(), b.rawBytes(), true);
    }

    pub fn hash(this: *const AutoBitSet) u64 {
        return bunCompatHash(this.rawBytes());
    }
...
// ----------------------------------------------------------------------
// Bun-compat shims. Upstream bit_set.zig pulls `bun.copy`, `bun.strings.eqlLong`,
// and `bun.hash` from the broader Bun namespace. We replicate them inline so
// this leaf has no extra `home_rt` surface dependencies beyond `Environment`
// + `assert` (both already exposed).
// ----------------------------------------------------------------------

inline fn bunCompatCopy(comptime T: type, dest: []T, src: []const T) void {
    // Mirrors `bun.copy`: a checked, non-overlapping byte copy. AutoBitSet's
    // `clone` only uses it on freshly-allocated buffers, so `@memcpy` is
    // semantically equivalent to upstream's `memmove`-backed fast path.
    std.debug.assert(dest.len >= src.len);
    @memcpy(dest[0..src.len], src);
}

inline fn bunCompatEqlLong(a: []const u8, b: []const u8, comptime check_len: bool) bool {
...
test "home_rt.bit_set: IntegerBitSet basics" {
    var s = IntegerBitSet(8).initEmpty();
    try testing.expectEqual(@as(usize, 0), s.count());
    s.set(0);
    s.set(7);
    try testing.expect(s.isSet(0));
    try testing.expect(!s.isSet(1));
    try testing.expect(s.isSet(7));
    try testing.expectEqual(@as(usize, 2), s.count());
    s.unset(0);
    try testing.expect(!s.isSet(0));
}

test "home_rt.bit_set: AutoBitSet picks static for small sizes" {
    var s = try AutoBitSet.initEmpty(std.testing.allocator, 16);
    defer s.deinit(std.testing.allocator);
    try testing.expectEqual(@as(usize, 0), s.count());
    s.set(3);
    s.set(15);
    try testing.expect(s.isSet(3));
    try testing.expect(s.isSet(15));
    try testing.expectEqual(@as(usize, 2), s.count());
}

test "home_rt.bit_set: DynamicBitSet round-trips set/unset" {
    var s = try DynamicBitSet.initEmpty(std.testing.allocator, 200);
    defer s.deinit();
    try testing.expect(!s.isSet(100));
    s.set(100);
    try testing.expect(s.isSet(100));
    s.unset(100);
    try testing.expect(!s.isSet(100));
