...
pub const Tag = enum(u32) {
    ...
    //
    // Copied VERBATIM from the codegen of the Bun build Home links against:
    // `~/Code/bun/build/release/codegen/ResolvedSourceTag.zig` at the pinned Bun
    // commit fd0b6f1a271fca0b8124b69f230b100f4d636af6 (1.3.14). These IDs are
    // ABI with the prebuilt C++ objects and MUST NOT be regenerated from a newer
    // Bun HEAD — doing so shifts every id (HEAD has ~21 extra internal modules),
    // so `require("node:path")` etc. select the wrong module and the internal
    // registry recurses (createInternalModuleById → createInternalModuleById).
    @"bun:ffi" = 512,
    @"bun:sql" = 513,
    @"bun:sqlite" = 514,
...
test "Tag builtin ids stay in lock-step with linked Bun codegen" {
    // These MUST match `~/Code/bun/build/release/codegen/ResolvedSourceTag.zig`
    // at the pinned Bun commit (fd0b6f1a27 / 1.3.14) — i.e. the codegen of the
    // C++ objects Home actually links. Regenerating from a newer Bun HEAD (which
    // has ~21 extra internal modules) shifts every id and breaks node builtins.
    try std.testing.expectEqual(@as(u32, 520), @intFromEnum(Tag.@"internal:buffer"));
    try std.testing.expectEqual(@as(u32, 535), @intFromEnum(Tag.@"internal:fs/streams"));
    try std.testing.expectEqual(@as(u32, 610), @intFromEnum(Tag.@"node:fs"));
    try std.testing.expectEqual(@as(u32, 619), @intFromEnum(Tag.@"node:path"));
    try std.testing.expectEqual(@as(u32, 631), @intFromEnum(Tag.@"node:test"));
    try std.testing.expectEqual(@as(u32, 650), @intFromEnum(Tag.@"bun:test"));
    try std.testing.expectEqual(@as(u32, 661), @intFromEnum(Tag.@"bun"));
}
