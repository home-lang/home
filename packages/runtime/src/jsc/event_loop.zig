...
pub fn autoTick(this: *EventLoop) void {
    ...
    if (loop.isActive()) {
        ...
        if (comptime Environment.isDebug) {
            log("tick {f}, timeout: {f}", .{
                bun.fmt.fmtDurationOneDecimal(event_loop_sleep_timer.read()),
                bun.fmt.fmtDurationOneDecimal(timespec.ns()),
            });
        }
    } else {
...
pub fn getActiveTasks(globalObject: *jsc.JSGlobalObject, call_frame: *jsc.CallFrame) bun.JSError!jsc.JSValue {
    ...
    const result = jsc.JSValue.createEmptyObject(globalObject, 21);
    ...
    result.put(globalObject, jsc.ZigString.static("nativePollableWorkPoolJobs"), jsc.JSValue.jsNumber(vm.native_pollable_work_pool_jobs.count()));
    result.put(globalObject, jsc.ZigString.static("nativeWorkPoolThreads"), jsc.JSValue.jsNumber(bun.getThreadCount()));
    ...
    result.put(globalObject, jsc.ZigString.static("cancelledArchiveTasks"), jsc.JSValue.jsNumber(@as(f64, @floatFromInt(jsc.API.Archive.shutdownCancellationCount()))));
    ...
    result.put(globalObject, jsc.ZigString.static("cancelledNapiAsyncWork"), jsc.JSValue.jsNumber(@as(f64, @floatFromInt(bun.api.napi.asyncWorkShutdownCancellationCount()))));
    result.put(globalObject, jsc.ZigString.static("cancelledShellTasks"), jsc.JSValue.jsNumber(@as(f64, @floatFromInt(bun.shell.interpret.shutdownCancellationCount()))));
