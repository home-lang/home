...
// Zig 0.17 removed the managed `std.StringArrayHashMap`; this thin managed
// wrapper over the unmanaged variant keeps the rest of this ported parser
// (which calls .put/.getPtr/.iterator/... without an allocator) unchanged.
...
pub const Table = struct {
    allocator: std.mem.Allocator,
    entries: StringArrayHashMap(Value),

    pub fn init(allocator: std.mem.Allocator) Table {
        return .{
            .allocator = allocator,
            .entries = StringArrayHashMap(Value).init(allocator),
        };
    }
...
pub const Toml = struct {
    ...
    fn serializeTable(list: *std.array_list.Managed(u8), table: *const Table, prefix: []const []const u8) !void {
        ...
        while (it.next()) |entry| {
            ...
            if (val.* == .table) {
                // Build full path
                var full_path = std.array_list.Managed([]const u8).init(list.allocator);
                ...
                if (arr.len > 0 and arr[0] == .table) {
                    ...
                    for (arr) |item| {
                        ...
                        const empty_prefix: []const []const u8 = &.{};
    ...
    fn serializeValue(list: *std.array_list.Managed(u8), key: []const u8, value: *const Value) !void {
...
const Parser = struct {
    ...
    fn parseArray(self: *Parser) ParseError![]Value {
        ...
        var items = std.array_list.Managed(Value).init(self.allocator);
        errdefer {
    ...
    fn getOrCreateArrayTable(self: *Parser, root: *Table, path: []const u8) ParseError!*Table {
        var current = root;
        var path_iter = std.mem.splitSequence(u8, path, ".");
        var segments = std.array_list.Managed([]const u8).init(self.allocator);
