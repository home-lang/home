// Copied verbatim from bun/src/libdeflate_sys/libdeflate.zig at upstream
// SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../cli/LICENSE.bun.md.
//
// Zig wrapper around the vendored libdeflate compression library.
// `libdeflate_*` extern symbols stay verbatim — they are the C ABI surface
// the linked `libdeflate.a` exposes and must not be renamed.
//
// The `load_once` hook routes the library's heap allocator through mimalloc
// to keep allocations consistent with the rest of the runtime.
...
test "libdeflate extern symbol signatures compile" {
    _ = @typeName(@TypeOf(libdeflate_alloc_compressor));
    _ = @typeName(@TypeOf(libdeflate_free_compressor));
    _ = @typeName(@TypeOf(libdeflate_zlib_compress));
    _ = @typeName(@TypeOf(libdeflate_gzip_decompress));
    _ = @typeName(@TypeOf(libdeflate_set_memory_allocator));
    try std.testing.expectEqual(@as(c_uint, 0), @intFromEnum(Status.success));
    try std.testing.expectEqual(@as(c_uint, 3), @intFromEnum(Status.insufficient_space));
    // Ensure the Encoding enum tags are stable
    try std.testing.expectEqualStrings("deflate", @tagName(Encoding.deflate));
    try std.testing.expectEqualStrings("gzip", @tagName(Encoding.gzip));
}
