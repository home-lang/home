...
pub const Row = struct {
    ...
    fn parseValueAndSetCell(this: *Row, cell: *SQLDataCell, column: *const ColumnDefinition41, value: *const Data) void {
        ...
        return switch (column.column_type) {
            ...
            // NEWDECIMAL is always ASCII decimal text. Computed decimals
            // (SUM/AVG/arithmetic/CAST) carry the BINARY flag and charset 63, so the
            // catch-all arm's binary-charset heuristic would wrongly return them as a Buffer.
    ...
    fn decodeText(this: *Row, allocator: std.mem.Allocator, comptime Context: type, reader: NewReader(Context)) AnyMySQLError.Error!void {
        ...
        for (cells, 0..) |*value, index| {
            if (decodeLengthInt(reader.peek())) |result| {
                ...
                // value is length-encoded as `0xfc 0xfb 0x00` and also decodes to
                // value 251, so the marker must be distinguished by its 1-byte
                // encoding or row decoding desynchronizes.
                if (result.bytes_read == 1 and result.value == 0xfb) {
                    // NULL value
