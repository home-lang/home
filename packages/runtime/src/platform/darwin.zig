// Copied from bun/src/platform/darwin.zig at upstream
// SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../../cli/LICENSE.bun.md.
// Imports rewritten: @import("bun") → @import("home_rt"). The upstream file
// has zero `bun.X` references (only an unused namespace import); the rewrite
// keeps the import for symmetry with the rest of the substrate.
// The `Bun__signpost_emit` extern stays declared — its C implementation
// resolves at link time in the macOS-only target chain.
...
test "darwin.OSLog.Category tag values match Instruments expectations" {
    try std.testing.expectEqual(@as(u8, 0), @intFromEnum(OSLog.Category.PointsOfInterest));
    try std.testing.expectEqual(@as(u8, 1), @intFromEnum(OSLog.Category.Dynamicity));
    try std.testing.expectEqual(@as(u8, 2), @intFromEnum(OSLog.Category.SizeAndThroughput));
    try std.testing.expectEqual(@as(u8, 3), @intFromEnum(OSLog.Category.TimeProfile));
    try std.testing.expectEqual(@as(u8, 4), @intFromEnum(OSLog.Category.SystemReporting));
    try std.testing.expectEqual(@as(u8, 5), @intFromEnum(OSLog.Category.UserCustom));
}

test "darwin.OSLog.Subsystem strings are non-empty" {
    try std.testing.expect(OSLog.Subsystem.Network.len > 0);
    try std.testing.expect(OSLog.Subsystem.FileIO.len > 0);
    try std.testing.expect(OSLog.Subsystem.Graphics.len > 0);
    try std.testing.expect(OSLog.Subsystem.Memory.len > 0);
    try std.testing.expect(OSLog.Subsystem.Performance.len > 0);
}
