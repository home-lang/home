...
pub fn init(
    ...
    return bun.new(WebSocketProxyTunnel, .{
        .ref_count = .init(),
        ._upgrade_client = if (comptime ssl) .{ .https = upgrade_client } else .{ .http = upgrade_client },
        ._socket = if (comptime ssl) .{ .ssl = socket } else .{ .tcp = socket },
        ._sni_hostname = try bun.default_allocator.dupe(u8, sni_hostname),
        ._reject_unauthorized = reject_unauthorized,
    });
}
...
fn deinit(this: *WebSocketProxyTunnel) void {
    ...
    this._write_buffer.deinit();
    if (this._sni_hostname) |hostname| {
        bun.default_allocator.free(hostname);
        this._sni_hostname = null;
    }
    bun.destroy(this);
}
...
pub fn start(this: *WebSocketProxyTunnel, ssl_options: SSLConfig, initial_data: []const u8) !void {
    ...
    // check uses this._reject_unauthorized field.
    const options = ssl_options.forClientVerification();

    this._wrapper = try SSLWrapperType.init(options, true, .{
        .ctx = this,
        .onOpen = onOpen,
        .onData = onData,
    ...
    if (initial_data.len > 0) {
        this._wrapper.?.startWithPayload(initial_data);
    } else {
        this._wrapper.?.start();
    }
}
        ...
        if (wrapper.ssl) |ssl_ptr| {
            if (this._sni_hostname) |hostname| {
                if (!bun.strings.isIPAddress(hostname)) {
                    // Set SNI hostname
                    const hostname_z = bun.dupeZ(bun.default_allocator, u8, hostname) catch return;
...
fn onHandshake(this: *WebSocketProxyTunnel, success: bool, ssl_error: uws.us_bun_verify_error_t) void {
    ...
    if (this._upgrade_client.isNone()) return;

    if (!success) {
        this._upgrade_client.terminate(ErrorCode.tls_handshake_failed);
        return;
    }

    // Check for SSL errors if we need to reject unauthorized
    if (this._reject_unauthorized) {
    ...
    this._upgrade_client.onProxyTLSHandshakeComplete();
}
...
fn onClose(this: *WebSocketProxyTunnel) void {
    ...
    this._upgrade_client.terminate(ErrorCode.ended);
}
...
pub fn clearConnectedWebSocket(this: *WebSocketProxyTunnel) void {
    this._connected_websocket = null;
}
...
fn writeEncrypted(this: *WebSocketProxyTunnel, encrypted_data: []const u8) void {
    ...
    if (this._write_buffer.isNotEmpty()) {
        bun.handleOom(this._write_buffer.write(encrypted_data));
        return;
    }

    // Try direct write to socket
    const written = this._socket.write(encrypted_data);
    if (written < 0) {
...
pub fn onWritable(this: *WebSocketProxyTunnel) void {
    ...
    const to_send = this._write_buffer.slice();
    if (to_send.len > 0) {
        ...
        if (written_usize == to_send.len) {
            ...
            this._write_buffer.cursor += written_usize;
...
pub fn hasBackpressure(this: *const WebSocketProxyTunnel) bool {
    return this._write_buffer.isNotEmpty();
}
