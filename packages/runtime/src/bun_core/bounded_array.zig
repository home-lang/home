// Copied verbatim from bun/src/bun_core/bounded_array.zig at upstream SHA
...
test "BoundedArray.init sets len and slice has matching length" {
    var a = try BoundedArray(u8, 16).init(4);
    try testing.expectEqual(@as(usize, 4), a.slice().len);
    try testing.expectEqual(@as(usize, 16), a.capacity());
}

test "BoundedArray.init rejects overflow" {
    try testing.expectError(error.Overflow, BoundedArray(u8, 4).init(5));
}

test "BoundedArray append / get / pop round trip" {
    var a = try BoundedArray(u8, 8).init(0);
    try a.append(1);
    try a.append(2);
    try a.append(3);
    try testing.expectEqual(@as(usize, 3), a.slice().len);
    try testing.expectEqual(@as(u8, 2), a.get(1));
    try testing.expectEqual(@as(u8, 3), a.pop().?);
    try testing.expectEqual(@as(usize, 2), a.slice().len);
}

test "BoundedArray fromSlice copies content" {
    var a = try BoundedArray(u8, 16).fromSlice("hello");
    try testing.expectEqual(@as(usize, 5), a.slice().len);
    try testing.expectEqualSlices(u8, "hello", a.constSlice());
}
