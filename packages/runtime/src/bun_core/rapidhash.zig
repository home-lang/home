// Extracted from bun/src/bun_core/deprecated.zig's `RapidHash` at upstream SHA
// fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT.
//
// Pulled into a dedicated module (mirrors the `singly_linked_list.zig`
// extraction) because importing `bun_core/deprecated.zig` whole eagerly parses
// its `test "RapidHash.hash"` body — whose `"abcdefgh" ** 128` trips the pinned
// Zig 0.17.0-dev.131 `**` tokenizer bug. The hash body itself is unaffected, so
// it lives here verbatim minus the test. Used by `Bun.hash.rapidhash`.
