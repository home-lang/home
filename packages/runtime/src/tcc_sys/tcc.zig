// Copied verbatim from bun/src/tcc_sys/tcc.zig at upstream
// SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../cli/LICENSE.bun.md.
//
// Zig wrapper around the vendored TinyCC C compiler.
// `tcc_*` extern symbols stay verbatim — they are the C ABI surface the
// linked `libtcc.a` exposes and must not be renamed.
...
test "tcc extern symbol signatures compile" {
    _ = @typeName(@TypeOf(tcc_new));
    _ = @typeName(@TypeOf(tcc_delete));
    _ = @typeName(@TypeOf(tcc_compile_string));
    _ = @typeName(@TypeOf(tcc_run));
    try std.testing.expectEqual(@as(c_int, 1), TCC_OUTPUT_MEMORY);
    try std.testing.expectEqual(@as(c_int, 2), @intFromEnum(OutputFormat.Exe));
    // Ensure both error tags referenced by the wrappers exist in the union
    const ErrorSet = Error;
    _ = @errorName(@as(ErrorSet, error.OutputError));
    _ = @errorName(@as(ErrorSet, error.RelocationError));
}
