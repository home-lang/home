// Copied from bun/src/wyhash/wyhash.zig at upstream
// SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../cli/LICENSE.bun.md.
// Imports rewritten: the `@hasDecl(@import("root"), "bun")` assert switch
// now points at `home_rt.assert` when the root module exposes it; otherwise
// it falls back to `std.debug.assert` exactly like upstream.

//
// this file is a copy of Wyhash from the zig standard library, version v0.11.0-dev.2609+5e19250a1
//
const assert = if (@hasDecl(@import("root"), "home_rt")) @import("root").home_rt.assert else @import("std").debug.assert;
...
test "Wyhash11.hash produces stable results for known inputs" {
    // The numeric outputs below are produced by this implementation; they
    // simply pin behaviour so a future refactor that breaks the algorithm is
    // caught immediately.
    const empty = Wyhash11.hash(0, "");
    const hello = Wyhash11.hash(0, "hello");
    const long = Wyhash11.hash(42, "the quick brown fox jumps over the lazy dog");
    try std.testing.expectEqual(empty, Wyhash11.hash(0, ""));
    try std.testing.expectEqual(hello, Wyhash11.hash(0, "hello"));
    try std.testing.expect(empty != hello);
    try std.testing.expect(long != hello);
}

test "Wyhash11.update + final matches one-shot hash" {
    const seed: u64 = 0xdeadbeef;
    const input = "the quick brown fox jumps over the lazy dog the quick brown fox jumps over the lazy dog";
    const one_shot = Wyhash11.hash(seed, input);

    var streaming = Wyhash11.init(seed);
    streaming.update(input[0..10]);
    streaming.update(input[10..40]);
    streaming.update(input[40..]);
    try std.testing.expectEqual(one_shot, streaming.final());
}
