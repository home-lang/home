// Copied verbatim from bun/src/io/heap.zig at upstream
// SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../cli/LICENSE.bun.md.
//
// Sole dependency on the bun namespace was `bun.assert`; rewritten to
// `home_rt.assert` (which mirrors the upstream semantics — see
// `src/global.zig`).
...
test "io.heap.Intrusive: insert + deleteMin yields sorted output" {
    ...
    const Elem = struct {
        value: u32,
        heap: IntrusiveField(@This()) = .{},
    };
    const lessThan = struct {
        fn less(_: void, a: *Elem, b: *Elem) bool {
    ...
    var heap = Intrusive(Elem, void, lessThan){ .context = {} };
    for (nodes[0..]) |*n| heap.insert(n);

    try std.testing.expectEqual(@as(usize, 5), heap.count());

    var prev: u32 = 0;
    var seen: usize = 0;
    while (heap.deleteMin()) |n| {
        try std.testing.expect(n.value > prev);
        prev = n.value;
        seen += 1;
    }
...
test "io.heap.Intrusive: remove(non-root) preserves ordering of remaining elements" {
    const std = @import("std");
    const Elem = struct {
        value: u32,
        heap: IntrusiveField(@This()) = .{},
    ...
    heap.remove(&nodes[2]); // remove value 30
    try std.testing.expectEqual(@as(usize, 3), heap.count());

    var values: [3]u32 = undefined;
    var i: usize = 0;
    while (heap.deleteMin()) |n| : (i += 1) {
        values[i] = n.value;
    }
    try std.testing.expectEqualSlices(u32, &.{ 10, 20, 40 }, &values);
}
