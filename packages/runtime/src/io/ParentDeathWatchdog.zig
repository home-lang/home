...
var sync_pgids_buf: [4]std.c.pid_t = std.mem.zeroes([4]std.c.pid_t);
...
fn parentPidOf(pid: std.c.pid_t) std.c.pid_t {
    ...
    if (comptime Environment.isLinux) {
        ...
        const path = std.fmt.bufPrintSentinel(&path_buf, "/proc/{d}/stat", .{pid}, 0) catch return 0;
...
fn listChildPidsLinux(parent: std.c.pid_t, out: []std.c.pid_t) ?usize {
    ...
    while (it.next().unwrap() catch null) |entry| {
        ...
        const children_path = std.fmt.bufPrintSentinel(&path_buf, "/proc/{d}/task/{d}/children", .{ parent, tid }, 0) catch continue;
