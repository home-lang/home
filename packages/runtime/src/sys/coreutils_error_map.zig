// Copied from bun/src/sys/coreutils_error_map.zig at upstream SHA
// fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../cli/LICENSE.bun.md.
//
// Comptime-built `SystemErrno → strerror string` table mirroring what
// coreutils prints. Strings have constant lifetime (comptime-evaluated
// `std.EnumMap`). The mac branch maps macOS's slightly different
// messages so shells/coreutils-on-Bun behave consistently.
//
// Rewrites vs. upstream:
//   * `bun.assert` / `bun.Environment` / `bun.sys.SystemErrno` →
//     `home_rt.assert` / `home_rt.Environment.os` / `errno.SystemErrno`.
//   * `@hasField(SystemErrno, key)` silently skips errnos that aren't
//     in the per-platform table (e.g. macOS-only `EDOOFUS` on Linux).
...
test "coreutils_error_map exposes a non-empty ENOENT entry" {
    try std.testing.expectEqualStrings("No such file or directory", coreutils_error_map.get(SystemErrno.ENOENT).?);
}

test "coreutils_error_map falls back to 'unknown error' for unmapped codes" {
    // SUCCESS is errno 0; coreutils never has a strerror entry for it.
    try std.testing.expectEqualStrings("unknown error", coreutils_error_map.get(SystemErrno.SUCCESS).?);
}
