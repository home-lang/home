// Copied from bun/src/perf/hw_timer.zig at upstream
// SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../cli/LICENSE.bun.md.
//
// Wave-19 unmined-corner port (2026-05-19). Pulls in the unbarriered TSC
// timer from an under-mined directory (`bun/src/perf/`). The upstream file
// references `bun.timespec` for its OS fallback path; this port replaces
// that with a direct `std.posix.clock_gettime` so we don't need to drag in
// the runtime-shared `timespec` wrapper just yet. All ASM paths and the
// calibration logic are byte-identical to upstream.
...
/// Monotonic nanoseconds, calibrated to the same epoch as the OS monotonic
/// clock. Falls back to the OS high-res clock if the HW counter frequency
...
fn osMonotonicNs() u64 {
    ...
    // Bun used `bun.timespec.now(.force_real_time).ns()`. We lower that to
    // a direct `clock_gettime` to avoid pulling the full `timespec` wrapper.
...
test "hw_timer: nowNs is non-zero and weakly monotonic on supported targets" {
    ...
    // Same-call ordering: `b` came after `a`. The HW path is monotonic by
    // construction; the OS fallback uses `CLOCK_MONOTONIC`.
    try std.testing.expect(b >= a);
}

test "hw_timer: is_supported tracks aarch64 + x86_64 only" {
    const expected = Environment.isAarch64 or Environment.isX64;
    try std.testing.expectEqual(expected, is_supported);
}
