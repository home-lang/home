// Copied verbatim from bun/src/zlib_sys/posix.zig at upstream
// SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../cli/LICENSE.bun.md.
//
// Wave-13 (2026-05-18) port. Pure FFI surface around `<zlib.h>` for POSIX
// targets. No `@import("bun")` needed — only depends on the sibling
// `shared.zig` enums + std.
...
test "zlib_sys.posix: z_stream is binary-compatible with `struct z_stream_s`" {
    const std = @import("std");
    // 13 pointer-or-aligned-int fields. Exact size varies with target; the
    // type just needs to be extern-struct so callers can pass `&zs` to the
    // FFI without any layout games. This test ensures the struct compiles
    // and has a non-zero size on the target.
    try std.testing.expect(@sizeOf(z_stream) > 0);
    try std.testing.expectEqual(@alignOf(*anyopaque), @alignOf(z_stream));
}

test "zlib_sys.posix: re-exports the shared enums under stable names" {
    const shared = @import("./shared.zig");
    const std = @import("std");
    try std.testing.expectEqual(shared.DataType, DataType);
    try std.testing.expectEqual(shared.FlushValue, FlushValue);
    try std.testing.expectEqual(shared.ReturnCode, ReturnCode);
}
