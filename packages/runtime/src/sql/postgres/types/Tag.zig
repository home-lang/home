// Copied from bun/src/sql/postgres/types/Tag.zig at upstream
// SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../../../cli/LICENSE.bun.md.
//
// JSC-bridge `toJSTypedArrayType` / `toJS` / `fromJS` re-exports omitted —
// re-lands in Phase 12.2 once the JSC engine is brought up.
...
test "Tag.tagName resolves canonical Postgres OIDs" {
    try std.testing.expectEqualStrings("bool", Tag.bool.tagName().?);
    try std.testing.expectEqualStrings("int4", Tag.int4.tagName().?);
    try std.testing.expectEqualStrings("text_array", Tag.text_array.tagName().?);
}

test "Tag.isBinaryFormatSupported flags binary-encoded scalar types" {
    try std.testing.expect(Tag.bool.isBinaryFormatSupported());
    try std.testing.expect(Tag.int4.isBinaryFormatSupported());
    try std.testing.expect(Tag.bytea.isBinaryFormatSupported());
    try std.testing.expect(Tag.numeric.isBinaryFormatSupported());
    try std.testing.expect(!Tag.text.isBinaryFormatSupported());
    try std.testing.expect(!Tag.uuid.isBinaryFormatSupported());
}

test "Tag.formatCode picks 1 for binary types, 0 otherwise" {
    try std.testing.expectEqual(@as(short, 1), Tag.int4.formatCode());
    try std.testing.expectEqual(@as(short, 0), Tag.text.formatCode());
}
