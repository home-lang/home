// Copied verbatim from bun/src/sql/postgres/protocol/Close.zig at
// upstream SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see
// ../../../cli/LICENSE.bun.md. No `@import("bun")` references.
//
// Postgres Close (F) packet. Wire writer body references the wave-16
// NewWriter stub method surface (writer.write); the calls trip
// `@compileError` until the real writer returns, so this leaf is
...
test "Close holds a PortalOrPreparedStatement" {
    const std = @import("std");
    const c: Close = .{ .p = .{ .portal = "stmt_1" } };
    try std.testing.expectEqualStrings("stmt_1", c.p.slice());
