// Copied verbatim from bun/src/sql/postgres/protocol/FieldType.zig at upstream
...
test "FieldType uses canonical Postgres single-byte tags" {
    const std = @import("std");
    try std.testing.expectEqual(@as(u8, 'S'), @intFromEnum(FieldType.severity));
    try std.testing.expectEqual(@as(u8, 'C'), @intFromEnum(FieldType.code));
    try std.testing.expectEqual(@as(u8, 'M'), @intFromEnum(FieldType.message));
    try std.testing.expectEqual(@as(u8, 'p'), @intFromEnum(FieldType.internal_position));
    try std.testing.expectEqual(@as(u8, 'q'), @intFromEnum(FieldType.internal));
    try std.testing.expectEqual(@as(u8, 'R'), @intFromEnum(FieldType.routine));
}
