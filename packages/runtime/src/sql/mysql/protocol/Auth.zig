...
pub const mysql_native_password = struct {
    pub fn scramble(password: []const u8, nonce: []const u8) ![20]u8 {
        ...
        home_rt.sha.SHA1.hash(password, &stage1, jsc.VirtualMachine.get().rareData().boringEngine());

        // Stage 2: SHA1(SHA1(password))
        home_rt.sha.SHA1.hash(&stage1, &stage2, jsc.VirtualMachine.get().rareData().boringEngine());

        // Stage 3: SHA1(nonce + SHA1(SHA1(password)))
        var sha1 = home_rt.sha.SHA1.init();
        defer sha1.deinit();
...
pub const caching_sha2_password = struct {
    pub fn scramble(password: []const u8, nonce: []const u8) ![32]u8 {
        ...
        // SHA256(password)
        home_rt.sha.SHA256.hash(password, &digest1, jsc.VirtualMachine.get().rareData().boringEngine());

        // SHA256(SHA256(password))
        home_rt.sha.SHA256.hash(&digest1, &digest2, jsc.VirtualMachine.get().rareData().boringEngine());

        // SHA256(SHA256(SHA256(password)) + nonce)
        const combined = try home_rt.default_allocator.alloc(u8, nonce.len + digest2.len);
        defer home_rt.default_allocator.free(combined);
        @memcpy(combined[0..nonce.len], nonce);
        @memcpy(combined[nonce.len..], &digest2);
        home_rt.sha.SHA256.hash(combined, &digest3, jsc.VirtualMachine.get().rareData().boringEngine());
        ...
        pub fn writeInternal(this: *const EncryptedPassword, comptime Context: type, writer: NewWriter(Context)) !void {
            ...
            var plain_password = brk: {
                ...
                if (needed_len > password_buf.len) {
                    ...
                    break :brk try home_rt.default_allocator.alloc(u8, needed_len);
                } else {
            ...
            defer if (needs_to_free_password) home_rt.default_allocator.free(plain_password);
            ...
            var encrypted_password = brk: {
                if (rsa_size > encrypted_password_buf.len) {
                    ...
                    break :brk try home_rt.default_allocator.alloc(u8, rsa_size);
                } else {
                    break :brk encrypted_password_buf[0..rsa_size];
                }
            };
            defer if (needs_to_free_encrypted_password) home_rt.default_allocator.free(encrypted_password);
...
const debug = home_rt.Output.scoped(.Auth, .hidden);
...
const BoringSSL = home_rt.BoringSSL;
