// Copied from bun/src/sql/mysql/MySQLTypes.zig at upstream
// SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../../cli/LICENSE.bun.md.
//
// JSC-bridge re-exports omitted — re-lands in Phase 12.2:
//   - `FieldType.fromJS` (lives in `sql_jsc/mysql/MySQLValue.zig`)
//   - `Value` (alias to `sql_jsc/mysql/MySQLValue.zig::Value`)
...
test "MySQLTypes integer aliases match canonical width" {
    const std = @import("std");
    try std.testing.expectEqual(@as(comptime_int, 8), @bitSizeOf(MySQLInt8));
    try std.testing.expectEqual(@as(comptime_int, 16), @bitSizeOf(MySQLInt16));
    try std.testing.expectEqual(@as(comptime_int, 24), @bitSizeOf(MySQLInt24));
    try std.testing.expectEqual(@as(comptime_int, 32), @bitSizeOf(MySQLInt32));
    try std.testing.expectEqual(@as(comptime_int, 64), @bitSizeOf(MySQLInt64));
}

test "FieldType.isBinaryFormatSupported flags wire-binary scalars" {
    const std = @import("std");
    try std.testing.expect(FieldType.MYSQL_TYPE_TINY.isBinaryFormatSupported());
    try std.testing.expect(FieldType.MYSQL_TYPE_LONGLONG.isBinaryFormatSupported());
    try std.testing.expect(FieldType.MYSQL_TYPE_DATE.isBinaryFormatSupported());
    try std.testing.expect(!FieldType.MYSQL_TYPE_VAR_STRING.isBinaryFormatSupported());
    try std.testing.expect(!FieldType.MYSQL_TYPE_BLOB.isBinaryFormatSupported());
    try std.testing.expect(!FieldType.MYSQL_TYPE_JSON.isBinaryFormatSupported());
}

test "CharacterSet.default round-trips" {
    const std = @import("std");
    try std.testing.expectEqual(@as(u8, 45), @intFromEnum(CharacterSet.default));
    try std.testing.expectEqualStrings("utf8mb4_general_ci", CharacterSet.utf8mb4_general_ci.label());
    try std.testing.expectEqualStrings("(unknown)", CharacterSet.utf8mb4_unicode_ci.label());
}
