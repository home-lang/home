// Copied verbatim from bun/src/sql/mysql/Capabilities.zig at upstream
...
test "Capabilities.getDefaultCapabilities respects ssl + db flags" {
    const caps = getDefaultCapabilities(true, false);
    try std.testing.expect(caps.CLIENT_PROTOCOL_41);
    try std.testing.expect(caps.CLIENT_PLUGIN_AUTH);
    try std.testing.expect(caps.CLIENT_SECURE_CONNECTION);
    try std.testing.expect(caps.CLIENT_SSL);
    try std.testing.expect(!caps.CLIENT_CONNECT_WITH_DB);

    const no_ssl = getDefaultCapabilities(false, true);
    try std.testing.expect(!no_ssl.CLIENT_SSL);
    try std.testing.expect(no_ssl.CLIENT_CONNECT_WITH_DB);
}

test "Capabilities.toInt + fromInt round-trip canonical bits" {
    const caps = getDefaultCapabilities(true, true);
    const raw = caps.toInt();
    const decoded = fromInt(raw);
    try std.testing.expectEqual(raw, decoded.toInt());
    try std.testing.expect(decoded.CLIENT_PROTOCOL_41);
    try std.testing.expect(decoded.CLIENT_SSL);
}

test "Capabilities.intersect masks to the common bits" {
    var lhs: Capabilities = .{};
    lhs.CLIENT_PROTOCOL_41 = true;
    lhs.CLIENT_SSL = true;
    var rhs: Capabilities = .{};
    rhs.CLIENT_SSL = true;
    rhs.CLIENT_PLUGIN_AUTH = true;

    const got = lhs.intersect(rhs);
    try std.testing.expect(got.CLIENT_SSL);
    try std.testing.expect(!got.CLIENT_PROTOCOL_41);
    try std.testing.expect(!got.CLIENT_PLUGIN_AUTH);
}

test "Capabilities.reject zeros server-only feature bits" {
    var caps: Capabilities = .{
        .CLIENT_COMPRESS = true,
        .CLIENT_PROTOCOL_41 = true,
        .CLIENT_QUERY_ATTRIBUTES = true,
        .CLIENT_REMEMBER_OPTIONS = true,
    };
    caps.reject();
    try std.testing.expect(caps.CLIENT_PROTOCOL_41);
    try std.testing.expect(!caps.CLIENT_COMPRESS);
    try std.testing.expect(!caps.CLIENT_QUERY_ATTRIBUTES);
    try std.testing.expect(!caps.CLIENT_REMEMBER_OPTIONS);
}
