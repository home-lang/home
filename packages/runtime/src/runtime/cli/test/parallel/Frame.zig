// Copied from bun/src/runtime/cli/test/parallel/Frame.zig at upstream SHA
...
// No Bun→Home identifier renames apply (no public `Bun…` symbols here).
...
pub fn u32_(self: *Frame, v: u32) void {
    ...
    home_rt.handleOom(self.buf.appendSlice(home_rt.default_allocator, &le));
}
pub fn str(self: *Frame, s: []const u8) void {
    ...
    if (s.len <= room) {
        ...
        home_rt.handleOom(self.buf.appendSlice(home_rt.default_allocator, s));
        return;
    }
    const keep: usize = if (room > trunc.len) room - trunc.len else 0;
    self.u32_(@intCast(keep + trunc.len));
    home_rt.handleOom(self.buf.appendSlice(home_rt.default_allocator, s[0..keep]));
    home_rt.handleOom(self.buf.appendSlice(home_rt.default_allocator, trunc));
}
...
pub fn finish(self: *Frame) []const u8 {
    ...
    home_rt.assert(payload_len <= max_payload);
    std.mem.writeInt(u32, self.buf.items[0..4], payload_len, .little);
    return self.buf.items;
}
pub fn deinit(self: *Frame) void {
    self.buf.deinit(home_rt.default_allocator);
}
...
test "Frame.Reader returns sentinels on truncation" {
    var r = Reader{ .p = &[_]u8{ 1, 2 } };
    try std.testing.expectEqual(@as(u32, 0), r.u32_());
