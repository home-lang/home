...
pub const CommandLineReporter = struct {
    ...
    pub fn printCodeCoverage(
        ...
        const lcov_file, const lcov_name, var lcov_buffered_writer = brk: {
            ...
            const tmpname = std.fmt.bufPrintSentinel(&shortname_buf, ".lcov.info.{x}.tmp", .{&base64_bytes}, 0) catch unreachable;
...
pub const TestCommand = struct {
    ...
    pub fn exec(ctx: Command.Context) !void {
        ...
        var inline_snapshots_to_write = bun.AutoArrayHashMap(TestRunner.File.ID, std.array_list.Managed(Snapshots.InlineSnapshotToWrite)).init(ctx.allocator);
