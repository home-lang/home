...
pub const ValkeyClient = struct {
    ...
    fn authenticate(this: *ValkeyClient) bun.JSTerminated!void {
        ...
        if (this.database > 0) {
            ...
            const db_str = std.fmt.bufPrintSentinel(&int_buf, "{d}", .{this.database}, 0) catch unreachable;
    ...
    pub fn writer(this: *ValkeyClient) bun.io.GenericWriter(*ValkeyClient, protocol.RedisError, write) {
        return .{ .context = this };
    }
