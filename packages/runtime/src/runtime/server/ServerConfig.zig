// Copied from bun/src/runtime/server/ServerConfig.zig at upstream SHA fd0b6f1a27. MIT.
// Bun.serve port (2026-06-24).
...
pub fn computeID(this: *const ServerConfig, allocator: std.mem.Allocator) []const u8 {
    ...
    // Zig-0.17: Managed ArrayList lost `.writer()`; use its print/appendSlice.
    arraylist.appendSlice("[http]-") catch {};
    switch (this.address) {
        .tcp => {
            if (this.address.tcp.hostname) |host| {
                arraylist.print("tcp:{s}:{d}", .{
                    bun.sliceTo(host, 0),
                    this.address.tcp.port,
                }) catch {};
            } else {
                arraylist.print("tcp:localhost:{d}", .{
                    this.address.tcp.port,
                }) catch {};
            }
        },
        .unix => {
            arraylist.print("unix:{s}", .{
    ...
    if (arguments.next()) |arg| {
        ...
        if (try getRoutesObject(global, arg)) |static| {
                ...
                init_ctx.framework_router_list.items.len > 0))
            {
                ...
                // Zig-0.17 ArenaAllocator.State has used_list/free_list (not
                // end_index/buffer_list); queryCapacity()==0 is the equivalent
                // "nothing was allocated" check.
                bun.debugAssert(init_ctx.arena.queryCapacity() == 0);
                init_ctx.arena.deinit();
            }
        }
