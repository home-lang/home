// Copied verbatim bun/src/runtime/image/thumbhash.zig at upstream SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../../cli/LICENSE.bun.md.
//
...
//!
//! Home divergence: upstream uses `bun.debugAssert` / `bun.default_allocator`.
//! We use `home_rt.assert` and `home_rt.default_allocator` — same semantics.
...
pub fn encode(out: *[max_len]u8, w: u32, h: u32, rgba: []const u8) []u8 {
    home_rt.assert(w > 0 and w <= 100 and h > 0 and h <= 100);
...
const Channel = struct {
    dc: f32 = 0,
    scale: f32 = 0,
    // Home divergence: see `encode` for the `** N` → `@splat(0)` rewrite.
...
/// hash is too short. Output is `home_rt.default_allocator`-owned.
pub fn decode(hash: []const u8) error{ DecodeFailed, OutOfMemory }!struct { rgba: []u8, w: u32, h: u32 } {
    ...
    const rgba = try home_rt.default_allocator.alloc(u8, @as(usize, w) * h * 4);
    errdefer home_rt.default_allocator.free(rgba);
...
test "thumbhash.max_len matches worst-case packing budget" {
    try std.testing.expectEqual(@as(comptime_int, 25), max_len);
}

test "thumbhash.decode: short hash returns DecodeFailed" {
    try std.testing.expectError(error.DecodeFailed, decode(""));
    try std.testing.expectError(error.DecodeFailed, decode("abcd"));
}

test "thumbhash.encode -> decode: solid red 8x8" {
    ...
    const hash = encode(&out, 8, 8, &rgba);
    try std.testing.expect(hash.len >= 5 and hash.len <= max_len);

    const decoded = try decode(hash);
    defer home_rt.default_allocator.free(decoded.rgba);
    try std.testing.expect(decoded.w > 0 and decoded.w <= 32);
    try std.testing.expect(decoded.h > 0 and decoded.h <= 32);
    // Centre pixel should still be predominantly red.
    const o = ((decoded.h / 2) * decoded.w + (decoded.w / 2)) * 4;
    try std.testing.expect(decoded.rgba[o + 0] > decoded.rgba[o + 1]);
    try std.testing.expect(decoded.rgba[o + 0] > decoded.rgba[o + 2]);
}

test "thumbhash.encode: opaque image produces no alpha bit" {
    ...
    var out: [max_len]u8 = undefined;
    const hash = encode(&out, 4, 4, &rgba);
    // Bit 23 of the 24-bit header signals alpha presence; should be 0.
    const h24: u32 = @as(u32, hash[0]) | (@as(u32, hash[1]) << 8) | (@as(u32, hash[2]) << 16);
    try std.testing.expectEqual(@as(u32, 0), (h24 >> 23) & 1);
}
