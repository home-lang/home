// Copied verbatim bun/src/runtime/image/quantize.zig at upstream SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../../cli/LICENSE.bun.md.
//
...
//!
//! Home divergence: `bun.default_allocator` → `home_rt.default_allocator`.
//! The `bun_image_nearest_palette` extern symbol is an FFI hook into the
//! Highway kernel; it stays verbatim and links the same C ABI once the
//! Highway backend lands. The unit tests run the parts of `quantize` that
//! don't take the extern code path — `Floyd–Steinberg dither: false` only
//! exercises the nearest-palette extern, so we cover the no-dither median-
//! cut path with the small "single-colour bitmap" cases.

pub const Result = struct {
    /// `[colors][4]u8` RGBA palette, `home_rt.default_allocator`-owned.
    palette: []u8,
    /// One palette index per input pixel, `home_rt.default_allocator`-owned.
    indices: []u8,
    ...
    pub fn deinit(self: *Result) void {
        home_rt.default_allocator.free(self.palette);
        home_rt.default_allocator.free(self.indices);
    }
};
...
pub fn quantize(rgba: []const u8, w: u32, h: u32, opts: Options) error{OutOfMemory}!Result {
    ...
    var order = try home_rt.default_allocator.alloc(u32, n);
    defer home_rt.default_allocator.free(order);
    for (order, 0..) |*o, i| o.* = @intCast(i);

    var boxes = try std.ArrayList(Box).initCapacity(home_rt.default_allocator, want);
    defer boxes.deinit(home_rt.default_allocator);
    ...
    var palette = try home_rt.default_allocator.alloc(u8, @as(usize, k) * 4);
    errdefer home_rt.default_allocator.free(palette);
    var has_alpha = false;
    for (boxes.items, 0..) |b, i| {
    ...
    var indices = try home_rt.default_allocator.alloc(u8, n);
    errdefer home_rt.default_allocator.free(indices);
    if (opts.dither) {
...
fn mapFloydSteinberg(
    ...
    var cur = try home_rt.default_allocator.alloc(i32, stride);
    defer home_rt.default_allocator.free(cur);
    var nxt = try home_rt.default_allocator.alloc(i32, stride);
    defer home_rt.default_allocator.free(nxt);
    @memset(cur, 0);
    @memset(nxt, 0);
...
test "quantize.Box.widestChannel: picks the largest range" {
    const b1: Box = .{ .lo = 0, .hi = 1, .min = .{ 0, 0, 0, 0 }, .max = .{ 50, 200, 10, 5 } };
    try std.testing.expectEqual(@as(u2, 1), b1.widestChannel());
    const b2: Box = .{ .lo = 0, .hi = 1, .min = .{ 0, 0, 0, 0 }, .max = .{ 200, 50, 10, 5 } };
    try std.testing.expectEqual(@as(u2, 0), b2.widestChannel());
}

test "quantize.shrink: tight bounds for a single pixel" {
    const rgba = [_]u8{ 10, 20, 30, 40 };
    const order = [_]u32{0};
    const b = shrink(&rgba, &order, 0, 1);
    try std.testing.expectEqual(@as(u8, 10), b.min[0]);
    try std.testing.expectEqual(@as(u8, 10), b.max[0]);
    try std.testing.expectEqual(@as(u8, 40), b.min[3]);
    try std.testing.expectEqual(@as(u8, 40), b.max[3]);
}

test "quantize.shrink: tight bounds across multiple pixels" {
    const rgba = [_]u8{ 10, 20, 30, 255, 200, 5, 60, 255 };
    const order = [_]u32{ 0, 1 };
    const b = shrink(&rgba, &order, 0, 2);
    try std.testing.expectEqual(@as(u8, 10), b.min[0]);
    try std.testing.expectEqual(@as(u8, 200), b.max[0]);
    try std.testing.expectEqual(@as(u8, 5), b.min[1]);
    try std.testing.expectEqual(@as(u8, 20), b.max[1]);
}

test "quantize.clamp255: clamps to [0, 255]" {
    try std.testing.expectEqual(@as(i32, 0), clamp255(-100));
    try std.testing.expectEqual(@as(i32, 0), clamp255(0));
    try std.testing.expectEqual(@as(i32, 128), clamp255(128));
    try std.testing.expectEqual(@as(i32, 255), clamp255(255));
    try std.testing.expectEqual(@as(i32, 255), clamp255(1000));
}

test "quantize.SortCtx.less: orders by chosen channel" {
    const rgba = [_]u8{ 10, 50, 0, 0, 20, 30, 0, 0 };
    const ctx: SortCtx = .{ .rgba = &rgba, .ch = 0 };
    try std.testing.expect(ctx.less(0, 1)); // 10 < 20
    const ctx2: SortCtx = .{ .rgba = &rgba, .ch = 1 };
    try std.testing.expect(!ctx2.less(0, 1)); // 50 > 30
}
