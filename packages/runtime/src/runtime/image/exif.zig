// Copied verbatim bun/src/runtime/image/exif.zig at upstream SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../../cli/LICENSE.bun.md.
...
test "exif.Orientation.transform: normal yields identity" {
    const t = Orientation.normal.transform();
    try std.testing.expectEqual(false, t.flop);
    try std.testing.expectEqual(false, t.flip);
    try std.testing.expectEqual(@as(u16, 0), t.rotate);
}

test "exif.Orientation.transform: rotate180 yields 180-degree rotate" {
    const t = Orientation.rotate180.transform();
    try std.testing.expectEqual(@as(u16, 180), t.rotate);
    try std.testing.expectEqual(false, t.flop);
    try std.testing.expectEqual(false, t.flip);
}

test "exif.readJpeg: empty bytes return normal" {
    try std.testing.expectEqual(Orientation.normal, readJpeg(""));
}

test "exif.readJpeg: non-JPEG signature returns normal" {
    try std.testing.expectEqual(Orientation.normal, readJpeg("PNG\x00"));
}
