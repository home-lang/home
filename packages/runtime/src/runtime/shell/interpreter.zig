...
pub const Interpreter = struct {
    ...
    fn initImpl(
        ...
        var export_env = brk: {
            ...
            var export_env = EnvMap.initWithCapacity(allocator, env_loader.map.map.count());
    ...
    fn deinitFromFinalizer(this: *ThisInterpreter) void {
        ...
        switch (this.cleanup_state) {
            ...
            .runtime_cleaned => {
                // finish() already cleaned up IO and shell env via _derefRootShellAndIOIfNeeded,
...
pub fn isPollable(fd: bun.FD, mode: bun.Mode) bool {
    return switch (bun.Environment.os) {
        .windows, .wasm => false,
        .linux, .freebsd => posix.S.ISFIFO(mode) or posix.S.ISSOCK(mode) or bun.c.isatty(fd.native()) != 0,
        ...
        .mac => if (posix.S.ISREG(mode)) false else posix.S.ISFIFO(mode) or posix.S.ISSOCK(mode) or bun.c.isatty(fd.native()) != 0,
    };
}
