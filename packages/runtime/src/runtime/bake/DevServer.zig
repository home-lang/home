...
// Original: src/runtime/bake/DevServer.zig at 4982b91e3702094330f3be3883354c52b8c01323.
...
pub fn init(options: Options) bun.JSOOM!*DevServer {
    ...
    var dump_dir = if (bun.FeatureFlags.bake_debugging_features)
        if (options.dump_sources) |dir|
            std.Io.Dir.cwd().createDirPathOpen(io, dir, .{}) catch |err| dir: {
            ...
            null;
    errdefer if (bun.FeatureFlags.bake_debugging_features) if (dump_dir) |*dir| dir.close(io);
...
pub fn deinit(dev: *DevServer) void {
    ...
    useAllFields(DevServer, .{
        ...
        .dump_dir = if (bun.FeatureFlags.bake_debugging_features) if (dev.dump_dir) |*dir| dir.close(std.Io.Threaded.global_single_threaded.io()),
        .log = dev.log.deinit(),
...
fn onIncrementalVisualizerCorked(resp: AnyResponse) void {
    const code = if (Environment.codegen_embed)
        ...
        bun.runtimeEmbedFile(.src_eager, "runtime/bake/incremental_visualizer.html");
    resp.end(code, false);
}
...
fn onMemoryVisualizerCorked(resp: AnyResponse) void {
    const code = if (Environment.codegen_embed)
        ...
        bun.runtimeEmbedFile(.src_eager, "runtime/bake/memory_visualizer.html");
    resp.end(code, false);
}
...
fn generateJavaScriptCodeForHTMLFile(
    ...
    const w = unmanagedArrayListWriter(&array, sfa);
...
fn indexFailures(dev: *DevServer) !void {
    ...
    if (dev.incremental_result.failures_added.items.len > 0) {
        ...
        const w = managedArrayListWriter(&payload);
...
pub fn finalizeBundle(
    ...
    for (result.htmlChunks()) |*chunk| {
        ...
        html.script_injection_offset = .init(RouteBundle.HTML.ByteOffset.init(compile_result.script_injection_offset));
    ...
    const w = managedArrayListWriter(&hot_update_payload);
...
fn startNextBundleIfPresent(dev: *DevServer) void {
    ...
    if (dev.next_bundle.reload_event != null or dev.next_bundle.requests.first != null or dev.next_bundle.promise.strong.hasValue()) {
        ...
        } else .{ false, bun.Timer.start() catch @panic("timers unsupported") };
...
pub fn handleParseTaskFailure(
    ...
    var fallback_log = bun.logger.Log.init(dev.allocator());
    defer fallback_log.deinit();
    const effective_log = if (log.msgs.items.len > 0) log else fallback: {
        try fallback_log.addErrorFmt(
            null,
            bun.logger.Loc.Empty,
            dev.allocator(),
            "{s} while bundling {s}",
            .{ @errorName(err), abs_path },
        );
        break :fallback &fallback_log;
    };
    ...
    if (err == error.FileNotFound or err == error.ModuleNotFound) {
        ...
        switch (graph) {
            .server => try dev.server_graph.insertFailure(.abs_path, abs_path, effective_log, false),
            .ssr => try dev.server_graph.insertFailure(.abs_path, abs_path, effective_log, true),
            .client => try dev.client_graph.insertFailure(.abs_path, abs_path, effective_log, false),
        }
    }
}
...
pub fn dumpBundle(dump_dir: std.Io.Dir, graph: bake.Graph, rel_path: []const u8, chunk: []const u8, wrap: bool) !void {
    ...
    var inner_dir = try bun.MakePath.makeOpenPath(dump_dir, bun.Dirname.dirname(u8, name).?, .{});
...
pub const HmrTopic = enum(u8) {
    ...
    pub const Bits = packed struct(u6) {
...
fn dumpStateDueToCrash(dev: *DevServer) !void {
    ...
    const filepath = std.fmt.bufPrint(&filepath_buf, "incremental-graph-crash-dump.{d}.html", .{bun.timestamp()}) catch "incremental-graph-crash-dump.html";
...
pub fn readString32(reader: *std.Io.Reader, alloc: Allocator) ![]const u8 {
...
const ManagedArrayListWriter = bun.io.GenericWriter(*std.array_list.Managed(u8), bun.OOM, writeManagedArrayList);

fn writeManagedArrayList(list: *std.array_list.Managed(u8), bytes: []const u8) bun.OOM!usize {
    try list.appendSlice(bytes);
    return bytes.len;
}

pub fn managedArrayListWriter(list: *std.array_list.Managed(u8)) ManagedArrayListWriter {
    return .{ .context = list };
}

const UnmanagedArrayListWriterContext = struct {
    list: *std.ArrayListUnmanaged(u8),
    allocator: Allocator,
};
const UnmanagedArrayListWriter = bun.io.GenericWriter(UnmanagedArrayListWriterContext, bun.OOM, writeUnmanagedArrayList);

fn writeUnmanagedArrayList(context: UnmanagedArrayListWriterContext, bytes: []const u8) bun.OOM!usize {
    try context.list.appendSlice(context.allocator, bytes);
    return bytes.len;
}

fn unmanagedArrayListWriter(list: *std.ArrayListUnmanaged(u8), alloc: Allocator) UnmanagedArrayListWriter {
    return .{ .context = .{ .list = list, .allocator = alloc } };
}
