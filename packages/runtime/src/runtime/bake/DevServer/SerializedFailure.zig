...
pub fn initFromLog(
    ...
    var payload = std.Io.Writer.Allocating.initCapacity(sfb.get(), 65536) catch
    ...
    const data = if (written.ptr == &sfb.buffer)
        try dev.allocator().dupe(u8, written)
    else
        payload.toArrayList().items;
