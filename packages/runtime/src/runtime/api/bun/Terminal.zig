// Ported from Bun terminal.zig, checked against terminal.rs at
// 4982b91e3702094330f3be3883354c52b8c01323. MIT — see ../../../cli/LICENSE.bun.md.
...
//!
//! Lifecycle:
//! - Holds a strong JSRef while initially reading; upgrades again on new data
...
/// State flags
flags: Flags = .{},
waiting_for_drain: bool = false,

// Each PTY owns its cooked-mode snapshot; changing one terminal must not
// reuse or restore another terminal's attributes via the process-wide TTY helper.
saved_termios: if (Environment.isPosix) ?Termios else void = if (Environment.isPosix) null else {},
...
pub const Options = struct {
    ...
    pub fn parseFromJS(globalObject: *jsc.JSGlobalObject, js_options: JSValue) bun.JSError!Options {
        ...
        if (try js_options.getOptional(globalObject, "name", jsc.ZigString.Slice)) |slice| {
            if (slice.len > Options.max_term_name_len) {
                slice.deinit();
                return globalObject.throw("Terminal name too long (max {d} characters)", .{Options.max_term_name_len});
            }
...
pub const max_term_name_len = Options.max_term_name_len;
pub const CreateError = InitError;
...
fn initTerminal(
    ...
    // POSIX start may synchronously call onReaderError while registering.
    ...
    switch (terminal.reader.start(pty_result.read_fd, true)) {
        .err => {
            // Windows has not adopted read_fd. POSIX owns it through the
            // reader handle and its pre-acquired ref; closeInternal releases it.
            // Finally drop the initial reference, since no wrapper was created.
            if (comptime !Environment.isPosix) {
                terminal.read_fd.close();
                terminal.read_fd = bun.invalid_fd;
            }
        ...
        .result => {
            if (comptime !Environment.isPosix) terminal.ref();
            if (terminal.flags.reader_done) {
                terminal.closeInternal();
                terminal.deref();
                return error.ReaderStartFailed;
            }
    ...
    // Register wrapper/callback ownership before any synchronous read callback.
...
const OpenPtyTermios = if (Environment.isPosix) std.posix.termios else opaque {};
...
fn createPtyPosix(cols: u16, rows: u16) CreatePtyError!PtyResult {
    ...
    if (getTermios(slave_fd_desc)) |termios| {
        var t = termios;
...
fn setTermiosFlag(this: *Terminal, globalObject: *jsc.JSGlobalObject, comptime field: enum { iflag, oflag, lflag, cflag }, value: JSValue) bun.JSError!void {
    ...
    // On Darwin tcflag_t is u64: f64(maxInt(u64)) rounds up to 2^64.
    // Saturate before conversion, including Infinity and NaN (min selects max).
    const bits: Int = if (clamped >= max_val) std.math.maxInt(Int) else @intFromFloat(clamped);
    @field(termios_data, @tagName(field)) = @bitCast(bits);
...
pub fn write(
    ...
    if (Environment.isPosix and write_result == .pending) this.waiting_for_drain = true;
...
pub fn setRawMode(
    ...
    if (comptime Environment.isPosix) {
        if (enabled != this.flags.raw_mode) {
            var term = if (enabled) getTermios(this.master_fd) else this.saved_termios;
            if (term) |*next| {
                const previous = next.*;
                if (enabled) {
                    ...
                    next.oflag.ONLCR = true;
                    next.cflag.CSIZE = .CS8;
                    next.lflag.ECHO = false;
                    next.lflag.ICANON = false;
                    next.lflag.IEXTEN = false;
                    next.lflag.ISIG = false;
                    next.cc[@backingInt(std.posix.V.MIN)] = 1;
                    next.cc[@backingInt(std.posix.V.TIME)] = 0;
                }
                if (!setTermios(this.master_fd, next)) {
                    return globalObject.throw("Failed to set raw mode", .{});
                }
                this.saved_termios = if (enabled) previous else null;
            } else {
                return globalObject.throw("Failed to set raw mode", .{});
            }
        }
...
fn getTermios(fd: bun.FD) ?Termios {
    ...
    while (true) {
        ...
        switch (std.c.errno(std.posix.system.tcgetattr(@intCast(fd.cast()), &term))) {
            .SUCCESS => return term,
            .INTR => continue,
            else => return null,
        }
...
fn setTermios(fd: bun.FD, termios_p: *const Termios) bool {
    ...
    while (true) {
        switch (std.c.errno(std.posix.system.tcsetattr(@intCast(fd.cast()), .NOW, termios_p))) {
            .SUCCESS => return true,
            .INTR => continue,
            else => return false,
        }
...
fn onWrite(this: *Terminal, amount: usize, status: bun.io.WriteStatus) void {
    ...
    // StreamingWriter's POSIX poll dispatch reports completion through onWrite.
    // Only notify after a write encountered backpressure and the buffer drained.
    if (Environment.isPosix and status == .drained and this.waiting_for_drain and !this.writer.hasPendingData()) {
...
pub fn onReaderDone(this: *Terminal) void {
    log("onReaderDone", .{});
    if (this.flags.reader_done) return;
    this.flags.reader_done = true;
    // Keep the reader's reference until re-entrant JavaScript returns.
    defer this.deref();
