...
// Re-attached 2026-06-23: home_rt now exposes the parse pipeline
// (interchange.toml.TOML, ast.ASTMemoryAllocator, js_printer.printJSON,
// String.toJSByParseJSON) + the JSC bridge, so restore the real Bun.TOML.parse.
// Previously a .zero-returning stub that segfaulted JSC on `Bun.TOML` access.
