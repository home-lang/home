// Copied from bun/src/threading/Mutex.zig at upstream SHA
// fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../cli/LICENSE.bun.md.
// Imports rewritten: @import("bun") → @import("home").
// Rewrites:
//   * `bun.Futex` → local `Futex = @import("./Futex.zig")`.
//   * `bun.assert` → `home_rt.assert`.
//   * Exported C-ABI symbols (`Bun__lock`, `Bun__unlock`, `Bun__lock__size`)
//     stay verbatim per the runtime IP convention (extern fn names are not
//     rewritten).
//
//! This is a copy-pasta of std.Thread.Mutex with some changes.
//! - No assert with unreachable
//! - uses home_rt.threading.Futex instead of std.Thread.Futex
...
// Note: `Bun__`-prefixed extern symbols stay verbatim — the C/C++ FFI ABI
// surface is what these export to. Renaming them would silently break
// callers on the native side.
...
test "Mutex: tryLock succeeds on uncontended mutex" {
    var m: Mutex = .{};
    try std.testing.expect(m.tryLock());
    m.unlock();
}

test "Mutex: tryLock fails on contended mutex" {
    var m: Mutex = .{};
    m.lock();
    defer m.unlock();
    try std.testing.expect(!m.tryLock());
