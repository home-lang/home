...
//     behind a `comptime` guard. `Futex.waitForever` is the only caller used
//     by the ported Mutex/Condition, so leaving Deadline parked is safe for
//     the wider runtime. Re-attach when the perf/system_timer adapter lands.
//
...
const LinuxImpl = struct {
    fn wait(ptr: *const atomic.Value(u32), expect: u32, timeout: ?u64) error{Timeout}!void {
        ...
        switch (linux.E.init(rc)) {
            ...
            else => |err| std.debug.panic("Unexpected futex_wait() return code: {d} - {s}", .{ rc, std.enums.tagName(linux.E, err) orelse "unknown" }),
        }
    }
...
// `Deadline` (upstream) is parked: it requires `std.time.Timer`, which Zig
// 0.17.0-dev removed (same gap that parked `perf/system_timer.zig`). The
// ported Mutex / Condition pair only ever calls `Futex.waitForever`, so
// nothing in the current threading subtree depends on Deadline. Re-attach
// when a `std.Io.Clock`-style adapter lands in home_rt.

test "Futex: zero-timeout returns Timeout when value still matches" {
    var v: atomic.Value(u32) = .init(7);
    try std.testing.expectError(error.Timeout, Futex.wait(&v, 7, 0));
}

test "Futex: zero-timeout returns OK when value already differs" {
    var v: atomic.Value(u32) = .init(7);
    try Futex.wait(&v, 99, 0);
}

test "Futex: wake with 0 waiters is a no-op" {
    var v: atomic.Value(u32) = .init(0);
    Futex.wake(&v, 0); // must not call into the OS
}
