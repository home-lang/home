// Copied from bun/src/analytics/schema.zig at upstream
// SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../cli/LICENSE.bun.md.
//
// Zig 0.17 compat adjustments (no behavior change):
//   - `@typeInfo(T).Enum` → `@typeInfo(T).@"enum"`
//   - `@typeInfo(T).Struct` → `@typeInfo(T).@"struct"`
//   - `Struct.layout == .Packed` → `Struct.layout == .@"packed"`
//   - `std.mem.readIntSliceNative` → `std.mem.readInt(T, slice[0..@sizeOf(T)], native_endian)`
// `ByteWriter` (which used `std.io.FixedBufferStream`, removed in 0.17)
// is parked behind a comment — a `std.Io.Writer.fixed`-based shim will
// re-attach when the analytics encoder rejoins the build.
//
...
pub const Reader = struct {
    ...
    pub fn readArray(this: *Self, comptime T: type) ![]const T {
        ...
        switch (T) {
            ...
            u16, u32, i8, i16, i32 => {
                const bytes = try this.read(length * @sizeOf(T));
    ...
    pub inline fn readInt(this: *Self, comptime T: type) !T {
        ...
        return std.mem.readInt(T, slice[0..@sizeOf(T)], native_endian);
    }
                ...
                switch (comptime @typeInfo(T)) {
                    .@"struct" => |Struct| {
                        switch (Struct.layout) {
                            .@"packed" => {
                                const sizeof = @sizeOf(T);
                                const slice = try this.read(sizeof);
...
// `ByteWriter = Writer(*std.io.FixedBufferStream([]u8))` was the upstream
// alias. `std.io.FixedBufferStream` was removed in Zig 0.17 (replaced by
// `std.Io.Writer.fixed`). Re-attaching requires a small writer-adapter
// struct; parked until the analytics encoder rejoins the build.
//
// `FileWriter = Writer(std.fs.File)` likewise depends on the old
// `std.fs.File.write` shape and is parked alongside.
...
test "schema: OperatingSystem tag names round-trip" {
    try std.testing.expectEqualStrings("linux", @tagName(analytics.OperatingSystem.linux));
    try std.testing.expectEqualStrings("macos", @tagName(analytics.OperatingSystem.macos));
    try std.testing.expectEqualStrings("windows", @tagName(analytics.OperatingSystem.windows));
}

test "schema: Architecture tags" {
    try std.testing.expectEqual(@as(u8, 1), @intFromEnum(analytics.Architecture.x64));
    try std.testing.expectEqual(@as(u8, 2), @intFromEnum(analytics.Architecture.arm));
}

test "schema: EventKind values match the wire layout" {
    try std.testing.expectEqual(@as(u32, 1), @intFromEnum(analytics.EventKind.bundle_success));
    try std.testing.expectEqual(@as(u32, 2), @intFromEnum(analytics.EventKind.bundle_fail));
    try std.testing.expectEqual(@as(u32, 5), @intFromEnum(analytics.EventKind.bundle_start));
}

test "schema: Reader.readByte advances the cursor" {
    var buf = [_]u8{ 0xAB, 0xCD };
    var r = Reader.init(&buf, std.testing.allocator);
    try std.testing.expectEqual(@as(u8, 0xAB), try r.readByte());
    try std.testing.expectEqual(@as(u8, 0xCD), try r.readByte());
    try std.testing.expectError(error.EOF, r.readByte());
}

test "schema: Reader.readInt little-endian native round-trip" {
    var buf: [4]u8 = undefined;
    const v: u32 = 0xDEADBEEF;
    std.mem.writeInt(u32, &buf, v, native_endian);
    var r = Reader.init(&buf, std.testing.allocator);
    try std.testing.expectEqual(v, try r.readInt(u32));
}

test "schema: Reader.readEnum decodes EventKind" {
    var buf: [4]u8 = undefined;
    std.mem.writeInt(u32, &buf, @intFromEnum(analytics.EventKind.bundle_success), native_endian);
    var r = Reader.init(&buf, std.testing.allocator);
    try std.testing.expectEqual(analytics.EventKind.bundle_success, try r.readEnum(analytics.EventKind));
}
