// Copied from bun/src/exe_format/pe.zig at upstream
// SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../cli/LICENSE.bun.md.
//
// Pure-Zig PE/COFF parser used by `bun build --compile` on Windows to embed
// the standalone module graph into a `.bun` section of the executable.
// Has no `bun.X` runtime dependencies — the upstream `@import("bun")` was
// stale and is omitted here. The C++ extern declarations at the bottom
// keep their `Bun__` names per the porting convention.
//
...
test "PEFile.BUN_COMPILED_SECTION_NAME matches the .bun section literal" {
    try std.testing.expectEqualStrings(".bun", BUN_COMPILED_SECTION_NAME);
}

test "PEFile error set contains the documented PE-specific codes" {
    // Sanity check the error set produced by init/strip/section ops is
    // reachable from outside the struct (catches accidental private err
    // sets after future refactors). Each name must round-trip via
    // @errorName on the dynamic value.
    const e1: anyerror = error.InvalidPEFile;
    const e2: anyerror = error.BunSectionNotFound;
    const e3: anyerror = error.InvalidDOSSignature;
    try std.testing.expectEqualStrings("InvalidPEFile", @errorName(e1));
