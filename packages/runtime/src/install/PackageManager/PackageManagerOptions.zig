...
pub fn openGlobalDir(explicit_global_dir: string) !std.Io.Dir {
    if (bun.env_var.BUN_INSTALL_GLOBAL_DIR.get()) |home_dir| {
        return try bun.MakePath.makeOpenPath(std.Io.Dir.cwd(), home_dir, .{});
    }

    if (explicit_global_dir.len > 0) {
        return try bun.MakePath.makeOpenPath(std.Io.Dir.cwd(), explicit_global_dir, .{});
    }
    ...
    if (bun.env_var.BUN_INSTALL.get()) |home_dir| {
        ...
        return try bun.MakePath.makeOpenPath(std.Io.Dir.cwd(), path, .{});
    }
