...
pub fn NewParser_(
    ...
    return struct {
        ...
        reported_stack_overflow: bool = false,
        ...
        pub fn reportStackOverflow(p: *P, loc: logger.Loc) void {
            if (p.reported_stack_overflow) return;
            p.reported_stack_overflow = true;
            bun.handleOom(p.log.addError(p.source, loc, "Maximum call stack size exceeded"));
        }
        ...
        pub fn pushScopeForVisitPass(noalias p: *P, kind: js_ast.Scope.Kind, loc: logger.Loc) anyerror!void {
            if (p.reported_stack_overflow) {
                while (p.scope_order_to_visit.len > 0) {
                    const head = p.scope_order_to_visit[0];
                    if (head.loc.start == loc.start and head.scope.kind == kind) break;
