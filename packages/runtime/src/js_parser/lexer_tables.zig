...
// Upstream:  packages/runtime/upstream/src/js_parser/lexer_tables.zig
...
//
// Pure-data lexer tables: the `T` token enum + `Keywords`,
// `StrictModeReservedWords`(`Remap`), `PropertyModifierKeyword`,
// `TypeScriptAccessibilityModifier`, `tokenToString` and the
// `jsxEntity` HTML-named-entity table. Only depends on
// `home_rt.ComptimeStringMap` and `std`.
...
// Upstream defines `pub const CodePoint = i32;` in `src/strings.zig`.
// `home_rt.strings` hasn't surfaced it yet, so mirror the alias locally.
const CodePoint = i32;

test "T.isAssign covers the assignment block exactly" {
    // Every assignment token reports true.
    try std.testing.expect(T.t_ampersand_ampersand_equals.isAssign());
    try std.testing.expect(T.t_equals.isAssign());
    try std.testing.expect(T.t_slash_equals.isAssign());
    try std.testing.expect(T.t_question_question_equals.isAssign());

    // Tokens just outside the range report false.
    try std.testing.expect(!T.t_tilde.isAssign()); // last before assignments
    try std.testing.expect(!T.t_private_identifier.isAssign()); // first after
    try std.testing.expect(!T.t_plus.isAssign());
    try std.testing.expect(!T.t_break.isAssign());
}

test "T.isReservedWord covers the reserved-word block" {
    try std.testing.expect(T.t_break.isReservedWord());
    try std.testing.expect(T.t_with.isReservedWord());
    try std.testing.expect(T.t_typeof.isReservedWord());
    try std.testing.expect(!T.t_identifier.isReservedWord());
    try std.testing.expect(!T.t_escaped_keyword.isReservedWord());
    try std.testing.expect(!T.t_plus.isReservedWord());
}

test "T.isString matches template + plain string literals" {
    try std.testing.expect(T.t_string_literal.isString());
    try std.testing.expect(T.t_no_substitution_template_literal.isString());
    try std.testing.expect(T.t_template_head.isString());
    try std.testing.expect(T.t_template_middle.isString());
    try std.testing.expect(T.t_template_tail.isString());
    try std.testing.expect(!T.t_numeric_literal.isString());
    try std.testing.expect(!T.t_identifier.isString());
}

test "T.isCloseBraceOrEOF only fires for `}` / EOF" {
    try std.testing.expect(T.t_end_of_file.isCloseBraceOrEOF());
    try std.testing.expect(T.t_close_brace.isCloseBraceOrEOF());
    try std.testing.expect(!T.t_syntax_error.isCloseBraceOrEOF());
    try std.testing.expect(!T.t_open_brace.isCloseBraceOrEOF());
}

test "Keywords / StrictModeReservedWords map JS reserved words" {
    try std.testing.expectEqual(@as(?T, .t_function), Keywords.get("function"));
    try std.testing.expectEqual(@as(?T, .t_return), Keywords.get("return"));
    try std.testing.expectEqual(@as(?T, .t_typeof), Keywords.get("typeof"));
    try std.testing.expectEqual(@as(?T, null), Keywords.get("yield")); // strict-only

    try std.testing.expect(StrictModeReservedWords.has("yield"));
    try std.testing.expect(StrictModeReservedWords.has("let"));
    try std.testing.expect(!StrictModeReservedWords.has("return"));

    try std.testing.expectEqualStrings(
        "_yield",
        StrictModeReservedWordsRemap.get("yield").?,
    );
}

test "PropertyModifierKeyword / TypeScriptAccessibilityModifier maps" {
    try std.testing.expectEqual(
        @as(?PropertyModifierKeyword, .p_async),
        PropertyModifierKeyword.List.get("async"),
    );
    try std.testing.expectEqual(
        @as(?PropertyModifierKeyword, .p_readonly),
        PropertyModifierKeyword.List.get("readonly"),
    );
    try std.testing.expectEqual(
        @as(?PropertyModifierKeyword, null),
        PropertyModifierKeyword.List.get("definitely-not-a-modifier"),
    );

    try std.testing.expect(TypeScriptAccessibilityModifier.has("public"));
    try std.testing.expect(TypeScriptAccessibilityModifier.has("readonly"));
    try std.testing.expect(!TypeScriptAccessibilityModifier.has("async"));
}

test "TypescriptStmtKeyword.List maps TS-only statement leads" {
    try std.testing.expectEqual(
        @as(?TypescriptStmtKeyword, .ts_stmt_type),
        TypescriptStmtKeyword.List.get("type"),
    );
    try std.testing.expectEqual(
        @as(?TypescriptStmtKeyword, .ts_stmt_namespace),
        TypescriptStmtKeyword.List.get("namespace"),
    );
    try std.testing.expectEqual(
        @as(?TypescriptStmtKeyword, .ts_stmt_declare),
        TypescriptStmtKeyword.List.get("declare"),
    );
    try std.testing.expectEqual(
        @as(?TypescriptStmtKeyword, null),
        TypescriptStmtKeyword.List.get("function"),
    );
}

test "tokenToString table covers a few representative tokens" {
    try std.testing.expectEqualStrings("end of file", tokenToString.get(.t_end_of_file));
    try std.testing.expectEqualStrings("identifier", tokenToString.get(.t_identifier));
    try std.testing.expectEqualStrings("\"return\"", tokenToString.get(.t_return));
    try std.testing.expectEqualStrings("\"+=\"", tokenToString.get(.t_plus_equals));
    try std.testing.expectEqualStrings("\"...\"", tokenToString.get(.t_dot_dot_dot));
}

test "jsxEntity decodes HTML named entities to code points" {
    try std.testing.expectEqual(@as(?CodePoint, 0x00C1), jsxEntity.get("Aacute"));
    try std.testing.expectEqual(@as(?CodePoint, 0x0026), jsxEntity.get("amp"));
    try std.testing.expectEqual(@as(?CodePoint, 0x003C), jsxEntity.get("lt"));
    try std.testing.expectEqual(@as(?CodePoint, 0x003E), jsxEntity.get("gt"));
    try std.testing.expectEqual(@as(?CodePoint, 0x0027), jsxEntity.get("apos"));
    try std.testing.expectEqual(@as(?CodePoint, 0x00A0), jsxEntity.get("nbsp"));
    try std.testing.expectEqual(@as(?CodePoint, 0x20AC), jsxEntity.get("euro"));
    try std.testing.expectEqual(@as(?CodePoint, null), jsxEntity.get("notarealentity"));
}
