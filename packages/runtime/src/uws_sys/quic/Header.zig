...
pub const Qpack = enum(u8) {
    ...
    pub fn classify(name: []const u8) ?Class {
        // home_rt.ComptimeStringMap doesn't currently expose
        // `.getAnyCase` (upstream Bun has it via the JSC-stripped
        // copy). Until the case-insensitive variant lands, we fall
        // back to a strict-lowercase lookup — callers must lowercase
        // before calling. Tracked under `getAnyCase` in
        // src/cli/PORTING_STATUS.md.
        return map.get(name);
    }

    const map = home_rt.ComptimeStringMap(Class, .{
...
test "Qpack.classify recognises forbidden headers" {
    const std = @import("std");
    const c = Qpack.classify("connection").?;
    try std.testing.expect(c == .forbidden);
    const h = Qpack.classify("host").?;
    try std.testing.expect(h == .host);
}

test "Qpack.classify maps known names to their static-table index" {
    const std = @import("std");
    const c = Qpack.classify("content-type").?;
    try std.testing.expect(c == .indexed);
    try std.testing.expectEqualStrings("content-type", c.indexed.name);
    try std.testing.expectEqual(Qpack.content_type, c.indexed.index);
}

test "Qpack.classify returns null for unknown headers" {
    const std = @import("std");
