// Copied from bun/src/uws_sys/SocketGroup.zig at upstream
// SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../cli/LICENSE.bun.md.
//
// Imports rewritten: upstream resolves `Loop` / `SslCtx` / `us_socket_t` /
// `ListenSocket` / `us_bun_verify_error_t` / `LIBUS_SOCKET_DESCRIPTOR` via
// `bun.uws`. Home's uws_sys aggregator only exposes the QUIC subtree +
// SocketKind + ConnectingSocket today; the still-unported deps are
// forward-declared as local opaques (same pattern as ConnectingSocket.zig
// uses for `SocketGroup` and `Loop`). `ConnectingSocket` is imported as a
// sibling. `LIBUS_SOCKET_DESCRIPTOR` is a `c_int` on POSIX (`uintptr_t` on
// Windows) — Home is POSIX-only for now, so we alias to `c_int` here. The
// VTable layout is the source of truth (asserted via `comptime`), and
// matches `uws_sys/vtable.zig`'s mirror; once the broader uws surface
// lands, the two converge on the same type.
...
pub const SocketGroup = extern struct {
    ...
    /// null); never read this field directly. Upstream spells this `#ext`
    /// (newer Zig private-field syntax); Home's Zig 0.17 pre-release doesn't
    /// accept the `#` prefix yet, so we keep the field name plain and rely on
    /// the doc-comment + the `owner(T)` accessor to keep callers honest.
    ext_owner: ?*anyopaque = null,
    ...
    pub fn owner(self: *const SocketGroup, comptime T: type) *T {
        return @ptrCast(@alignCast(self.ext_owner.?));
    }
    ...
    pub fn connect(
        ...
        // ABI (context.c / libusockets.h): us_socket_group_connect(
        //   group, kind, ssl_ctx, host, port, options, socket_ext_size,
        //   is_connecting) — 8 params, is_connecting is `nonnull`.
        // An earlier version inserted two extra "local bind address" args
        // (`null, 0`) before `options`, believing a newer bun-usockets ABI.
        // Home's vendored bun-usockets has no such params, so the extra args
        // shifted the C side: `is_connecting` received the `options` value (0)
        // as a pointer and `*is_connecting = 1` wrote to NULL — a segfault on
        // connect() to an IP literal (e.g. net.connect(port) to 127.0.0.1).
...
/// lands (mirrors the upstream `if (Environment.isWindows)` switch).
pub const LIBUS_SOCKET_DESCRIPTOR = c_int;

test "SocketGroup layout matches us_socket_group_t" {
    const std = @import("std");
    // The `comptime` block above already asserts this, but mirroring it as a
    // test surfaces the constants in `zig test` output and catches drift if
    // someone disables UBSAN or runs under a different ABI.
    try std.testing.expectEqual(@as(usize, 9 * @sizeOf(*anyopaque) + 16), @sizeOf(SocketGroup));
    try std.testing.expectEqual(@as(usize, 11 * @sizeOf(*anyopaque)), @sizeOf(SocketGroup.VTable));
}

test "SocketGroup zero-initialises to an empty group" {
    const std = @import("std");
    const g: SocketGroup = .{};
    try std.testing.expect(g.isEmpty());
    try std.testing.expect(g.loop == null);
    try std.testing.expect(g.vtable == null);
    try std.testing.expectEqual(@as(u32, 0), g.global_tick);
    try std.testing.expectEqual(@as(u16, 0), g.low_prio_count);
    try std.testing.expectEqual(@as(u8, 0), g.linked);
}

test "SocketGroup.ConnectResult is a tagged union of the two success paths" {
    const std = @import("std");
    // Compile-time sanity — `connect` builds these from extern returns; the
...
test "bun-usockets connect declaration matches the C ABI (8 params)" {
    const std = @import("std");
    // Must match us_socket_group_connect in context.c / libusockets.h:
    //   (group, kind, ssl_ctx, host, port, options, socket_ext_size, is_connecting)
    // A wrong arg count shifts the C params, landing `is_connecting` on an int
    // value; the C side then writes `*is_connecting = 1` through a bogus
    // pointer → segfault on connect() to an IP literal.
    const fn_info = @typeInfo(@TypeOf(c.us_socket_group_connect)).@"fn";
    try std.testing.expectEqual(@as(usize, 8), fn_info.param_types.len);
    try std.testing.expect(fn_info.param_types[7].? == *c_int);
}
