...
test "bun-usockets connect declaration matches the local-bind C ABI" {
    const std = @import("std");
    // Must match us_socket_group_connect in context.c / libusockets.h:
    //   (group, kind, ssl_ctx, host, port, local_host, local_port,
    //    options, socket_ext_size, is_connecting)
