// Copied verbatim bun/src/runtime/node/nodejs_error_code.zig at upstream SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../cli/LICENSE.bun.md.
...
test "Code: canonical Node.js error codes are distinct values" {
    try std.testing.expect(@intFromEnum(Code.ABORT_ERR) != @intFromEnum(Code.ERR_ASSERTION));
    try std.testing.expect(@intFromEnum(Code.ERR_INVALID_ARG_TYPE) != @intFromEnum(Code.ERR_INVALID_ARG_VALUE));
}

test "Code: @tagName round-trips canonical codes" {
    try std.testing.expectEqualStrings("ABORT_ERR", @tagName(Code.ABORT_ERR));
    try std.testing.expectEqualStrings("ERR_ASSERTION", @tagName(Code.ERR_ASSERTION));
    try std.testing.expectEqualStrings("ERR_INVALID_ARG_TYPE", @tagName(Code.ERR_INVALID_ARG_TYPE));
}

test "Code: std.meta.stringToEnum resolves Node codes" {
    try std.testing.expectEqual(Code.ERR_ASSERTION, std.meta.stringToEnum(Code, "ERR_ASSERTION").?);
    try std.testing.expectEqual(@as(?Code, null), std.meta.stringToEnum(Code, "NOT_A_REAL_CODE"));
}
