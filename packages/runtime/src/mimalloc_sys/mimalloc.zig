// Copied verbatim from bun/src/mimalloc_sys/mimalloc.zig at upstream
// SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../cli/LICENSE.bun.md.
//
// Pure FFI extern declarations for the vendored mimalloc allocator.
// `mi_*` extern symbols stay verbatim — they are the C ABI surface the
// linked `libmimalloc.a` exposes and must not be renamed.
...
test "mimalloc extern symbol signatures compile" {
    // Sanity check: ensure the extern declarations compile without referencing
    // the actual libmimalloc symbols (which only resolve at link time).
    _ = @typeName(@TypeOf(mi_malloc));
    _ = @typeName(@TypeOf(mi_free));
    _ = @typeName(@TypeOf(mi_heap_new));
    _ = @typeName(@TypeOf(mi_option_set));
    try std.testing.expectEqual(@as(c_int, 128), MI_SMALL_WSIZE_MAX);
    try std.testing.expectEqual(@as(usize, 16), MI_MAX_ALIGN_SIZE);
    try std.testing.expectEqual(@as(c_uint, 0), @intFromEnum(Option.show_errors));
}
