// Copied verbatim from bun/src/unicode/uucode/lut.zig at upstream SHA fd0b6f1a271fca0b8124b69f230b100f4d636af6. MIT — see ../../cli/LICENSE.bun.md.
...
test "lut.Generator builds a 3-level table that round-trips a constant mapping" {
    // A trivial Context that maps every codepoint to the same value. The
    // resulting tables should compress to a single block at every stage.
    const Elem = u8;
    const Context = struct {
        pub fn get(_: @This(), _: u21) !Elem {
            return 7;
        }
        pub fn eql(_: @This(), a: Elem, b: Elem) bool {
            return a == b;
        }
    };

    const gen: Generator(Elem, Context) = .{};
    const t = try gen.generate(std.testing.allocator);
    defer std.testing.allocator.free(t.stage1);
    defer std.testing.allocator.free(t.stage2);
    defer std.testing.allocator.free(t.stage3);

    const tables: Tables(Elem) = .{
        .stage1 = t.stage1,
        .stage2 = t.stage2,
        .stage3 = t.stage3,
    };

    // Spot-check several codepoints: ASCII boundary, BMP mid, supplementary.
    try std.testing.expectEqual(@as(Elem, 7), tables.get(0));
    try std.testing.expectEqual(@as(Elem, 7), tables.get(0x7F));
    try std.testing.expectEqual(@as(Elem, 7), tables.get(0x4E2D));
    try std.testing.expectEqual(@as(Elem, 7), tables.get(0x1F600));
    try std.testing.expectEqual(@as(Elem, 7), tables.get(std.math.maxInt(u21)));

    // Constant mapping must dedupe stage3 down to a single entry.
    try std.testing.expectEqual(@as(usize, 1), t.stage3.len);
}
