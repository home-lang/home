...
- `home test packages/runtime/test/bun-corpus/` is the full acceptance gate and
  ...
  Latest measured subset run: `157` files, `681` passed, `0` failed,
  ...
  including POSIX `pathToFileURL`, Bun file URL helper conversion, path `toNamespacedPath` / `_makeLong`, and relative CJS fixture smokes, a narrow inline-snapshot Unicode object
