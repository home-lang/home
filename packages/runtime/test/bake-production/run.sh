...
BUN_FEATURE_FLAG_EXPERIMENTAL_BAKE=1 BUN_DEV_SERVER_TEST_RUNNER=1 "$home_exe" build --app ./app.ts --outdir ./dist >build.log 2>&1
...
if BUN_FEATURE_FLAG_EXPERIMENTAL_BAKE=1 BUN_DEV_SERVER_TEST_RUNNER=1 "$home_exe" build --app ./missing.ts >missing.log 2>&1; then
