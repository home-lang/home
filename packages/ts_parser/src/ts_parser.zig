        ...
        if (self.peek().kind != .close_paren) {
            while (true) {
                ...
                if (self.peek().kind == .question) {
                    ...
                    // TS1047: `...rest?` — a rest parameter cannot also
                    // carry the `?` optional marker. Anchored at the
                    // `?` token to match upstream tsc's column for
                    // `parserParameterList9.ts(2,14)` and
                    // `parserParameterList11.ts(1,8)`.
                    if (flags.is_rest) {
                ...
                if (self.match(.equal)) {
                    ...
                    // TS1048: `...rest = init` — a rest parameter cannot
