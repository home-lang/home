...
pub const Expr = union(NodeType) {
    ...
    TupleStructLiteral: *TupleStructLiteral,
    ...
    pub fn getLocation(self: Expr) SourceLocation {
        return switch (self) {
            ...
            .TupleStructLiteral => |expr| expr.node.loc,
            .AnonymousStruct => |expr| expr.node.loc,
            .ArrayComprehension => |expr| expr.node.loc,
            .DictComprehension => |expr| expr.node.loc,
            .SetComprehension => |expr| expr.node.loc,
            .NestedComprehension => |expr| expr.node.loc,
            .GeneratorExpr => |expr| expr.node.loc,
            .SplatExpr => |expr| expr.node.loc,
            .ArrayDestructuring => |expr| expr.node.loc,
            .ObjectDestructuring => |expr| expr.node.loc,
            .DispatchCall => |expr| expr.node.loc,
            .IsExpr => |expr| expr.node.loc,
            .ExtendDecl => |expr| expr.node.loc,
