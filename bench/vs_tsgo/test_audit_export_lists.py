...
class ExportListAuditTests(unittest.TestCase):
    def test_declarations_placements_orders_and_controls(self):
        ...
        self.assertEqual(128, len({(case.family, case.name) for case in cases}))
    ...
    def test_each_negative_only_appends_to_the_same_program(self):
        ...
        for start in range(0, len(cases), 4):
            ...
            for negative in cases[start + 1:start + 4]:
                ...
                for path, source in positive.files.items():
                    ...
                    elif negative.name.endswith("/renamed-hidden") and path == "a-owner.ts":
                        self.assertEqual(source + "export { hidden as public };\n", negative.files[path])
                    else:
    ...
    def test_root_order_preserves_source_and_expected_errors(self):
        ...
        for start in range(0, len(cases), 8):
            for before, after in zip(cases[start:start + 4], cases[start + 4:start + 8]):
