...
> TS diagnostic-code coverage (634 / 2,076 emitted) tracks the catalog-
...
| **TypeScript — diagnostic codes emitted** | **634 / 2,076 — ~30.4%** | `docs/TS_DIAGNOSTIC_CODE_STATUS.md` — codes referenced from production source; 1,438 catalog-only remain |
...
| **Total test count** | **7,021 / 7,023 — ~100%** (2 skipped, 0 failed) | `./pantry/.bin/zig build test --summary all` on Zig 0.17.0-dev — full unit + integration + conformance pin suite |
...
| TS diagnostic-code catalogue | **634 / 2,076 emitted — ~30.4%** | Mirrors the full upstream code → message table; powers `home-lsp` hover-on-`TS1234`. 1,438 catalog-only entries remain (see `docs/TS_DIAGNOSTIC_CODE_STATUS.md`). |
