...
> TS diagnostic-code coverage (628 / 2,076 emitted) tracks the catalog-
...
| **TypeScript — diagnostic codes emitted** | **628 / 2,076 — ~30.3%** | `docs/TS_DIAGNOSTIC_CODE_STATUS.md` — codes referenced from production source; 1,444 catalog-only remain |
...
| **Total test count** | **7,015 / 7,017 — ~100%** (2 skipped, 0 failed) | `./pantry/.bin/zig build test --summary all` on Zig 0.17.0-dev — full unit + integration + conformance pin suite |
...
| TS diagnostic-code catalogue | **628 / 2,076 emitted — ~30.3%** | Mirrors the full upstream code → message table; powers `home-lsp` hover-on-`TS1234`. 1,444 catalog-only entries remain (see `docs/TS_DIAGNOSTIC_CODE_STATUS.md`). |
