...
pub fn join(comptime T: type, futures: []Future(T)) ![]T {
    var results = std.ArrayList(T){};
    defer results.deinit(undefined); // Will be owned by caller

    for (futures) |*future| {
        while (true) {
            if (try future.poll()) |value| {
                try results.append(undefined, value);
                break;
            }
    ...
    return results.toOwnedSlice(undefined);
}
