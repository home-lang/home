...
1. **Don't chase a fixture that no longer breaks.** Confirm independently on a clean build (`./pantry/.bin/zig build test -Dfilter=ts_conformance --summary all` — should report 45/45 in ~30 s) that the leak is truly gone.
2. **Re-run the bisect harness** (`HOME_TS_CONFORMANCE_BISECT=1 HOME_TS_CONFORMANCE_BISECT_LIMIT=2000 ./pantry/.bin/zig build test -Dfilter=ts_conformance`) before declaring the §3.A follow-up closed. If it stays green, retire the NOTE block at `ts_conformance.zig:2844` and the §3.A entry in `TS_PARITY_PLAN.md`.
