...
### Remaining queued-task ownership regression

`tests/runtime/worker-queued-transfer-close.pending.mjs` intentionally fails on [#465](https://github.com/home-lang/home/issues/465); it is not a skipped or passing test. One worker blocks a timer callback for a finite interval. The parent queues one transferred port and terminates the worker before delivery. The surviving peer never closes, while the normal-delivery positive control does. This small bounded test demonstrates stranded native ownership without large allocations or an RSS threshold. Fixing it requires safe task admission/cancellation and thread-affine cleanup before heap teardown; deleting arbitrary queued callbacks is insufficient.

The linked Worker implementation is still external (`UnifiedSource-src_jsc_bindings_webcore-5.cpp`). Its `dispatchExit` already releases the create-time C++ reference through the pre-enqueue hook; the corresponding Home mirrored source is stale. That mirror discrepancy must not be misreported as the runtime cause of #465. Callback-only parent-poll cleanup and thread-affine payload cleanup still need consideration in a future cancellation implementation.

### Pending-close verification checkpoint

The build and unchanged rebuild pass **13/13 steps**. All **16 native regression files** pass. The new pending-close suite passes **15 cases** on three fresh Home runs and three Node 24.18.0 control runs. The prior 13-case close suite, main/worker message-presence coverage, Node-API/FFI lifetime checks, eval, and exit-dispatch regressions remain passing. The FileHandle and transfer/terminate standalone fixtures pass with natural exits and actual file cleanup. Eight generator/ABI tests (69 assertions) and four Zig binding-helper tests pass.
...
Node controls avoid reading inside a close callback or its immediate continuation, which reproducibly crashes Node 24.18.0; terminal reads happen after callbacks unwind. No crash is treated as desired behavior. The independent #465 pending regression still fails with its positive control passing. Neither these focused results nor upstream skips establish full Bun parity.
