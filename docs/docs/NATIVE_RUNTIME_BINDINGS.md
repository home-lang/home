...
The internal synchronous-spawn helper now passes inherited descriptors directly instead of capturing and replaying all output. Buffered outputs are drained concurrently. CLI binary and system-shell signal termination retains the actual signal; re-raising SIGKILL/SIGSTOP avoids an invalid disposition reset. Shell startup failures exit unsuccessfully. Unsupported internal IPC/detached/stdin-buffer variants now report explicit errors; their complete implementation remains [#509](https://github.com/home-lang/home/issues/509).
