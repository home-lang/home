...
- The unmodified full `worker_threads.test.ts` reports **33 pass / 1 timeout / 1 between-test error**; all three snapshot cases pass, but the file is **not green**. Home's native CLI ignored `-t getHeapSnapshot`: its manual context setup discards that flag instead of setting the existing test-filter options ([#481](https://github.com/home-lang/home/issues/481)). Bun's control correctly reports 3 pass / 31 filtered out.
