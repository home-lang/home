...
Home owns `ScriptExecutionContext.cpp` through a generated `HomeScriptExecutionContext.cpp`, which preserves every sibling implementation and its order from the linked build's own `UnifiedSource-src_jsc_bindings-4.cpp`. It is produced by `build-support/bundle-native-modules.ts` on the same path as the other owned units: each sibling include is rewritten to its external ABI-matched source, the owned source must appear exactly once, and `ScriptExecutionContext.h` is checked against the external header's class ABI. Deriving the sibling set from the linked bundle rather than a checked-in copy is what keeps the replacement object and the objects it links beside describing the same partition — a checked-in list only matches the one build it was written from, and a bundle it disagrees with drops or duplicates a translation unit.
