...
Primary and vendor selections can now be retained as owned plans before later phases begin. Their journals are written from the retained plan, and execution uses the plan's exact selected indices and range. Changing a tracked manifest, adding a vendor file, releasing the caller's expectation bytes, or changing start/limit after preparation cannot silently change the executed set.

The production coordinator journal now retains the immutable plan and the ordered Docker, vendor checkout/discovery, root/test setup, remap, primary, guarded vendor preparation, vendor execution, and service-shutdown phases. It links each child journal and aggregates original case counts without awarding case credit to setup or services. Process/API failures are recorded as phase errors and leave later independently applicable vendor work visible. Complete primary execution, native Linux Docker verification, crash processing, and full parity remain open under #724 and #66.
...
Vendor preparation plans now record `phase` (`all`, `checkout`, or `install_build`) and the expected checkout revision when provided. Summaries distinguish `vendor_checked_out` from `vendor_prepared`, so successful checkout cannot be presented as successful installation/build. The validator checks each phase's exact selection and deadlines, rejects a revision mismatch, and retains compatibility with earlier combined journals. An interrupted revision check leaves the later operations explicitly unstarted. Native launch records also retain the crash-report URL, reporting setting and published Docker coordinator socket alongside the existing environment fields.
