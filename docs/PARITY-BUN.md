...
--bun-corpus-native-subset=minimal-js` executes eighty-one allowlisted JS
...
`HTMLRewriter.onDocument({ doctype })` removal, `jest.mock` argument
validation, and `expect.extend` matcher validation plus installed
expectation-object matchers. The bootstrap harness is installed once
