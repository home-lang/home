...
--bun-corpus-native-subset=minimal-js` executes thirty-three allowlisted JS
...
`Map`/`Set` deep-equality, `MessageEvent` constructor behavior, lifecycle
hook, own-key matcher, and stack-trace smokes. The bootstrap harness is
