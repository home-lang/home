...
--bun-corpus-native-subset=minimal-js` executes forty allowlisted JS
...
`Bun.wrapAnsi` smokes, retry/repeats runner behavior, `test.concurrent.each`, Deno `Event` / `CustomEvent`, and narrow bootstrap coverage for Node `DOMException`, Web
