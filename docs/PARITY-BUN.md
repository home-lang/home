...
config shells, TLS/uWS identity fixes, HTTP request buffering compatibility,
managed map/string helpers, and shell event-loop forwarding. This does not
raise the audited integrated parity count; remaining blockers are the
server all-connections-closed task carrier, hot-reload watcher wiring,
shell parser export, and shell glob walker export.
...
todo-registration smoke, the Web `atob`/`btoa` smoke, sixty-four
regression smokes, one bundler constant-fold smoke, bundler
function-toString `require()` preservation, `allowUnresolved`,
compile standalone/bytecode smokes, plugin defer coverage,
bun-target/sqlite bundler coverage, banner,
barrel, expanded Bake dev/deinitialization, browser-target builtin diagnostics, bundler comments,
...
smokes, the bundler minify corpus smoke, broader bundler edgecase /
...
MessagePort context cleanup, FormData set/append/get/delete, the full
Web `FormData` fixture, File-backed multipart serialization, Blob/File
write/unlink/writer/stat behavior, streaming Body clone and byte-reader
coverage, malformed multipart boundary rejection,
UTF-8 BOM stripping across Body readers, and `server.fetch` invalid-argument
rejection, Web `Response` inspection and clone/body stream teeing,
...
Web `URLSearchParams`, URL, and Headers Bun-extension coverage,
...
`Symbol.dispose`,
Node `util.inspect` / `util.format` / `util.formatWithOptions`
coverage for default and named `util` imports, compact/non-compact object
