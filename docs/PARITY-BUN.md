...
--bun-corpus-native-subset=minimal-js` executes twenty-five allowlisted JS
...
smokes, one bundler constant-fold smoke, five test-runner
...
`Buffer` binary/UTF-16LE, own-key matcher, and stack-trace smokes. The
bootstrap harness is installed once per JSC engine, resets counters
