...
> Bun baseline, 550 files are integrated into Home (~46.1%): rewritten
...
--bun-corpus-native-subset=minimal-js` executes one hundred sixty-seven allowlisted JS
...
Latest measured subset run: `167` files, `706` passed, `0` failed,
`42` todo.
...
| Files integrated into Home | 550 | ~46.1% |
...
| Files remaining to integrate | 643 | ~53.9%; excludes raw copy-only files that duplicate already-integrated Home paths |
