...
--bun-corpus-native-subset=minimal-js` executes thirty-one allowlisted JS
...
Bun file-metadata, Node `Buffer` binary/UTF-16LE/compare, `Map`/`Set`
deep-equality, lifecycle hook, own-key matcher, and stack-trace smokes. The bootstrap harness is
