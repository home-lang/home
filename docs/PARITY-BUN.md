...
--bun-corpus-native-subset=minimal-js` executes one hundred sixty-six allowlisted JS
...
expectation smokes plus `expect().toBeEmpty`, one nested-describe smoke, two `expectTypeOf` type-only smokes, a narrow `Bun.TOML.parse` throw smoke, a TOML build invalid-source diagnostic `lineText` crash-regression smoke, `Bun.stripANSI` and
...
Latest measured subset run: `166` files, `702` passed, `0` failed,
`42` todo.
