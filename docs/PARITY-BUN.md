...
--bun-corpus-native-subset=minimal-js` executes twenty-six allowlisted JS
...
todo-registration smoke, the Web `atob`/`btoa` smoke, nine regression
...
`Buffer` binary/UTF-16LE, lifecycle hook, own-key matcher, and
...
module shim. This is deliberately not the acceptance gate.
