...
--bun-corpus-native-subset=minimal-js` executes seventeen allowlisted JS
...
expectation smokes, one nested-describe smoke, and the Node
`DOMException`, JSC `ShadowRealm`, Bun file-metadata, and stack-trace
smokes. This is deliberately not the acceptance gate.
