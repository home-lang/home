...
--bun-corpus-native-subset=minimal-js` executes thirty-four allowlisted JS
...
todo-registration smoke, the Web `atob`/`btoa` smoke, thirteen regression
...
`Map`/`Set` deep-equality, `expect.any` constructor matching, `MessageEvent`
constructor behavior, lifecycle hook, own-key matcher, and stack-trace smokes. The bootstrap harness is
