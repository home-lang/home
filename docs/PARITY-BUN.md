...
--bun-corpus-native-subset=minimal-js` executes seven allowlisted JS
...
smoke, the Web `atob`/`btoa` smoke, two pure-JS regression smokes, and
the Node `DOMException`, JSC `ShadowRealm`, and Bun file-metadata
