...
--bun-corpus-native-subset=minimal-js` executes twenty-nine allowlisted JS
...
todo-registration smoke, the Web `atob`/`btoa` smoke, eleven regression
...
Bun file-metadata, Node `Buffer` binary/UTF-16LE/compare, lifecycle hook,
own-key matcher, and stack-trace smokes. The bootstrap harness is
