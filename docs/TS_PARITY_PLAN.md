...
| 6 — Conformance hardening | 🟡 runner + 56-case canon corpus + local TS smoke/category ratchets landed | `packages/ts_conformance/` runs source through the compiler and diffs against tsc-style baselines. `runCorpus(gpa, corpus, *results) -> Stats` plus `runDirectory` cover inline and disk fixtures. `runCategorySpecs` now summarizes named local TypeScript conformance folders with bounded memory, and the current category gate is **86/86**. Phase 6 punch list (§6.A): expand from named local folders to the full locally-installed TypeScript conformance corpus and wire a per-PR delta gate. |
...
4. ~~**Categorize the 5 907-case corpus by feature.**~~ 🟢 mostly landed 2026-05-05/08. The `builtin_corpus` array now has 56 cases keyed by feature (00..55). **Smoke runs now execute against three local TypeScript conformance subdirectories** (commit `95a716b`) — per-subdir + COMBINED pass-rate prints to stderr without failing the build. **TS conformance suite is now categorized by feature folder** (commit `3278cb2`) so per-folder pass-rate ratchet replaces the prior flat list as the data-driven prioritization input. `runCategorySpecs` now accepts named local TypeScript conformance folders and frees per-file result details after each category, so larger surveys do not retain the whole corpus in memory; `runCategorySpecsWithOptions(..., .{ .baseline_root = ... })` now recognizes upstream `.errors.txt` files for coarse expected-error awareness. 2026-05-08 ratchet: the current local smoke is **16/16** (`comparable` 13/13, `inOperator` 2/2, `stringLiteral` 1/1), the named category gate is **86/86** (`assignmentCompatibility` 70/70, `comparable` 13/13, `inOperator` 2/2, `stringLiteral` 1/1), and the baseline-aware `types/typeRelationships` survey is **95/175**. Raw direct probes now stand at apparentType **1/2**, bestCommonType **8/8**, recursiveTypes **12/13**, subtypesAndSuperTypes **49/52**, typeAndMemberIdentity **47/48**, and typeInference **34/51**; the apparent/subtype raw drops are expected-error files now correctly surfacing TS2416. The remaining unexpected direct-positive failures are `genericContextualTypes1.ts` and `keyofInferenceLowerPriorityThanReturn.ts`; most other raw direct failures are expected-error exactness gaps. The full 5907-case external suite still needs to be loaded from the local TypeScript checkout. *Remaining: load full disk corpus.*
...
> **Status (2026-05-08):** 🟡 partial. `packages/ts_conformance/` runner ships a 56-case canon corpus, on-disk fixtures at `tests/conformance/*.ts`, disk `runDirectory`, patience-diff unified mismatch output, a clean **16/16** smoke run against local TypeScript conformance subdirectories, a named-category runner currently clean at **86/86**, and a coarse baseline-aware `types/typeRelationships` survey at **95/175**. Raw direct triage is now at apparentType **1/2**, bestCommonType **8/8**, recursiveTypes **12/13**, subtypesAndSuperTypes **49/52**, typeAndMemberIdentity **47/48**, and typeInference **34/51**; the apparent/subtype raw deltas are expected-error files now correctly surfacing TS2416. The remaining unexpected direct-positive blockers are higher-order contextual generic inference in `genericContextualTypes1.ts` and return-priority `keyof` inference in `keyofInferenceLowerPriorityThanReturn.ts`. Full local-corpus loading, exact `.errors.txt` comparison in disk surveys, and per-PR delta gating remain. See **§6.A** above.
