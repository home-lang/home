...
> **Status (2026-05-08):** 🟡 partial. `packages/ts_conformance/` runner ships a 56-case canon corpus, on-disk fixtures at `tests/conformance/*.ts`, disk `runDirectory`, patience-diff unified mismatch output, a clean **16/16** smoke run against local TypeScript conformance subdirectories, a named-category runner currently clean at **86/86**, and a coarse baseline-aware `types/typeRelationships` survey at **90/175**. Direct positive-only triage is now at apparentType **2/2**, bestCommonType **8/8**, recursiveTypes **12/13**, subtypesAndSuperTypes **51/52**, typeAndMemberIdentity **47/48**, and typeInference **32/51**, with the latest batch removing parser/crash blockers from the type-inference slice and clearing `discriminatedUnionInference.ts`. Full local-corpus loading, exact `.errors.txt` comparison in disk surveys, and per-PR delta gating remain. See **§6.A** above.
