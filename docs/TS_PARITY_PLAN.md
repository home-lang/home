...
| 6 — Conformance hardening | 🟡 runner + 56-case canon corpus + local TS smoke/category/full-corpus hooks landed | `packages/ts_conformance/` runs source through the compiler and diffs against tsc-style baselines. `runCorpus(gpa, corpus, *results) -> Stats` plus `runDirectory` cover inline and disk fixtures. `runCategorySpecs` summarizes named local TypeScript conformance folders with bounded memory, the current category gate is **86/86** with `assignmentCompatibility` at **70/70** and baseline-aware `comparable` at **13/13**; the baseline-aware type-relationships survey is **175/175**. `exact_error_headers` loads upstream `.errors.txt` diagnostic headers for byte comparison, and `HOME_TS_CONFORMANCE_FULL=1` runs the local 5 907-case corpus with start/limit controls for crash bisection. Current ratchets: every 40-case window from `START=838` through `START=3998` is **40/40**; every larger window from `START=4038 LIMIT=200` through final `START=5838 LIMIT=200` is clean; `START=0 LIMIT=80` is **80/80**, `START=80 LIMIT=40` is **40/40**, `START=120 LIMIT=40` is **40/40**, `START=0 LIMIT=200` is **200/200**, `START=0 LIMIT=260` is **260/260**, `START=200 LIMIT=200` is **200/200**, `START=400 LIMIT=200` is **200/200**, `START=300 LIMIT=120` is **120/120**, `START=0 LIMIT=620` is **620/620**, `START=620 LIMIT=220` is **220/220**, `START=4900 LIMIT=100` is **100/100**, and `START=997 LIMIT=80` is **80/80**. The latest completed unbounded full local corpus is crash-free at **5 584/5 907**; **323 cases** remain as expected-error/expected-clean parity gaps. Phase 6 punch list (§6.A): reduce the remaining unbounded misses, promote exact diagnostic comparison, and wire a per-PR delta gate. |
...
14. ~~**JSDoc binder pass.**~~ 🟡 partial 2026-05-06/09. Standalone JSDoc tag scanner at `packages/ts_parser/src/jsdoc.zig` recognizes `@type`, `@param`, `@returns`, `@template`, `@typedef` with type-text + name + description capture. Some `@type` / multiline `@typedef` shapes feed checkJs variables today, but the `START=3038` / `START=3078` ratchets show the remaining checker work clearly: `@implements`, `@template`, `@satisfies`, `@constructor`, `@extends`, typedef duplicate/namepath validation, JSDoc syntax diagnostics, import-type object inference, and JSDoc-driven type-guard shapes still need first-class semantic validation rather than harness modeling.
...
> **Status (2026-05-13):** 🟡 partial. `packages/ts_conformance/` runner ships a 56-case canon corpus, on-disk fixtures at `tests/conformance/*.ts`, disk `runDirectory`, patience-diff unified mismatch output, a clean **16/16** smoke run against local TypeScript conformance subdirectories, a named-category runner clean at **86/86** with `assignmentCompatibility` at **70/70** and baseline-aware `comparable` at **13/13**, and a coarse baseline-aware `types/typeRelationships` survey clean at **175/175**. Full-corpus windows stand at every 40-case window from `START=838` through `START=3998` at **40/40**, plus every larger window from `START=4038 LIMIT=200` through the final `START=5838 LIMIT=200` corpus tail clean; `START=0 LIMIT=80` is **80/80**, `START=80 LIMIT=40` is **40/40**, `START=120 LIMIT=40` is **40/40**, `START=0 LIMIT=200` is **200/200**, `START=0 LIMIT=260` is **260/260**, `START=200 LIMIT=200` is **200/200**, `START=400 LIMIT=200` is **200/200**, `START=300 LIMIT=120` is **120/120**, `START=0 LIMIT=620` is **620/620**, `START=620 LIMIT=220` is **220/220**, `START=4900 LIMIT=100` is **100/100**, and `START=997 LIMIT=80` is **80/80**. The latest completed unbounded full local corpus is crash-free at **5 584/5 907**; **323 / 5 907 cases (5.5%)** remain as expected-error/expected-clean parity gaps under the current coarse gate. The smoke, named-category, type-relationships, first four broad early-window ratchets, and the crash-neighborhood tail ratchet are saturated; remaining Phase 6 work is reducing those 323 completed-run misses, continuing from the visible unbounded failure list, exact `.errors.txt` comparison, and per-PR delta gating. See **§6.A** above.
