...
> **Status (2026-05-12):** 🟡 partial. `packages/ts_conformance/` runner ships a 56-case canon corpus, on-disk fixtures at `tests/conformance/*.ts`, disk `runDirectory`, patience-diff unified mismatch output, a clean **16/16** smoke run against local TypeScript conformance subdirectories, a named-category runner now clean at **86/86** with `assignmentCompatibility` at **70/70** and baseline-aware `comparable` at **13/13**, and a coarse baseline-aware `types/typeRelationships` survey currently at **173/175** with `genericContextualTypes2/3` open. Full-corpus windows now stand at every 40-case window from `START=838` through `START=3998` at **40/40**, plus every larger window from `START=4038 LIMIT=200` through the final `START=5838 LIMIT=200` corpus tail clean; `START=0 LIMIT=80` is **80/80**, `START=80 LIMIT=40` is **40/40**, `START=120 LIMIT=40` is **40/40**, `START=0 LIMIT=200` is **200/200**, `START=0 LIMIT=260` is **260/260**, `START=200 LIMIT=200` is **200/200**, `START=400 LIMIT=200` is **200/200**, `START=300 LIMIT=120` is **120/120**, and `START=4900 LIMIT=100` is **100/100**. The latest completed unbounded full local corpus is crash-free at **5 551/5 907**; **356 / 5 907 cases (6.0%)** remain as expected-error/expected-clean parity gaps under the current coarse gate. The smoke, named-category, first four broad early-window ratchets, and the crash-neighborhood tail ratchet are saturated; remaining Phase 6 work is reducing those 356 completed-run misses, continuing from the visible unbounded failure list, exact `.errors.txt` comparison, and per-PR delta gating. See **§6.A** above.
