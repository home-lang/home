...
**Current phase:** Phase 1 — TypeScript frontend (foundation substantially complete; full grammar coverage tracked as follow-ups)
**Active deliverable:** ready to start Phase 2 — Binder + symbol table
...
**Phase 1 follow-ups (mechanical, gated by per-feature tests):** function/class/interface/enum/namespace declarations; full type-annotation parsing (union, intersection, generics, mapped, conditional, template literal, tuple, fn types); JSX/TSX; arrow functions (with the `(T) => U` ambiguity); decorators (legacy + Stage 3); imports/exports incl. type-only forms; control flow (if/while/for/switch/try); template literal substitutions (parser-driven `rescanTemplate` after each interpolated expression); regex literal scanning (parser-driven `rescanSlashAsRegex`); full Unicode ID-Start / ID-Continue tables; lib loading.

### Phases ahead

| Phase | Status | Notes |
|---|---|---|
| 2 — Binder + symbol table | ⬜ blocked-by Phase 1 follow-ups | |
| 3 — Type checker | ⬜ blocked-by Phase 2 | |
| 4 — JS emit + .d.ts + .d.hm | ⬜ blocked-by Phase 3 | |
| 4.5 — Bundler integration | ⬜ blocked-by Phase 4 | |
