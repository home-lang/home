...
**Current phase:** Phase 3 type checker substantially complete — generic instantiation via call-site inference, signatures, narrowing (typeof + null + else-branch), object-literal/array-literal inference, member access typing with TS2339, argument count + type diagnostics (TS2554/TS2345). Phase 8 LSP wire-protocol layer ready for editor integration. Phase 5 incremental rebuild flow + persistent compilation cache landed.
