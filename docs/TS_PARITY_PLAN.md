...
**Current phase:** Phase 3 type checker substantially complete + Phase 4 emit + Phase 8 LSP all advancing in parallel. Type checker covers: generic instantiation (incl. explicit type args properly substituted through signatures), generic type-alias instantiation, mapped types over string-literal-union constraints, conditional types evaluating eagerly with union distribution + deferred-conditional substitution under generic-alias instantiation, structural object assignability, signatures with return-type inference, full narrowing surface (typeof / null / undefined / else-branch / instanceof / `in` / discriminated unions / `as const` / **type predicates `arg is T`** / **`asserts arg is T` fall-through narrowing**) with proper union subtraction, arrow-fn signatures, class+interface+type-alias resolution, `this`/`super` typing, `extends` inheritance, ctor signatures, index signatures, tuple lowering, `Array<T>` shape, optional params, `keyof T`, strict-mode flags `noImplicitAny` (TS7005/TS7006), `noUnusedLocals`/`noUnusedParameters` (TS6133), `strictFunctionTypes` (bivariant ↔ contravariant signature-assignability), TS2353 fresh-object excess-property checks **including nested object literals**, non-null assertion `expr!`. JS emit covers: full Phase 1 surface, **`??` and `?.` lowering at ES2019 and below**, **JSX automatic runtime (`_jsx`/`_jsxs`/`_jsxDEV`)**, **CommonJS module emit with `__importDefault`/`__importStar` interop helpers**, source map V3, symbol-driven `.d.ts`, zig-dtsx fast path, class-decorator `__decorate` lowering. LSP covers: hover, definition, cross-file references, completion (module-level), **signatureHelp**, **inlayHints (inferred-let-type)**, **documentSymbols**, **semanticTokens (13-element legend)**, **rename (cross-file)**, **codeActions (Organize Imports)**, diagnostics. Program graph: parallel parse/bind, incremental rebuild, persistent on-disk compilation cache, multi-file cache wiring, **streaming diagnostics callback hook**. Conformance: harness + 56-case canon corpus exercising every landed feature, **patience-diff for triage output**.
**Active deliverable:** Phase 6 — full TS conformance corpus integration (vendor `microsoft/TypeScript` submodule); Phase 7 — native codegen for typed TS subset; Phase 4.5 — bundler integration via Bun; Phase 4 follow-ups — generators→state-machine, async→awaiter ES5 transform, parameter decorators. Total `zig build test` count: **1370 tests passing** across 30+ packages. Recent landings (2026-05-06): downlevel arrow/class/for-of at ES5, dynamic `import()` + CJS lowering, homomorphic `Partial<T>`/`Readonly<T>` with `+/- ?` and `+/- readonly` modifiers, `infer X` placeholder binding, `[T] extends [U]` non-distribution, `Module.augment` for cross-file declaration merging, method/property decorators, `release-fast` build with LTO, type predicates on class methods + arrows, JSDoc tag scanner stub, explicit `in`/`out` variance modifiers interned, **await/yield expression support**, **`home-tsc --watch` loop**, **`--listFiles` / `--listFilesOnly` / `--showConfig`** wired.
...
4. ~~**Template-literal types — partial.**~~ 🟢 partial 2026-05-05. `TemplateLiteralTypePayload` HIR shape + `addTemplateLiteralType` builder + `templateLiteralTypeTexts/Types` accessors. Parser handles the no-substitution form (`` `hello` ``); the substitution form (`` `hello-${T}` ``) requires parser-driven `rescanTemplate` after each interpolated expression — tracked as Phase 1.B follow-up. Lowerer maps `template_literal_type` to `Primitive.string_t`. Pattern matching of templates against literal-type targets is a Phase 6 follow-up.
5. ~~**Variance computation at definition site.**~~ 🟢 partial 2026-05-06. Explicit `in` / `out` modifiers (TS 4.7) now flow parser → HIR → interner; `TypeParameterPayload` carries a `Variance` enum (`bivariant`/`contravariant`/`covariant`/`invariant`) and the interner key includes it so `T` / `in T` / `out T` / `in out T` are distinct ids. `internTypeParameterWithVariance` + `typeParameterVariance(id)` accessors land. Mapped-type `K` and `infer R` placeholders stay bivariant. Remaining: auto-variance inference (walk each generic's body to compute the parameter's usage variance and cross-check against any explicit modifier), plus the relation-engine instantiation-pair fast path that reads the precomputed variance instead of falling back to structural recursion. *Remaining effort: 3 days; impact still ~5–10% on generic-heavy projects.*
...
10. ~~**Aliased conditional narrowing**~~ ✅ landed 2026-05-05. `let cond = isString(x); if (cond) { ... }` now narrows `x` inside the if. New `cond_aliases` map records `name -> guard_expr_node`; `applyTypeGuard` recursively expands the alias before applying guard logic. Reassignment clears the alias.
11. ~~**Late-bound `this` types — partial.**~~ 🟢 partial 2026-05-05. Parser consumes and skips `this: T` parameters (TS doesn't surface them at runtime). Full threading of the type into the body's narrow scope so `this.foo` resolves against the declared type is the remaining piece. Plus: `ThisType<T>` flipping contextual `this` inside object literals.
...
15. ~~**Module augmentation + `declare global`.**~~ 🟢 partially landed 2026-05-06. `Module.augment(other)` walks every symbol in `other`'s root scope (values / types / namespaces) and merges into `self`'s root scope. Internal helpers `mergeScope` + `mergeSymbolMap` factored out. Program-graph wiring (which files share an interner so cross-module merge actually unifies symbols by StringId) is a follow-up.
...
5. ~~**`async` / `await` parsing + native pass-through.**~~ 🟢 partially landed 2026-05-06. New `addAwaitExpr` + `addYieldExpr` HIR builders + accessors. Parser recognizes `await` / `yield` / `yield*` in unary position; statement-level `async function f()` consumed via `Hir.markFnAsync`. Emit: `await <expr>`, `yield <expr>`, `yield* <expr>`. Native pass-through works at ES2017+. The `__awaiter` state-machine downlevel for ES2015-ES2016 is the remaining piece.
...
8. ~~**Method / property / parameter decorators.**~~ 🟢 partially landed 2026-05-06. Method + property decorators emit `__decorate([...], ClassName.prototype, "name", null);` after the class body. Parameter decorators (positional metadata) and `emitDecoratorMetadata` (reified design type) are follow-ups.
...
12. ~~**`.tsbuildinfo` writer.**~~ ✅ landed 2026-05-05. Pure-Zig writer at `packages/ts_emit/src/tsbuildinfo.zig`. Wired into `home-tsc`: when `compilerOptions.incremental: true`, writes `tsconfig.tsbuildinfo` next to `outDir` with file paths + SHA-1 content hashes. Output respects `compilerOptions.tsBuildInfoFile`. Round-trip read for incremental rebuilds is a follow-up.
...
3. ~~**Patience-diff implementation.**~~ ✅ landed 2026-05-05. Pure-Zig `patience.zig` with anchor-finding + LIS + recursive gap diffing.
...
5. ~~**`codeAction`.**~~ 🟢 partially landed 2026-05-05. "Organize Imports" sorts top-level imports alphabetically. fix-all + missing-return-type + infer-parameter-types are remaining quick-fixes.
6. ~~**`rename`.**~~ ✅ landed 2026-05-05. Cross-file via `findReferences`; produces one `TextEdit` per occurrence.
7. ~~**`workspace/symbol`** + **`documentSymbol`.**~~ ✅ landed 2026-05-05. `documentSymbols(file)` enumerates top-level declarations; `workspaceSymbols(query)` searches across every file in the program with substring filter.
