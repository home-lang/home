...
**Active deliverable:** Phase 6 — full TS conformance corpus integration (vendor `microsoft/TypeScript` submodule); Phase 7 — native codegen for typed TS subset; Phase 4.5 — bundler integration via Bun; Phase 4 follow-ups — generators→state-machine, Stage 3 decorators, `.d.hm` member rendering. Total `zig build test` count: **1421 tests passing** across 30+ packages. Recent landings (2026-05-06): narrow on `===` with literal RHS, `--pretty` source-snippet diagnostics, incremental `compileAll` skipping unchanged files, `await` unwraps structural `Promise<T>`, LSP `formatDocument` stub + `foldingRanges`, two-level relation cache, `declare global` collection across files, patience-diff baseline output, basic variance auto-inference, `await`/`yield` expression typing, LSP `didChangeFile` recompile, `home-tsc --watch` + `RealStatFs`, `.d.hm` framing emitter, private fields → WeakMap, async/await `__awaiter` downlevel, smoke conformance runs, parameter decorators.
...
10. ~~**Aliased conditional narrowing + `===` with literal RHS.**~~ ✅ landed 2026-05-05/06. `let cond = isString(x); if (cond) { ... }` narrows `x` inside the if. `cond_aliases` map records `name -> guard_expr_node`; `applyTypeGuard` recursively expands the alias before applying guard logic. Reassignment clears the alias. **`if (x === "literal") { ... }` / `if (x === 42)` / `=== true / false / null / undefined` now narrow `x` to the matching literal type in the then-branch and subtract it in the else-branch** (commit `339d13b`).
