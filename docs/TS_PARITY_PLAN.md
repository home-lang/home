...
> **Status (2026-05-11):** 🟡 partial. `packages/ts_conformance/` runner ships a 56-case canon corpus, on-disk fixtures at `tests/conformance/*.ts`, disk `runDirectory`, patience-diff unified mismatch output, a clean **16/16** smoke run against local TypeScript conformance subdirectories, a named-category runner currently clean at **86/86**, and a coarse baseline-aware `types/typeRelationships` survey now clean at **175/175**. Full-corpus windows now stand at every 40-case window from `START=838` through `START=3998` at **40/40**, plus the larger `START=4038 LIMIT=200`, `START=4238 LIMIT=200`, `START=4438 LIMIT=200`, `START=4638 LIMIT=200`, `START=4838 LIMIT=200`, `START=5038 LIMIT=200`, `START=5238 LIMIT=200`, `START=5438 LIMIT=200`, and `START=5638 LIMIT=200` sweeps at **200/200**. The next active window is the `START=5838 LIMIT=200` corpus tail; **69 / 5 907 cases (1.2%)** remain ungreen or unratcheted. The named type-relationships ratchet is saturated; remaining Phase 6 work is exact `.errors.txt` comparison, finishing the corpus tail, full unbounded corpus stability, and per-PR delta gating. See **§6.A** above.
