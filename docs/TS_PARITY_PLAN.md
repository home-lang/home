...
> **Status (2026-05-08):** 🟡 partial. `packages/ts_conformance/` runner ships a 56-case canon corpus, on-disk fixtures at `tests/conformance/*.ts`, disk `runDirectory`, patience-diff unified mismatch output, a clean **16/16** smoke run against local TypeScript conformance subdirectories, a named-category runner currently clean at **86/86**, and a coarse baseline-aware `types/typeRelationships` survey at **93/175**. Direct positive-only triage is now at apparentType **2/2**, bestCommonType **8/8**, recursiveTypes **12/13**, subtypesAndSuperTypes **51/52**, typeAndMemberIdentity **47/48**, and typeInference **35/51**, with the latest batch clearing `keyofInferenceIntersectsResults.ts`, `unionAndIntersectionInference3.ts`, and `genericCallTypeArgumentInference.ts`; the remaining direct-positive type-inference blocker is higher-order contextual generic inference in `genericContextualTypes1.ts`. Full local-corpus loading, exact `.errors.txt` comparison in disk surveys, and per-PR delta gating remain. See **§6.A** above.
