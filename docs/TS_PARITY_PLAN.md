...
**Current phase:** Phase 4.5 substantially complete — driver wires lex→parse→bind→emit; HIR→TypeId lowering connects parser to checker; multi-file program graph operates on resolved imports; CLI flag surface ships; source maps end-to-end; tsc-compatible diagnostic formatting; decorator capture
**Active deliverable:** Phase 5 — parallel parse/bind/check, query DB, performance tuning; Phase 6 — TS conformance harness
...
| **New: `packages/ts_emit`** | JS + .d.ts emit + source maps V3 | Phase 4 — *JS emit + VLQ source maps + streaming printer wired 2026-05-05* |
| **New: `packages/ts_driver`** | Lex → parse → bind → emit pipeline | Phase 4.5 — *single-file E2E + tsconfig integration landed 2026-05-05* |
