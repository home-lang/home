...
**Active deliverable:** Phase 6 — full TS conformance corpus integration (vendor `microsoft/TypeScript` submodule); Phase 7 — native codegen for typed TS subset; Phase 4.5 — bundler integration via Bun; Phase 4 follow-ups — generators→state-machine, Stage 3 decorators, `.d.hm` member rendering. Total `zig build test` count: **1733 tests passing** across 30+ packages. Recent landings (2026-05-07 batch 4): destructuring with defaults — undefined removal (commit `47b7e53`), logical assignment narrowing for `??=` (commit `96b6c50`), `ts_lsp` semantic tokens delta encoding tests (commit `306996d`), BigInt literal native + downlevel emit tests (commit `2f27595`), `ts_lsp` `workspace/symbol` case-insensitive substring search (commit `45b1084`), array spread element typing (commit `189fc1d`), object spread merges member types (commit `bdf34ef`), template literal type assignability (commit `be8f1c4`), `ts_lsp_server` LSP 3.17 pull-based diagnostics (commit `165062a`), tagged template literal call typing v0 (commit `26dd839`), `ts_bundler` minify pass — strip comments + collapse whitespace (commit `cb95c1c`), `ts_lsp_server` `textDocument/inlineCompletion` wire handler (commit `260ca8d`), discriminated union member-access narrowing (commit `60e6bee`), JSX `react-jsxdev` runtime (commit `1449c7c`), `ts_lsp_server` `textDocument/declaration` wire handler (commit `ec6cb8c`), const enum literal type at member access (commit `da62a2f`), `yield*` delegated iteration (commit `b26e61c`), `// @ts-nocheck` file-level directive (commit `844fda6`), enum member auto-increment + value tracking (commit `7399a3e`), basic generator function return type inference (commit `bac3df6`). Earlier landings (2026-05-06 evening): `const` type parameters TS 5.0 syntax (commit `a941758`), `Awaited<T>` recursive-unwrap intrinsic (commit `1b96b2a`), `ts_lsp` `textDocument/inlineValue` (commit `fb3cbd6`), import attributes (`with` / `assert`) syntax (commit `eb5baa2`), `accessor` class member modifier TS 4.9 (commit `6547339`), `using` and `await using` declarations (commit `35b5e3f`), `.d.hm.map` source-map v0 framing (commit `508ba88`), `ts_lsp` codeLens reference counts on top-level decls (commit `2089aa2`), `ts_lsp_server` `textDocument/onTypeFormatting` wire handler (commit `bbc70f6`), `ts_bundler` JSON manifest emit (commit `a956c58`), checker `noUncheckedIndexedAccess` (commit `c97d43b`), `isolatedModules` basic checks (commit `6541824`), inferred type predicates from narrowing returns TS 5.5 (commit `cecf32c`), `exactOptionalPropertyTypes` + spelling suggestions (commit `06681a3`), tsconfig `baseUrl` + `paths` aliases in resolver (commit `bd75393`), `resolveJsonModule` option (commit `8170764`), parser fix to emit `kw_undefined` as `literal_undefined` in expression position (commit `ea1fdf3`).
...
10. ~~**JSX transforms.**~~ 🟢 partially landed 2026-05-05/07. `Options.jsx_runtime` selects classic / automatic / automatic_dev / preserve. Automatic emits `_jsx(tag, props)` for single-children and `_jsxs(tag, props)` for multiple, threading children into the props bag. **Classic-runtime (`React.createElement`) emit baselines pinned 2026-05-07** (commit `7c7f67e`) — single-child / multi-child / fragment / member-access tag / spread attrs all round-trip-tested. Driver wires from tsconfig's `compilerOptions.jsx`. Remaining: auto-injecting `import { jsx as _jsx, jsxs as _jsxs } from "react/jsx-runtime"` (today the user adds it manually); `react-native` mode tagging for the bundler.
...
4. ~~**Categorize the 5 907-case corpus by feature.**~~ 🟢 mostly landed 2026-05-05/06. The `builtin_corpus` array now has 56 cases keyed by feature (00..55). **Smoke runs now execute against three local TypeScript conformance subdirectories** (commit `95a716b`) — per-subdir + COMBINED pass-rate prints to stderr without failing the build. **TS conformance suite is now categorized by feature folder** (commit `3278cb2`) so per-folder pass-rate ratchet replaces the prior flat list as the data-driven prioritization input. The full 5907-case external suite still needs to be loaded once §6.A.2 lands. *Remaining: load full disk corpus.*
