...
**Active deliverable:** Phase 6 — full local TypeScript conformance corpus integration (no submodule; use the locally-installed TypeScript checkout per user direction); Phase 7 — native codegen for typed TS subset; Phase 4.5 — bundler integration via Bun; Phase 4 follow-ups — generators→state-machine, full Stage 3 decorator initializer semantics, `.d.hm` type re-printing. Last verified full `zig build test --summary failures` under Pantry-managed Zig `0.17.0-dev.263+0add2dfc4`: **passing**, diagnostics snapshots pass **95/95**, the local TypeScript conformance smoke is clean at **16/16** (`comparable` 13/13, `inOperator` 2/2, `stringLiteral` 1/1), the named category ratchet is clean at **86/86** (`assignmentCompatibility` 70/70, `comparable` 13/13, `inOperator` 2/2, `stringLiteral` 1/1), and the baseline-aware type-relationships survey is clean at **175/175** (`apparentType` 2/2, `bestCommonType` 8/8, `recursiveTypes` 13/13, `subtypesAndSuperTypes` 52/52, `typeAndMemberIdentity` 48/48, `typeInference` 52/52). Recent Phase 6 landings added exact `.errors.txt` diagnostic-header loading/comparison (`DirectoryLoadOptions.exact_error_headers`) plus an opt-in full 5 907-case local TypeScript corpus survey (`HOME_TS_CONFORMANCE_FULL=1`, with `HOME_TS_CONFORMANCE_START` / `HOME_TS_CONFORMANCE_LIMIT` for bisection). Full-corpus slice ratchets from `START=838` through `START=3998` in 40-case windows are now all **40/40**; the next 80-case destructuring sweep at `START=4038` currently stands at **78/80**, with only `destructuringParameterDeclaration10` (nested generic object type in contextual function signature parsing) and `destructuringInFunctionType` (TS2842 unused-renaming diagnostics in function-type syntax) left in that sweep. The 2026-05-09 class/decorator/accessor/heritage/private-name/accessibility/static-block/module/async/decorator-expression/JS-salsa/parser-diagnostic/statement-recovery/internal-module/JSDoc/external-module/resource-management/statement-control/import-defer/generator/destructuring batches added class-member decorator parsing that does not swallow computed names, decorators after class/parameter modifiers, optional methods, static blocks, object-literal accessors including computed accessor keys, numeric/string class member names including dotted numeric keys, `abstract new` constructor types, contextual `abstract`/`await`/`async`/`global`/`require`/`module`/`of`/`yield` identifiers, top-level `accessor` recovery for malformed modifier fixtures, CommonJS `export =` / `import X = Y` tolerance, comma expressions, `NoIn` parsing for `for` headers, assertion-return function types, `import.meta` expression-statement parsing, re-export import-attributes, `export * as default from` tolerance, `debugger;` statements, `with` statements with strict-mode diagnostics, computed enum members, do-while continuation without a mandatory ASI boundary, namespace ASI disambiguation, namespace-only value diagnostics, qualified namespace type-ref lookup, static class/namespace member conflict checks, CommonJS `module[...]` expression parsing, typed-array/DOM globals, member-target logical assignment, CommonJS expando target-chain tolerance, top-level module `await` binding/import diagnostics, import-equals string-literal diagnostics, duplicate default export diagnostics, export-assignment exclusivity diagnostics, non-local named-export diagnostics, module-variant baseline discovery, explicit-resource-management parser diagnostics, multi-declarator variable binding, `break`/`continue` loop-depth diagnostics, `for...in` destructuring diagnostics, `import.defer` / invalid `import.*` meta-property diagnostics, generator/class semantic diagnostics, rest/binding-pattern diagnostics, binding-pattern default self/later-reference diagnostics (TS2448/TS2372/TS2373), destructuring assignment rest/iterator assignability, catch binding-pattern scope, strict-null destructuring diagnostics, `[Symbol.iterator]` class-member iterable recognition, `Map` entry inference for spread rest calls, target-suffixed `.errors.txt` discovery, and coarse JSDoc/ERM semantic-gap modeling. The harness now explicitly tracks coarse expected-error/expected-clean gaps for option-deprecation, resolver/package/library/reference-path, decorator signature/name-evaluation/metadata/emit-helper gaps, accessor override, auto-accessor, mixin, class constructor/accessibility/abstractness/static-block/async target semantics, `super` ordering, static-side `this`, private-name, ES2020/ES2021/lib globals, enum constant/merging/classification diagnostics, ambient/globalThis rules, logical-assignment narrowing, JS CommonJS/property-assignment inference cases, parser computed-property/generic-ambiguity/parameter-list/error-recovery/statement-control-flow diagnostics, ES5 parser-only `for-of` fixtures, internal-module export visibility / declaration-accessibility diagnostics, namespace import value-space diagnostics, JSDoc semantic validation/import-type inference gaps, module-resolution/verbatim-module/type-only namespace export diagnostics, explicit-resource-management disposable/lib/import-helper diagnostics, target/downlevelIteration deprecation-only baselines, and regex/divide lexer-mode exactness that are not yet modeled by the single-source checker. The coarse type-relationships ratchet is saturated; next Phase 6 work is clearing the two parser blockers in the `START=4038` sweep, then graduating from expected-any checks to exact `.errors.txt` text comparison and a per-PR delta gate.
