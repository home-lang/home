...
18. **`Awaited<T>` recursive unwrap intrinsic.** Open follow-up. Current `await`-site typing handles single-level `Promise<T>` unwrap; the standalone `Awaited<T>` intrinsic that recursively unwraps nested promises and thenables is the open piece.
