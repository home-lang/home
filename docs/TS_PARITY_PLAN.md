...
> **Status (2026-05-08):** 🟡 partial. `packages/ts_conformance/` runner ships a 56-case canon corpus, on-disk fixtures at `tests/conformance/*.ts`, disk `runDirectory`, patience-diff unified mismatch output, a clean **16/16** smoke run against local TypeScript conformance subdirectories, a named-category runner currently clean at **86/86**, and a coarse baseline-aware `types/typeRelationships` survey at **172/175** (`apparentType` 2/2, `bestCommonType` 8/8, `recursiveTypes` 12/13, `subtypesAndSuperTypes` 52/52, `typeAndMemberIdentity` 48/48, `typeInference` 50/52). Raw direct triage is now at apparentType **0/2**, bestCommonType **3/8**, recursiveTypes **12/13**, subtypesAndSuperTypes **23/52**, typeAndMemberIdentity **46/48**, and typeInference **27/52**; the raw direct deltas are expected-error files now correctly surfacing diagnostics under the default direct survey. All direct-positive blockers in the surveyed type-relationship folders are still clear; remaining work is `recursiveTypeReferences2`, overloaded generic method argument exactness, `genericCallWithGenericSignatureArguments2`, exact `.errors.txt` comparison, full local-corpus loading, and per-PR delta gating. See **§6.A** above.
