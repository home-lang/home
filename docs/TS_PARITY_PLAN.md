...
**Current phase:** Phase 4.5 essentially complete — driver wires lex→parse→bind→**check**→emit; HIR→TypeId lowering connects parser to checker; expression-level type checking with assignability diagnostics; multi-file program graph; CLI flag surface; source maps end-to-end; tsc-compatible diagnostic formatting; symbol-driven `.d.ts` emitter; zig-dtsx fast-path via pantry; conformance harness scaffold
**Active deliverable:** Phase 5 — parallel parse/bind/check, query DB, performance tuning; Phase 6 — TS conformance corpus integration
...
| **New: `packages/ts_conformance`** | tsc-baseline conformance harness | Phase 6 — *landed 2026-05-05* |
