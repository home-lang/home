...
**Active deliverable:** Phase 6 — full local TypeScript conformance corpus integration (no submodule; use the locally-installed TypeScript checkout per user direction); Phase 7 — native codegen for typed TS subset; Phase 4.5 — bundler integration via Bun; Phase 4 follow-ups — generators→state-machine, full Stage 3 decorator initializer semantics, `.d.hm` type re-printing. Last verified full `zig build test --summary all` under Pantry-managed Zig `0.17.0-dev.263+0add2dfc4`: **1973/1973 tests passing**, all **166/166** build steps exit successfully, diagnostics snapshots pass **95/95**, the local TypeScript conformance smoke is clean at **16/16** (`comparable` 13/13, `inOperator` 2/2, `stringLiteral` 1/1), the named category ratchet is clean at **86/86** (`assignmentCompatibility` 70/70, `comparable` 13/13, `inOperator` 2/2, `stringLiteral` 1/1), and the baseline-aware type-relationships survey is **148/175** (`apparentType` 2/2, `bestCommonType` 8/8, `recursiveTypes` 11/13, `subtypesAndSuperTypes` 40/52, `typeAndMemberIdentity` 48/48, `typeInference` 39/52). Recent landings since `59bfcaa` (2026-05-07/08 continuation): `#!` shebang as leading comment (commit `c86c4df`), NaN/isNaN/Infinity/parseFloat globals (commit `32b6b8c`), Stage 3 decorator emit-shape tests (commit `5f6cf60`), object method shorthand emit + ES5 lowering tests (commit `4838d35`), `keyof` literal union + indexed-access tests (commit `2a5424d`), class getter/setter accessor typing tests (commit `18bd82b`), Math/Number/console lib globals (commit `e46d293`), Home parser ratchets for `@memset`/`@memcpy` 2-arg form, while-continue expression, error-union return types, try expressions, `if let`, multi-pattern switch arms, and Zig-style non-empty return-type params (commits `da11d98`..`99bbda9`), lint scripts switched from ESLint to pickier (commit `9066886`), and the Phase 6 raw direct type-relationships probes now show assignment compatibility **70/70**, apparent types **0/2**, best common type **3/8**, recursive types **12/13**, subtypes/supertypes **31/52**, type/member identity **46/48**, and type inference **36/52** under direct `home-tsc --noEmit --no-pretty` surveys. The raw apparent/best-common/subtype movement is expected-error fixtures now correctly producing diagnostics, while the baseline-aware runner's strict-default expected-error mode captures more upstream-negative parity without regressing the smoke/category gates; all direct-positive blockers in the surveyed type-relationship folders are still clear, leaving exact-diagnostic comparison, broader disk-corpus loading, and remaining expected-error false negatives as the next Phase 6 work.
...
> **Status (2026-05-08):** 🟡 partial. `packages/ts_conformance/` runner ships a 56-case canon corpus, on-disk fixtures at `tests/conformance/*.ts`, disk `runDirectory`, patience-diff unified mismatch output, a clean **16/16** smoke run against local TypeScript conformance subdirectories, a named-category runner currently clean at **86/86**, and a coarse baseline-aware `types/typeRelationships` survey at **148/175** (`apparentType` 2/2, `bestCommonType` 8/8, `recursiveTypes` 11/13, `subtypesAndSuperTypes` 40/52, `typeAndMemberIdentity` 48/48, `typeInference` 39/52). Raw direct triage is now at apparentType **0/2**, bestCommonType **3/8**, recursiveTypes **12/13**, subtypesAndSuperTypes **31/52**, typeAndMemberIdentity **46/48**, and typeInference **36/52**; the raw direct deltas are expected-error files now correctly surfacing diagnostics under the default direct survey. All direct-positive blockers in the surveyed type-relationship folders are still clear; remaining work is expected-error false negatives, exact `.errors.txt` comparison, full local-corpus loading, and per-PR delta gating. See **§6.A** above.
