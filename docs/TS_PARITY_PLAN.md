...
**Active deliverable:** Phase 6 — full TS conformance corpus integration (vendor `microsoft/TypeScript` submodule); Phase 7 — native codegen for typed TS subset; Phase 4.5 — bundler integration via Bun; Phase 4 follow-ups — generators→state-machine, async→awaiter, parameter decorators. Total `zig build test` count: **1344 tests passing** across 30+ packages. Recent landings (2026-05-06): downlevel arrow/class/for-of at ES5, dynamic `import()` + CJS lowering, homomorphic `Partial<T>`/`Readonly<T>`, `infer X` placeholder binding, `[T] extends [U]` non-distribution, `Module.augment` for cross-file declaration merging, method/property decorators, `release-fast` build with LTO, type predicates on class methods + arrows.
