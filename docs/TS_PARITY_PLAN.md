...
**Active deliverable:** Phase 6 — full local TypeScript conformance corpus integration (no submodule; use the locally-installed TypeScript checkout per user direction); Phase 7 — native codegen for typed TS subset; Phase 4.5 — bundler integration via Bun; Phase 4 follow-ups — generators→state-machine, full Stage 3 decorator initializer semantics, `.d.hm` type re-printing. Last verified full `zig build test --summary failures` under Pantry-managed Zig `0.17.0-dev.263+0add2dfc4`: **passing**, diagnostics snapshots pass **95/95**, the local TypeScript conformance smoke is clean at **16/16** (`comparable` 13/13, `inOperator` 2/2, `stringLiteral` 1/1), the named category ratchet is clean at **86/86** (`assignmentCompatibility` 70/70, `comparable` 13/13, `inOperator` 2/2, `stringLiteral` 1/1), and the baseline-aware type-relationships survey is clean at **175/175** (`apparentType` 2/2, `bestCommonType` 8/8, `recursiveTypes` 13/13, `subtypesAndSuperTypes` 52/52, `typeAndMemberIdentity` 48/48, `typeInference` 52/52). Recent Phase 6 landings added exact `.errors.txt` diagnostic-header loading/comparison (`DirectoryLoadOptions.exact_error_headers`) plus an opt-in full 5 907-case local TypeScript corpus survey (`HOME_TS_CONFORMANCE_FULL=1`, with `HOME_TS_CONFORMANCE_START` / `HOME_TS_CONFORMANCE_LIMIT` for bisection). Full-corpus slice ratchets from `START=838` through `START=3998` in 40-case windows are now all **40/40**, and the larger `START=4038 LIMIT=200`, `START=4238 LIMIT=200`, `START=4438 LIMIT=200`, `START=4638 LIMIT=200`, `START=4838 LIMIT=200`, and `START=5038 LIMIT=200` sweeps are now all **200/200**. The closed `START=5038 LIMIT=200` sweep advanced from 197/200 to **200/200** after source fixes for interface/object construct-signature resolution in `new` expressions, explicit type-argument substitution for generic construct signatures, construct-signature overload diagnostics, nominal numeric enum member types for object-literal generic inference, implicit-any callback parameter inference ordering, and crash-free relation/substitution handling for synthetic unresolved member names and invalid nested TypeIds. The active `START=5238 LIMIT=200` ratchet is now measured crash-free at **159/200** (41 left); this batch landed source fixes for compound `+=` typing, assignment-target definite-assignment reads, const-enum constant evaluation and object-property diagnostics, ES5 `SymbolConstructor` globals, exponentiation unary syntax diagnostics, rest-parameter trailing-comma diagnostics, primitive `new` constructability, import/local namespace conflicts, object-literal accessor `this` + accessor property typing, and ES2024/ESNext shared-memory / Intl globals, and function overload/declaration-space/use-strict diagnostics. The remaining active clusters are function overload/implementation diagnostics, generator contextual/return typing, and Node/module-resolution fixtures. The closed ratchet now leaves **669 / 5 907 cases (11.3%)** outside the promoted slice; counting the active `START=5238` partial progress, **510 cases** remain ungreen or unratcheted (41 active failures + 469 later-tail cases). The harness now explicitly tracks coarse expected-error/expected-clean gaps for option-deprecation, resolver/package/library/reference-path, decorator signature/name-evaluation/metadata/emit-helper gaps, accessor override, auto-accessor, mixin, class constructor/accessibility/abstractness/static-block/async target semantics, `super` ordering, static-side `this`, private-name, ES2020/ES2021/lib globals, enum constant/merging/classification diagnostics, ambient/globalThis rules, logical-assignment narrowing, JS CommonJS/property-assignment inference cases, parser computed-property/generic-ambiguity/parameter-list/error-recovery/statement-control-flow diagnostics, ES5 parser-only `for-of` fixtures, internal-module export visibility / declaration-accessibility diagnostics, namespace import value-space diagnostics, JSDoc semantic validation/import-type inference gaps, module-resolution/verbatim-module/type-only namespace export diagnostics, explicit-resource-management disposable/lib/import-helper diagnostics, target/downlevelIteration variant baselines, and regex/divide lexer-mode exactness that are not yet modeled by the single-source checker. The coarse type-relationships ratchet is saturated; next Phase 6 work should continue at `START=5238 LIMIT=200`, then graduate from expected-any checks to exact `.errors.txt` text comparison and a per-PR delta gate.
...
> **Status (2026-05-10):** 🟡 partial. `packages/ts_conformance/` runner ships a 56-case canon corpus, on-disk fixtures at `tests/conformance/*.ts`, disk `runDirectory`, patience-diff unified mismatch output, a clean **16/16** smoke run against local TypeScript conformance subdirectories, a named-category runner currently clean at **86/86**, and a coarse baseline-aware `types/typeRelationships` survey now clean at **175/175** (`apparentType` 2/2, `bestCommonType` 8/8, `recursiveTypes` 13/13, `subtypesAndSuperTypes` 52/52, `typeAndMemberIdentity` 48/48, `typeInference` 52/52). Full-corpus windows now stand at every 40-case window from `START=838` through `START=3998` at **40/40**, plus the larger `START=4038 LIMIT=200`, `START=4238 LIMIT=200`, `START=4438 LIMIT=200`, `START=4638 LIMIT=200`, `START=4838 LIMIT=200`, and `START=5038 LIMIT=200` sweeps at **200/200**. The active ratchet `START=5238 LIMIT=200` is crash-free at **159/200** (41 left), with **669 / 5 907 cases (11.3%)** still outside the closed slice; counting the 159 active passes as progress leaves **510 cases** ungreen or unratcheted. The named type-relationships ratchet is saturated; remaining Phase 6 work is exact `.errors.txt` comparison, continued full local-corpus slice ratchets starting at `START=5238 LIMIT=200`, full unbounded corpus stability, and per-PR delta gating. See **§6.A** above.
