...
> **Status (2026-05-11):** 🟡 partial. `packages/ts_conformance/` runner ships a 56-case canon corpus, on-disk fixtures at `tests/conformance/*.ts`, disk `runDirectory`, patience-diff unified mismatch output, a clean **16/16** smoke run against local TypeScript conformance subdirectories, a named-category runner currently clean at **86/86**, and a coarse baseline-aware `types/typeRelationships` survey now clean at **175/175**. Full-corpus windows now stand at every 40-case window from `START=838` through `START=3998` at **40/40**, plus every larger window from `START=4038 LIMIT=200` through the final `START=5838 LIMIT=200` corpus tail clean; `START=0 LIMIT=80` is **80/80**, `START=80 LIMIT=40` is **40/40**, `START=120 LIMIT=40` is **40/40**, and `START=0 LIMIT=200` is **200/200**. The next broad frontier is `START=200 LIMIT=200` at **191/200**. The unbounded full local corpus now completes without crashes, with conservative focused progress at **5 500/5 907**; **407 / 5 907 cases (6.9%)** remain as expected-error/expected-clean parity gaps under the current coarse gate. The named type-relationships ratchet is saturated; remaining Phase 6 work is reducing those 407 misses, exact `.errors.txt` comparison, and per-PR delta gating. See **§6.A** above.
