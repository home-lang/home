...
**Current phase:** Phase 3 type checker substantially complete + Phase 4 emit + Phase 8 LSP all advancing in parallel. Type checker covers: generic instantiation (incl. explicit type args properly substituted through signatures), generic type-alias instantiation, mapped types over string-literal-union constraints, conditional types evaluating eagerly with union distribution + deferred-conditional substitution under generic-alias instantiation, structural object assignability, signatures with return-type inference, full narrowing surface (typeof / null / undefined / else-branch / instanceof / `in` / discriminated unions / `as const` / **type predicates `arg is T`** / **`asserts arg is T` fall-through narrowing** / **`===`-with-literal-RHS narrowing**) with proper union subtraction, **`await` unwraps structural `Promise<T>`**, **basic auto-variance inference for generic params**, arrow-fn signatures, class+interface+type-alias resolution, `this`/`super` typing, `extends` inheritance, ctor signatures, index signatures, tuple lowering, `Array<T>` shape, optional params, `keyof T`, strict-mode flags `noImplicitAny` (TS7005/TS7006), `noUnusedLocals`/`noUnusedParameters` (TS6133), `strictFunctionTypes` (bivariant ↔ contravariant signature-assignability), TS2353 fresh-object excess-property checks **including nested object literals**, non-null assertion `expr!`. JS emit covers: full Phase 1 surface, **`??` and `?.` lowering at ES2019 and below**, **JSX automatic runtime (`_jsx`/`_jsxs`/`_jsxDEV`)**, **CommonJS module emit with `__importDefault`/`__importStar` interop helpers**, **async/await `__awaiter` downlevel for ES2015-ES2016**, **private fields → WeakMap**, **parameter decorators**, source map V3, symbol-driven `.d.ts`, **basic `.d.hm` framing emitter**, zig-dtsx fast path, class-decorator `__decorate` lowering. LSP covers: hover, definition, cross-file references, completion (module-level), **signatureHelp**, **inlayHints (inferred-let-type)**, **documentSymbols + workspaceSymbols**, **semanticTokens (13-element legend)**, **rename (cross-file)**, **codeActions (Organize Imports)**, **formatDocument stub + foldingRanges**, **`didChangeFile` triggers recompile + fresh diagnostics**, diagnostics. Program graph: parallel parse/bind, **incremental `compileAll` skips unchanged files**, persistent on-disk compilation cache, multi-file cache wiring, **streaming diagnostics callback hook**, **two-level relation cache (L1+L2) for parallelization readiness**, **`declare global` augmentation collection across files**. Conformance: harness + 56-case canon corpus exercising every landed feature, **patience-diff unified output on baseline mismatch**, **smoke runs against local TS conformance subdirectory**. CLI: **`home-tsc --watch` uses real `ts_watch.Watcher` + `RealStatFs`**, **`--pretty` wires through `formatPretty` with source-snippet excerpt**.
**Active deliverable:** Phase 6 — full TS conformance corpus integration (vendor `microsoft/TypeScript` submodule); Phase 7 — native codegen for typed TS subset; Phase 4.5 — bundler integration via Bun; Phase 4 follow-ups — generators→state-machine, Stage 3 decorators, `.d.hm` member rendering. Total `zig build test` count: **1544 tests passing** across 30+ packages. Recent landings (2026-05-06): LSP `initialize`/`shutdown`/`exit` lifecycle wire handlers, re-export forms (`export *` / `export { x } from`), `documentSymbols` includes nested members, sourceMap → `.js.map` write, `.tsbuildinfo` round-trip wiring (read on startup), basic `ThisType<T>` recognition, `this:T` parameter binding, `textDocument/hover` wire handler, shadowing-aware `findReferences`, tsbuildinfo round-trip reader, member-access narrowing on identifier-rooted access, codeAction "Add explicit type annotation", `home-tsc --watch` proper sleep API, hover renders function/class/let declaration shape, gotoDefinition follows imports across files, 8 commonly-used tsconfig fields, semantic-tokens delta-encoded wire format, narrow on `===` with literal RHS, `--pretty` source-snippet diagnostics, incremental `compileAll` skipping unchanged files, `await` unwraps structural `Promise<T>`, LSP `formatDocument` stub + `foldingRanges`, two-level relation cache, `declare global` collection across files, patience-diff baseline output.
...
10. ~~**Aliased conditional narrowing + `===` with literal RHS + `Array.isArray` + `typeof function`.**~~ ✅ landed 2026-05-05/06. `let cond = isString(x); if (cond) { ... }` narrows `x` inside the if. `cond_aliases` map records `name -> guard_expr_node`; `applyTypeGuard` recursively expands the alias before applying guard logic. Reassignment clears the alias. **`if (x === "literal") { ... }` / `if (x === 42)` / `=== true / false / null / undefined` now narrow `x` to the matching literal type in the then-branch and subtract it in the else-branch** (commit `339d13b`). **`if (Array.isArray(x))` narrows `x` to its array-typed union variants** (commit `9b1e481`); the same commit lands `typeof x === "function"` narrowing to signature-typed variants. Member-access narrowing on identifier-rooted access landed earlier this week (commit `bef89da`).
...
8. ~~**Method / property / parameter decorators.**~~ 🟢 partially landed 2026-05-06. Method + property decorators emit `__decorate([...], ClassName.prototype, "name", null);` after the class body. **Parameter decorators now flow through HIR + ts_parser + ts_emit** (commit `6c25c8f`); positional `__param(i, decorator)` metadata is wired. `emitDecoratorMetadata` (reified design type) is the remaining follow-up.
...
1. ~~**Salsa-style query memoization across phases.**~~ 🟢 partial 2026-05-06. `packages/query/` is the generic memoization primitive; **`packages/ts_query/` now ships TS-phase-specific query keys** + **incremental `Program.compileAll` skips files whose source hasn't changed since the prior run** (commit `ed8ab71`). Full reverse-dep tracking through `(file → tokens) → (tokens → AST) → (AST → bound module) → (bound module → diagnostics)` is the remaining slice. *Impact: primary lever for the 80 ms watch target.*
...
4. ~~**Categorize the 5 907-case corpus by feature.**~~ 🟢 partially started 2026-05-05/06. The `builtin_corpus` array now has 56 cases keyed by feature (00..55). **Smoke runs now execute against three local TypeScript conformance subdirectories** (commit `95a716b`) — per-subdir + COMBINED pass-rate prints to stderr without failing the build, giving us per-feature triage data ahead of full external-suite integration. The full 5907-case external suite still needs to be loaded once §6.A.2 lands. *Remaining: load + categorize disk corpus.*
...
6. ~~**Triage failing cases in priority order.**~~ 🟢 ongoing 2026-05-06. Declaration merging → control-flow narrowing → generic inference (~70% of typical conformance gaps). **Smoke runs now execute against a local `tests/conformance/` subdirectory** (commit `07a9936`) so the test step prints per-case PASS/FAIL without failing the build, giving us a triage-feedback channel ahead of the full submodule integration. Each fix becomes its own PR with a one-line journal entry.
...
5. ~~**`codeAction`.**~~ 🟢 partially landed 2026-05-05/06. "Organize Imports" sorts top-level imports alphabetically. **"Add explicit type annotation" quick-fix** for inferred lets landed (commit `80a0662`). fix-all + missing-return-type + infer-parameter-types are remaining quick-fixes.
