...
> **Status (2026-05-08):** 🟡 partial. `packages/ts_conformance/` runner ships a 56-case canon corpus, on-disk fixtures at `tests/conformance/*.ts`, disk `runDirectory`, patience-diff unified mismatch output, a clean **16/16** smoke run against local TypeScript conformance subdirectories, a named-category runner currently clean at **86/86**, and a coarse baseline-aware `types/typeRelationships` survey at **103/175**. Raw direct triage is now at apparentType **0/2**, bestCommonType **8/8**, recursiveTypes **12/13**, subtypesAndSuperTypes **44/52**, typeAndMemberIdentity **47/48**, and typeInference **37/52**; the apparent/subtype raw deltas are expected-error files now correctly surfacing TS2416/TS2411/TS2420. All direct-positive blockers in the surveyed type-relationship folders are now clear; remaining work is expected-error false negatives, exact `.errors.txt` comparison, full local-corpus loading, and per-PR delta gating. See **§6.A** above.
