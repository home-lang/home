...
**Active deliverable:** Phase 6 — full TS conformance corpus integration (vendor `microsoft/TypeScript` submodule); Phase 7 — native codegen for typed TS subset; Phase 4.5 — bundler integration via Bun; Phase 4 follow-ups — generators→state-machine, Stage 3 decorators, `.d.hm` member rendering. Total `zig build test` count: **1569 tests passing** across 30+ packages. Recent landings (2026-05-06): LSP `workspace/willRenameFiles` real impl with import-path rewrites (commit `644364c`), JSX tag-pair `linkedEditingRanges` real impl (commit `949b735`), CI gate on PRs that test pass-count must not regress (commit `b7f4b13`), `ts_lsp_server` `workspace/executeCommand` + `linkedEditingRange` + `workspace/willRenameFiles` + `textDocument/selectionRange` wire handlers (commits `b7d420e`, `b829fa5`, `e94e72b`), `ts_bundler` deduplicates entries in bundle output (commit `9133340`), readonly array type annotation in parser+checker (commit `1db9f01`), comprehensive LSP method-coverage audit + `SUPPORTED_METHODS` list in `ts_lsp_server` docs (commit `89c6b79`), codeAction "Add import for 'X'" for unresolved identifiers (commit `d65e515`).
