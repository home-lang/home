...
**Active deliverable:** Phase 6 — full TS conformance corpus integration (vendor `microsoft/TypeScript` submodule); Phase 7 — native codegen for typed TS subset; Phase 4.5 — bundler integration via Bun; Phase 4 follow-ups — generators→state-machine, Stage 3 decorators, `.d.hm` member rendering. Total `zig build test` count: **1510 tests passing** across 30+ packages. Recent landings (2026-05-06): LSP `initialize`/`shutdown`/`exit` lifecycle wire handlers, re-export forms (`export *` / `export { x } from`), `documentSymbols` includes nested members, sourceMap → `.js.map` write, `.tsbuildinfo` round-trip wiring (read on startup), basic `ThisType<T>` recognition, `this:T` parameter binding, `textDocument/hover` wire handler, shadowing-aware `findReferences`, tsbuildinfo round-trip reader, member-access narrowing on identifier-rooted access, codeAction "Add explicit type annotation", `home-tsc --watch` proper sleep API, hover renders function/class/let declaration shape, gotoDefinition follows imports across files, 8 commonly-used tsconfig fields, semantic-tokens delta-encoded wire format, narrow on `===` with literal RHS, `--pretty` source-snippet diagnostics, incremental `compileAll` skipping unchanged files, `await` unwraps structural `Promise<T>`, LSP `formatDocument` stub + `foldingRanges`, two-level relation cache, `declare global` collection across files, patience-diff baseline output.
